...
export default function ComplianceManager({ drivers }: ComplianceManagerProps) {
  ...
  const processComplianceIssues = () => {
    ...
    setComplianceIssues(
      issues.sort((a, b) => a.daysUntilExpiry - b.daysUntilExpiry)
    );
    setLoading(false);
  };
  ...
  const filterIssues = () => {
    ...
    if (documentFilter !== 'all') {
      filtered = filtered.filter(
        issue => issue.documentType === documentFilter
      );
    }
  ...
  const highIssues = complianceIssues.filter(
    issue => issue.severity === 'high'
  );
  const mediumIssues = complianceIssues.filter(
        ...
        <HStack justify="space-between">
          <VStack align="start" spacing={1}>
            ...
            <Text color="text.secondary">
              Monitor driver document expiries and compliance issues
            </Text>
          </VStack>
          <Button
            leftIcon={<FiMail />}
            ...
            onClick={() => {
              ...
              toast({
                ...
                description:
                  'Sending reminders to all drivers with expiring documents',
                status: 'info',
                duration: 3000,
                isClosable: true,
              });
        ...
        {criticalIssues.length > 0 && (
          <Alert status="error">
            ...
            <VStack align="start" spacing={1}>
              ...
              <AlertDescription>
                {criticalIssues.length} driver(s) have expired documents that
                require immediate attention. These drivers should be blocked
                from accepting new jobs until documents are renewed.
              </AlertDescription>
            </VStack>
          </Alert>
        )}
                      ...
                      <Text
                        ...
                        color={
                          issue.daysUntilExpiry <= 0
                            ? 'red.500'
                            : issue.daysUntilExpiry <= 7
                              ? 'orange.500'
                              : 'text.tertiary'
                        }
                        fontWeight={
                          issue.daysUntilExpiry <= 7 ? 'bold' : 'normal'
                        }
                      >
                        {issue.daysUntilExpiry <= 0
                          ? 'Expired'
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="lg">
        ...
        <ModalContent>
          <ModalHeader>
            <HStack>
              <Icon
                as={getSeverityIcon(selectedIssue?.severity || 'low')}
                color={`${getSeverityColor(selectedIssue?.severity || 'low')}.500`}
              />
            ...
            {selectedIssue && (
              <VStack spacing={4} align="stretch">
                <VStack align="start" spacing={2}>
                  ...
                  <Text fontSize="sm" color="text.secondary">
                    Name: {selectedIssue.driverName}
                  </Text>
                  <Text fontSize="sm" color="text.secondary">
                    Email: {selectedIssue.driverEmail}
                  </Text>
                </VStack>
                ...
                <VStack align="start" spacing={2}>
                  ...
                  <Text fontSize="sm" color="text.secondary">
                    Type: {selectedIssue.documentType}
                  </Text>
                  <Text fontSize="sm" color="text.secondary">
                  ...
                  <Text fontSize="sm" color="text.secondary">
                    Expiry Date:{' '}
                    {new Date(selectedIssue.expiryDate).toLocaleDateString()}
                  </Text>
                  <Text fontSize="sm" color="text.secondary">
                    Days Until Expiry:{' '}
                    {selectedIssue.daysUntilExpiry <= 0
                      ? 'Expired'
                      : selectedIssue.daysUntilExpiry}
                  </Text>
                </VStack>
