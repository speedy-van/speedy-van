      ...
      {activeBookings.length === 0 ? (
        <Box textAlign="center" py={12}>
          <Text fontSize="lg" mb={4}>
            No active moves to track
          </Text>
          <Text color="gray.600" mb={6}>
            You'll see tracking information here when you have an active
            booking.
          </Text>
            ...
            >
              <HStack justify="space-between" mb={4}>
                <VStack align="start" spacing={1}>
                  ...
                  <Text color="gray.600">
                    {booking.pickupAddress?.label || 'Pickup TBD'} →{' '}
                    {booking.dropoffAddress?.label || 'Dropoff TBD'}
                  </Text>
                </VStack>
                <Badge
                  colorScheme={
                    booking.Assignment?.JobEvent?.[0]?.step ===
                    'en_route_to_dropoff'
                      ? 'green'
                      : booking.Assignment?.JobEvent?.[0]?.step ===
                          'loading_completed'
