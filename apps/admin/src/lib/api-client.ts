...
class ApiClient {
  ...
  async getAssignmentSuggestions(
    ...
    return this.get<AssignmentSuggestionsResponse>(
      `/orders/${reference}/assign`
    );
  }
  ...
  async exportOrders(filters?: OrderFilters): Promise<Blob> {
    const response = await fetch(
      `${this.baseUrl}/orders/export?${new URLSearchParams(filters as Record<string, string>)}`,
      {
  ...
  async exportRefunds(filters?: RefundFilters): Promise<Blob> {
    const response = await fetch(
      `${this.baseUrl}/refunds/export?${new URLSearchParams(filters as Record<string, string>)}`,
      {
        headers: this.sessionToken
          ? { Cookie: `session=${this.sessionToken}` }
          : {},
  ...
  subscribeToDriverLocations(
    callback: (location: { driverId: string; lat: number; lng: number }) => void
  ): () => void {
    const eventSource = new EventSource(
      `${this.baseUrl}/drivers/locations/stream`
    );
...
export function validateOrderFilters(filters: OrderFilters): boolean {
  ...
  if (
    filters.dateRange &&
    !['today', 'week', 'month'].includes(filters.dateRange)
  ) {
