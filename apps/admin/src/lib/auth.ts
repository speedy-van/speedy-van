      ...
      async authorize(creds) {
        ...
        try {
          ...
          if (user.role === 'driver') {
            ...
            if (
              user.driverApplication &&
              user.driverApplication.status === 'pending'
            ) {
...
export async function requireRole(role: 'admin' | 'driver' | 'customer') {
  ...
  if (role === 'driver') {
    ...
    if (
      !(session.user as any).driverId ||
      (session.user as any).driverStatus !== 'approved'
    ) {
...
export async function canAccessCustomerPortal(
  userId: string
): Promise<boolean> {
  try {
    const user = await prisma.user.findUnique({
