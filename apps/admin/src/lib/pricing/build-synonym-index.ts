  ...
  public buildIndex(): SynonymIndex {
    ...
    return {
      ...
      metadata: {
        ...
        totalSynonyms: Object.values(synonymsMap).reduce(
          (total, synonyms) => total + synonyms.length,
          0
        ),
  ...
  public validateCatalog(): string[] {
    ...
    this.catalog.forEach((item, index) => {
      ...
      if (!isValidCategory(item.category)) {
        errors.push(
          `Invalid category "${item.category}" for item ${item.id} at line ${index + 1}`
        );
      }
      ...
      if (item.volumeFactor < 0 || item.volumeFactor > 10) {
        errors.push(
          `Invalid volume factor ${item.volumeFactor} for item ${item.id} at line ${index + 1}`
        );
      }
      ...
      if (typeof item.requiresTwoPerson !== 'boolean') {
        errors.push(
          `Invalid requiresTwoPerson value for item ${item.id} at line ${index + 1}`
        );
      }
      if (typeof item.isFragile !== 'boolean') {
        errors.push(
          `Invalid isFragile value for item ${item.id} at line ${index + 1}`
        );
      }
      if (typeof item.requiresDisassembly !== 'boolean') {
        errors.push(
          `Invalid requiresDisassembly value for item ${item.id} at line ${index + 1}`
        );
      }
      ...
      if (typeof item.basePriceHint !== 'number' || item.basePriceHint < 0) {
        errors.push(
          `Invalid basePriceHint for item ${item.id} at line ${index + 1}`
        );
      }
    });
...
export function searchSynonymIndex(
  index: SynonymIndex,
  query: string
): string[] {
