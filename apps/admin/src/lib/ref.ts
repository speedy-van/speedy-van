 ...
 */
export async function createUniqueReference(
  prefix = 'SV-',
  maxTries = 5
): Promise<string> {
  ...
  throw new Error(
    `Could not generate unique reference after ${maxTries} attempts`
  );
}
 ...
 */
export async function reserveUniqueReference(
  prefix = 'SV-',
  maxTries = 5
): Promise<string> {
  for (let i = 0; i < maxTries; i++) {
  ...
  throw new Error(
    `Could not reserve unique reference after ${maxTries} attempts`
  );
}
