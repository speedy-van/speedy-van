...
import { unifiedBookingAnalytics } from '../analytics/unified-booking-analytics';
import { 
  Coordinates, 
  Address, 
  PropertyDetails, 
...
export class UATTestRunner {
  ...
  private async testBookingFlow(): Promise<boolean> {
    try {
      ...
      const step1Data = {
        pickupAddress: '123 Test Street, London, SW1A 1AA',
        dropoffAddress: '456 Test Avenue, Manchester, M1 1AA',
        items: [{ name: 'Test Box', quantity: 1, category: 'boxes' }],
      };

      // Test Step 2: Date and service
      const step2Data = {
      ...
      // Simulate API calls
      const pricingResponse = await unifiedBookingApi.getPricing({
        pickupPostcode: 'SW1A 1AA',
        dropoffPostcode: 'M1 1AA',
        serviceType: 'man-and-van',
        items: step1Data.items,
        date: step2Data.date,
      });
      ...
      const bookingResponse = await unifiedBookingApi.createBooking({
        ...step1Data,
        ...step2Data,
        ...step3Data,
  ...
  private async testRealTimeUpdates(): Promise<boolean> {
    try {
      ...
      const connectionStatus = realtimeBookingUpdates.getConnectionStatus();
      ...
      return connectionStatus.isConnected && updateReceived;
    } catch (error) {
  ...
  private async testPerformanceBenchmarks(): Promise<boolean> {
    try {
      ...
      await unifiedBookingApi.getHealthStatus();
  ...
  private async testErrorHandling(): Promise<boolean> {
    try {
      ...
      const invalidResponse = await unifiedBookingApi.getPricing({
        pickupPostcode: 'INVALID',
        dropoffPostcode: 'INVALID',
...
// Export types
export type { UATTestScenario, UATTestResult, UATTestSession };
