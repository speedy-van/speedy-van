    ...
    for (const driver of drivers) {
      ...
      for (const assignment of driver.Assignment) {
        ...
        const feeAmountPence = Math.round(
          (baseAmountPence + surgeAmountPence) * 0.15
        ); // 15% fee
        ...
        console.log(
          `Created earnings for assignment ${assignment.id}: £${(netAmountPence / 100).toFixed(2)}`
        );
      }
      ...
      for (const assignment of completedAssignments.slice(0, 3)) {
        // Max 3 tips per driver
        ...
        console.log(
          `Created tip for assignment ${assignment.id}: £${(tipAmountPence / 100).toFixed(2)}`
        );
      }
    ...
    for (const driver of drivers) {
      ...
      if (unpaidEarnings.length > 0) {
        ...
        if (totalAmount >= 5000) {
          ...
          console.log(
            `Created payout for driver ${driver.id}: £${(totalAmount / 100).toFixed(2)}`
          );
        }
      }
    }
