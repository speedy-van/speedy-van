...
const ImageGallery: React.FC<ImageGalleryProps> = ({
  ...
  const [selectedImages, setSelectedImages] =
    useState<Map<string, ImageSelection>>(initialSelections);
  ...
  const handleImageSelect = useCallback(
    (imageId: string, quantity: number = 1) => {
  ...
  const handleQuantityChange = useCallback(
    (imageId: string, quantity: number) => {
      const newSelections = new Map(selectedImages);
      const existingSelection = newSelections.get(imageId);
  ...
  const handleCustomNotesChange = useCallback(
    (imageId: string, notes: string) => {
      const newSelections = new Map(selectedImages);
      const existingSelection = newSelections.get(imageId);
      ...
      <ImageModal
        ...
        isSelected={
          selectedImageForModal
            ? selectedImages.has(selectedImageForModal.id)
            : false
        }
      />
