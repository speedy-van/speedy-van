...
import { EnhancedTimeSlot } from './types';
...
const SCHEDULE_CONFIG = {
  ...
  popularSlots: ['09:00', '10:00', '14:00', '15:00'] as const,
} as const;
...
const WEATHER_CONDITIONS = {
  ...
  lightRain: {
    available: true,
    multiplier: 1.1,
    message: 'Light rain expected - extra care for items',
  },
  heavyRain: {
    available: false,
    multiplier: 1.3,
    message: 'Heavy rain - moving not recommended',
  },
  snow: {
    available: false,
    multiplier: 1.5,
    message: 'Snow conditions - moving not available',
  },
  storm: {
    available: false,
    multiplier: 2.0,
    message: 'Storm warning - moving suspended',
  },
} as const;
...
const getWeatherCondition = (date: Date): keyof typeof WEATHER_CONDITIONS => {
  ...
  const conditions: (keyof typeof WEATHER_CONDITIONS)[] = [
    'clear',
    'cloudy',
    'lightRain',
  ];
...
export class ScheduleService {
  ...
  checkDateAvailability(date: Date): AvailabilityCheck {
    ...
    const minBookingTime = addMinutes(
      new Date(),
      SCHEDULE_CONFIG.minAdvanceBookingHours * 60
    );
    ...
    const maxBookingDate = addDays(
      today,
      SCHEDULE_CONFIG.maxAdvanceBookingDays
    );
    ...
    while (currentTime + SCHEDULE_CONFIG.slotDuration <= endMinutes) {
      ...
      if (!isBooked && !hasConflict) {
        ...
        if (
          isTimeInRange(
            timeStr,
            SCHEDULE_CONFIG.peakHours.morning.start,
            SCHEDULE_CONFIG.peakHours.morning.end
          ) ||
          isTimeInRange(
            timeStr,
            SCHEDULE_CONFIG.peakHours.evening.start,
            SCHEDULE_CONFIG.peakHours.evening.end
          )
        ) {
          multiplier *= TIME_SLOT_MULTIPLIERS.peak;
        }
        if (SCHEDULE_CONFIG.popularSlots.includes(timeStr as any)) {
        ...
        if (SCHEDULE_CONFIG.popularSlots.includes(timeStr as any)) demand = 'high';
        ...
        const slot: EnhancedTimeSlot = {
          ...
          popular: SCHEDULE_CONFIG.popularSlots.includes(timeStr as any),
          recommended: false, // Will be set by recommendation engine
          type,
          multiplier,
          weatherImpact: weatherData.available
            ? undefined
            : {
                condition: weather,
  ...
  getScheduleRecommendations(
    ...
    const searchDays =
      flexibility === 'exact' ? 1 : flexibility === 'asap' ? 7 : 14;
  ...
  private getNearbyAvailableDates(targetDate: Date, count: number): Date[] {
    ...
    while (dates.length < count && offset <= 14) {
      ...
      if (
        dates.length < count &&
        this.checkDateAvailability(beforeDate).available
      ) {
...
export const useScheduleRecommendations = (
  startDate: Date,
  options: Parameters<ScheduleService['getScheduleRecommendations']>[1] = { flexibility: 'flexible' }
) => {
...
// Utility function to convert TimeSlot to EnhancedTimeSlot
export const convertToEnhancedTimeSlot = (timeSlot: TimeSlot): EnhancedTimeSlot => {
  const timeStr = timeSlot.startTime;
  const type = getTimeSlotType(timeStr);
  const multiplier = getTimeSlotMultiplier(timeStr, type);
  const demand = SCHEDULE_CONFIG.popularSlots.includes(timeStr as any) ? 'high' : 
                 (type === 'early' || type === 'late') ? 'low' : 'medium';
...
// Helper function to determine time slot type
const getTimeSlotType = (timeStr: string): EnhancedTimeSlot['type'] => {
...
// Helper function to get time slot multiplier
const getTimeSlotMultiplier = (timeStr: string, type: EnhancedTimeSlot['type']): number => {
  let multiplier = 1.0;
