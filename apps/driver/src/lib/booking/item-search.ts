...
const RECENT_ITEMS = ['1', '3', '13', '20', '26'];
...
export const searchItems = (
  ...
  if (!query.trim() && category === 'all') {
    ...
    return MOCK_ITEMS.filter(item =>
      includePopular ? POPULAR_ITEMS.includes(item.id) : true
    )
      .map(item => ({
        ...item,
  ...
  MOCK_ITEMS.forEach(item => {
    ...
    const categoryData =
      ITEM_CATEGORIES[item.category as keyof typeof ITEM_CATEGORIES];
    if (categoryData) {
      const keywordMatch = categoryData.keywords.some(
        keyword =>
      ...
      // Synonym match
      const synonymMatch = categoryData.synonyms.some(
        synonym =>
...
export const getPopularItems = (limit: number = 12): BookingItem[] => {
  return MOCK_ITEMS.filter(item => POPULAR_ITEMS.includes(item.id)).slice(
    0,
    limit
  );
};
...
export const getRecentItems = (limit: number = 6): BookingItem[] => {
  return MOCK_ITEMS.filter(item => RECENT_ITEMS.includes(item.id)).slice(
    0,
    limit
  );
};
...
export const estimateTotals = (
  selectedItems: Array<BookingItem & { quantity: number }>
) => {
  ...
  if (
    totalWeight > 200 ||
    selectedItems.some(item => item.fragile && item.quantity > 0)
  ) {
    recommendedCrew = 2;
  }
