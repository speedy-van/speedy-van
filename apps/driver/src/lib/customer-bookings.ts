...
export async function linkExistingBookingsToCustomer(
  ...
  try {
    console.log('🔗 Linking existing bookings for customer:', {
      userId,
      email,
      phone,
    });
...
export async function getCustomerBookings(userId: string): Promise<{
  ...
  try {
    ...
    return {
      ...
      totalCount:
        formattedLinkedBookings.length + formattedUnlinkedBookings.length,
    };
  } catch (error) {
    ...
    const booking = await prisma.booking.findFirst({
      where: {
        ...
        OR: [
          {
            customerEmail: {
              equals: (await prisma.user.findUnique({ where: { id: userId } }))
                ?.email,
            },
          },
        ],
...
export async function getCustomerBookingStats(userId: string): Promise<{
  ...
  try {
    ...
    const completedBookings = linkedBookings.filter(
      b => b.status === 'COMPLETED'
    ).length;
    const pendingBookings = linkedBookings.filter(b =>
