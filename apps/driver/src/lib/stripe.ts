  ...
  if (process.env.STRIPE_SECRET_KEY) {
    ...
    if (process.env.NODE_ENV === 'production') {
      if (process.env.STRIPE_SECRET_KEY.startsWith('sk_test_')) {
        console.error(
          '❌ ERROR: Using Stripe test keys in production environment!'
        );
        console.error(
          '   Please update STRIPE_SECRET_KEY to use live keys (sk_live_...)'
        );
      ...
      if (process.env.STRIPE_SECRET_KEY.startsWith('sk_live_')) {
        ...
        console.error(
          '❌ ERROR: Invalid Stripe secret key format in production'
        );
        console.error(
          '   Expected: sk_live_..., got:',
