...
class FeatureFlagService {
      ...
      const defaultFlags: FeatureFlag[] = [
        {
          ...
          rolloutPercentage: parseInt(process.env.NEXT_PUBLIC_FEATURE_DRIVER_PORTAL_V2_ROLLOUT || '100'),
        ...
        },
        {
          ...
          enabled: process.env.NEXT_PUBLIC_FEATURE_ADVANCED_NAVIGATION === 'true',
          rolloutPercentage: parseInt(process.env.NEXT_PUBLIC_FEATURE_ADVANCED_NAVIGATION_ROLLOUT || '50'),
          targetEnvironments: ['production', 'staging'],
          metadata: {
        ...
        },
        {
          ...
          enabled: process.env.NEXT_PUBLIC_FEATURE_REAL_TIME_TRACKING === 'true',
          rolloutPercentage: parseInt(process.env.NEXT_PUBLIC_FEATURE_REAL_TIME_TRACKING_ROLLOUT || '25'),
        ...
        },
        {
          ...
          rolloutPercentage: parseInt(process.env.NEXT_PUBLIC_FEATURE_OFFLINE_MODE_ROLLOUT || '100'),
        ...
        },
        {
          ...
          enabled: process.env.NEXT_PUBLIC_FEATURE_ENHANCED_ANALYTICS === 'true',
          rolloutPercentage: parseInt(process.env.NEXT_PUBLIC_FEATURE_ENHANCED_ANALYTICS_ROLLOUT || '10'),
          targetRoles: ['admin'],
          targetEnvironments: ['production', 'staging'],
          metadata: {
  ...
  isEnabled(flagName: string): boolean {
    ...
    if (flag.targetEnvironments && flag.targetEnvironments.length > 0) {
      const currentEnv = this.context.environment || process.env.NODE_ENV || 'development';
    ...
    if (flag.targetRoles && flag.targetRoles.length > 0) {
      if (!this.context.userRole || !flag.targetRoles.includes(this.context.userRole)) {
        return false;
      }
    }
    ...
    if (flag.targetUsers && flag.targetUsers.length > 0) {
      if (!this.context.userId || !flag.targetUsers.includes(this.context.userId)) {
        return false;
      }
    }
    ...
    if (flag.rolloutPercentage < 100) {
      const hash = this.hashString(this.context.userId || this.context.sessionId || 'anonymous');
