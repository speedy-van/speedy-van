...
interface SecurityEvent {
  type: 'login_failure' | 'unauthorized_access' | 'rate_limit_exceeded' | 'suspicious_activity';
  userId?: string;
  ip: string;
  userAgent: string;
  details: any;
...
const SECURITY_CONFIG = {
  ...
  SUSPICIOUS_PATTERNS: {
    ...
    KNOWN_MALICIOUS_USER_AGENTS: [
      'bot', 'crawler', 'scraper', 'sqlmap', 'nikto', 'nmap'
    ]
  }
};
 ...
 */
export async function trackSecurityEvent(event: Omit<SecurityEvent, 'timestamp'>) {
...
async function checkSuspiciousPatterns(event: SecurityEvent) {
  ...
  const recentEventsInWindow = recentEvents.filter(e => e.timestamp > oneHourAgo);
  
  // Check login failure threshold
  const loginFailures = recentEventsInWindow.filter(e => e.type === 'login_failure');
  ...
  const unauthorizedAccess = recentEventsInWindow.filter(e => e.type === 'unauthorized_access');
  if (unauthorizedAccess.length >= SECURITY_CONFIG.UNAUTHORIZED_ACCESS_THRESHOLD) {
  ...
  const rateLimitViolations = recentEventsInWindow.filter(e => e.type === 'rate_limit_exceeded');
  if (rateLimitViolations.length >= SECURITY_CONFIG.RATE_LIMIT_EXCEEDED_THRESHOLD) {
  ...
  const isSuspiciousUserAgent = SECURITY_CONFIG.SUSPICIOUS_PATTERNS.KNOWN_MALICIOUS_USER_AGENTS.some(
  ...
  if (recentEventsInWindow.length > SECURITY_CONFIG.SUSPICIOUS_PATTERNS.RAPID_REQUESTS) {
