...
export class AutocompleteEngine {
  ...
  private findExactMatches(query: string): CatalogItem[] {
    ...
    if (this.synonymIndex.exactMap[query]) {
      const item = this.catalog.find(i => i.id === this.synonymIndex.exactMap[query]);
  ...
  private getPopularSuggestions(maxResults: number, category?: string): AutocompleteSuggestion[] {
  ...
  public getQuickSuggestions(category?: string): AutocompleteSuggestion[] {
    ...
    const popularItemIds = ['sofa-3seat', 'bed-double', 'wardrobe-2door', 'table-dining-4', 'fridge-standard'];
    ...
    if (popularItems.length === 0) {
      return items.slice(0, 5).map(item => this.createSuggestion(item, 0.8, 'category'));
    }

    return popularItems.map(item => this.createSuggestion(item, 0.8, 'category'));
  }
...
export async function getPopularSuggestions(maxResults: number = 10): Promise<AutocompleteSuggestion[]> {
  try {
    // Load catalog data
    const catalog = await loadCatalogDataset();
    const synonymIndex = await loadSynonymIndex();
...
import { getCategoryImage, getCategoryDisplayName, CATEGORY_GROUPS } from './category-images';

export async function getQuickCategories(): Promise<Array<{ name: string; icon: string; count: number; image: string; displayName: string; group: string }>> {
  try {
    ...
    if (!catalog) {
      ...
      return [
        { name: 'sofas', icon: '🛋️', count: 5, image: getCategoryImage('sofas'), displayName: getCategoryDisplayName('sofas'), group: 'furniture' },
        { name: 'beds', icon: '🛏️', count: 3, image: getCategoryImage('beds'), displayName: getCategoryDisplayName('beds'), group: 'furniture' },
        { name: 'tables', icon: '🪑', count: 2, image: getCategoryImage('tables'), displayName: getCategoryDisplayName('tables'), group: 'furniture' },
        { name: 'wardrobe', icon: '🗄️', count: 2, image: getCategoryImage('wardrobe'), displayName: getCategoryDisplayName('wardrobe'), group: 'storage' },
        { name: 'appliances', icon: '🔌', count: 5, image: getCategoryImage('appliances'), displayName: getCategoryDisplayName('appliances'), group: 'appliances' },
        { name: 'electronics', icon: '📺', count: 2, image: getCategoryImage('electronics'), displayName: getCategoryDisplayName('electronics'), group: 'electronics' },
        { name: 'boxes', icon: '📦', count: 4, image: getCategoryImage('boxes'), displayName: getCategoryDisplayName('boxes'), group: 'boxes' },
        { name: 'outdoor', icon: '🌳', count: 3, image: getCategoryImage('outdoor'), displayName: getCategoryDisplayName('outdoor'), group: 'outdoor' }
      ];
    }
    ...
    const allCategories = [
      // Furniture
      { name: 'sofas', icon: '🛋️', count: categoryCounts.get('sofas') || 5, image: getCategoryImage('sofas'), displayName: getCategoryDisplayName('sofas'), group: 'furniture' },
      { name: 'chairs', icon: '🪑', count: categoryCounts.get('chairs') || 3, image: getCategoryImage('chairs'), displayName: getCategoryDisplayName('chairs'), group: 'furniture' },
      { name: 'beds', icon: '🛏️', count: categoryCounts.get('beds') || 3, image: getCategoryImage('beds'), displayName: getCategoryDisplayName('beds'), group: 'furniture' },
      { name: 'tables', icon: '🪑', count: categoryCounts.get('tables') || 2, image: getCategoryImage('tables'), displayName: getCategoryDisplayName('tables'), group: 'furniture' },
      
      // Storage
      { name: 'wardrobe', icon: '🗄️', count: categoryCounts.get('storage') || 2, image: getCategoryImage('wardrobe'), displayName: getCategoryDisplayName('wardrobe'), group: 'storage' },
      { name: 'bookshelf', icon: '📚', count: categoryCounts.get('bookshelf') || 1, image: getCategoryImage('bookshelf'), displayName: getCategoryDisplayName('bookshelf'), group: 'storage' },
      
      // Appliances
      { name: 'appliances', icon: '🔌', count: categoryCounts.get('appliances') || 5, image: getCategoryImage('appliances'), displayName: getCategoryDisplayName('appliances'), group: 'appliances' },
      { name: 'fridge_freezer', icon: '❄️', count: categoryCounts.get('fridge_freezer') || 2, image: getCategoryImage('fridge_freezer'), displayName: getCategoryDisplayName('fridge_freezer'), group: 'appliances' },
      { name: 'washing_machine', icon: '🧺', count: categoryCounts.get('washing_machine') || 1, image: getCategoryImage('washing_machine'), displayName: getCategoryDisplayName('washing_machine'), group: 'appliances' },
      
      // Electronics
      { name: 'electronics', icon: '📺', count: categoryCounts.get('electronics') || 2, image: getCategoryImage('electronics'), displayName: getCategoryDisplayName('electronics'), group: 'electronics' },
      { name: 'tv', icon: '📺', count: categoryCounts.get('tv') || 1, image: getCategoryImage('tv'), displayName: getCategoryDisplayName('tv'), group: 'electronics' },
      { name: 'computer', icon: '💻', count: categoryCounts.get('computer') || 1, image: getCategoryImage('computer'), displayName: getCategoryDisplayName('computer'), group: 'electronics' },
      
      // Boxes
      { name: 'boxes', icon: '📦', count: categoryCounts.get('boxes') || 4, image: getCategoryImage('boxes'), displayName: getCategoryDisplayName('boxes'), group: 'boxes' },
      
      // Outdoor
      { name: 'outdoor', icon: '🌳', count: categoryCounts.get('outdoor') || 3, image: getCategoryImage('outdoor'), displayName: getCategoryDisplayName('outdoor'), group: 'outdoor' },
      
      // Sports
      { name: 'sports', icon: '⚽', count: categoryCounts.get('sports') || 2, image: getCategoryImage('sports'), displayName: getCategoryDisplayName('sports'), group: 'sports' },
      
      // Decorative
      { name: 'lamp', icon: '💡', count: categoryCounts.get('lamp') || 1, image: getCategoryImage('lamp'), displayName: getCategoryDisplayName('lamp'), group: 'decorative' }
    ];
