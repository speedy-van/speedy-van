...
export class ItemNormalizer {
  ...
  private trySpecialConversions(input: string): NormalizedItem[] {
    ...
    if (input.includes('sofa')) {
      if (input.includes('small')) {
        ...
        input.includes('corner') ||
        input.includes('l-shaped') ||
        input.includes('sectional')
      ) {
    ...
    if (input.includes('fridge') || input.includes('refrigerator')) {
      items.push(
        ...this.findItemsByPattern(['fridge-standard', 'fridge-american'])
      );
    } else if (input.includes('freezer')) {
      items.push(
        ...this.findItemsByPattern(['freezer-chest', 'freezer-upright'])
      );
      ...
      items.push(
        ...this.findItemsByPattern(['cooker', 'oven-single', 'oven-double'])
      );
    }
  ...
  private findClosestMatches(input: string): string[] {
    ...
    this.catalog.forEach(item => {
      ...
      const itemText =
        `${item.canonicalName} ${synonyms} ${item.category}`.toLowerCase();
  ...
  public validateItems(items: NormalizedItem[]): string[] {
    ...
    items.forEach(item => {
      ...
      if (item.volumeFactor !== catalogItem.volumeFactor) {
        errors.push(
          `Volume factor mismatch for item ${item.id}: expected ${catalogItem.volumeFactor}, got ${item.volumeFactor}`
        );
      }
    });
...
export async function smartNormalize(
  ...
  if (!catalog || !synonymIndex) {
    const { loadCatalogDataset, loadSynonymIndex } = await import(
      './catalog-dataset'
    );
