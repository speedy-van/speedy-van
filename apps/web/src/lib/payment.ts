...
export async function createBookingPaymentIntent(data: PaymentIntentData) {
  const { bookingId, amount, currency = 'gbp', customerEmail, metadata = {} } = data;
 ...
 */
export async function processFailedPayment(paymentIntentId: string, errorMessage?: string) {
  // Find booking by payment intent ID
  const booking = await prisma.booking.findFirst({
    where: { stripePaymentIntentId: paymentIntentId },
  });
...
export async function processRefund(data: RefundData) {
  ...
  const refund = await createRefund({ paymentIntentId, amount, reason, bookingId });
 ...
 */
export async function updateRefundStatus(refundId: string, status: string, notes?: string) {
