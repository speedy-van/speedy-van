...
export async function logAudit({ req, action, targetType, targetId, before, after }: AuditParams) {
  try {
    const session = await getServerSession(authOptions);
    const ip = req ? (req.headers.get("x-forwarded-for") || req.headers.get("x-real-ip") || null) : null;
    const userAgent = req ? (req.headers.get("user-agent") || null) : null;
    const actorId = (session?.user as any)?.id ?? "anonymous";
    const actorRole = (session?.user as any)?.adminRole || (session?.user as any)?.role || "unknown";
    ...
    if (isSecurityEvent) {
      console.log(`[SECURITY] ${action}: ${actorId} (${actorRole}) from ${ip} - ${targetType}${targetId ? `:${targetId}` : ''}`);
    }
  } catch (err) {
