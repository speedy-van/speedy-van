import { ItemImage, CategoryInfo, ImageCategory } from '../../types/image-selection';
...
Object.keys(CATEGORIES).forEach(category => {
  CATEGORIES[category as ImageCategory].count = IMAGE_DATA[category as ImageCategory].length;
});
...
export function searchImages(query: string, category?: ImageCategory): ItemImage[] {
...
export function getPopularImages(limit: number = 10): ItemImage[] {
  ...
  Object.values(IMAGE_DATA).forEach(categoryImages => {
    popular.push(...categoryImages.slice(0, Math.ceil(limit / Object.keys(IMAGE_DATA).length)));
  });
  return popular.slice(0, limit);
}

export function getRecentlyUsedImages(userId?: string, limit: number = 10): ItemImage[] {
  // In a real app, this would fetch from user's history
  // For now, return popular images
  return getPopularImages(limit);
}

export function getSuggestionsBasedOnSelection(selectedImages: string[], limit: number = 5): ItemImage[] {
  // Simple suggestion algorithm based on selected items
  const selected = selectedImages.map(id => getImageById(id)).filter(Boolean) as ItemImage[];
