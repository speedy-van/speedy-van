...
export function handleApiError(error: unknown, context?: string): NextResponse {
  ...
  return NextResponse.json(
    createErrorResponse(appError, statusCode),
    { status: statusCode }
  );
}
...
export function validateRequest<T>(
  ...
  try {
    ...
    throw new ValidationError(
      ...
      { validationErrors: (error as any).errors, context }
    );
  }
}
...
export function requireRole(session: any, requiredRole: string, context?: string) {
...
export function handleExternalServiceError(error: unknown, service: string, context?: string): never {
  ...
  throw new ExternalServiceError(
    `${service} service error: ${errorMessage}`,
    { 
      service,
      originalError: error,
      context 
