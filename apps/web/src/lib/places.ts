import { PlacesIndex, PlacesIndexSchema, UkPlace, UkPlaceType } from "@/data/places.schema";
...
const _load = async (): Promise<DBShape> => {
  try {
    ...
    return { all, bySlug, buckets, updatedAt: parsed.updatedAt };
  } catch (error) {
    console.error('Error loading places data:', error);
    // Fallback to empty data structure
    return { 
...
export async function getNearbyPlaces(place: UkPlace, limit = 12): Promise<UkPlace[]> {
  try {
    const db = await getDB();
...
function _getNearbyPlacesFromDB(db: DBShape, place: UkPlace, limit: number): UkPlace[] {
  // collect neighbors from 9 buckets (cell + surrounding)
  const latSteps = [-1,0,1], lonSteps = [-1,0,1];
  const candidates: UkPlace[] = [];
  for (const i of latSteps) for (const j of lonSteps) {
    const key = bucketKey(place.lat + i*BUCKET_SIZE, place.lon + j*BUCKET_SIZE);
  ...
  const bucketsDir = { N: [], S: [], E: [], W: [] } as Record<string, {p:UkPlace;dKm:number}[]>;
  for (const s of scored) {
    const dir = s.p.lat >= place.lat ? "N" : "S";
    const dir2 = s.p.lon >= place.lon ? "E" : "W";
    bucketsDir[dir].push(s); bucketsDir[dir2].push(s);
  }
  const mixed: UkPlace[] = [];
  const pool = [...scored];
...
export function canonicalFor(place: UkPlace): string {
  if (place.population && place.population < 10000 && place.parentSlug) {
    return `https://speedy-van.co.uk/uk/${place.parentSlug}`;
  }
