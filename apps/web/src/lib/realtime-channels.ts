...
export class RealtimeManager {
  ...
  private subscribeToPusher(channelName: string, event: string, handler: EventHandler) {
    try {
      ...
      channel.bind(event, (data: any) => {
        try {
          ...
          console.error(`Error in event handler for ${channelName}:${event}:`, error);
        }
      });
  ...
  private unsubscribe(channelName: string, event: string, handler: EventHandler) {
    const subscriptionKey = `${channelName}:${event}`;
    ...
    if (subscriptions) {
      const subscription = subscriptions.find(s => s.event === event && s.handler === handler);
      if (subscription) {
  ...
  private setupPollingFallback(channelName: string, event: string, interval: number) {
  ...
  private async pollForUpdates(channelName: string, event: string) {
    ...
    try {
      ...
      const response = await fetch(`/api/realtime/poll?channel=${channelName}&event=${event}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
...
export function getRealtimeManager(config?: ConstructorParameters<typeof RealtimeManager>[0]): RealtimeManager {
...
export async function initializeRealtime(config?: ConstructorParameters<typeof RealtimeManager>[0]): Promise<RealtimeManager> {
