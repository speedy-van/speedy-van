 ...
 */
async function searchRealAddresses(
  query: string
): Promise<AddressSuggestion[]> {
  ...
  try {
    ...
    return data.features.map((feature: any) => {
      ...
      const place =
        context.find((ctx: any) => ctx.id.startsWith('place'))?.text || '';
      const postcode =
        context.find((ctx: any) => ctx.id.startsWith('postcode'))?.text || '';
      const region =
        context.find((ctx: any) => ctx.id.startsWith('region'))?.text || '';
