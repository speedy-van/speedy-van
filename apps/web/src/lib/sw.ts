...
export async function storeActionForSync(action: {
  ...
  try {
    ...
    if (sw) {
      ...
      if (action.url.includes('/api/driver/status') || action.url.includes('/api/driver/location')) {
        await registerDriverStatusSync();
      } else if (action.url.includes('/api/driver/jobs') || action.url.includes('/api/driver/accept') || action.url.includes('/api/driver/decline')) {
