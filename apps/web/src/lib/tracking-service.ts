...
class TrackingService {
   ...
   */
  async lookupBooking(bookingCode: string, includeTracking: boolean = true): Promise<TrackingData | null> {
    try {
      const response = await fetch(`/api/track/${encodeURIComponent(bookingCode)}?tracking=${includeTracking}&realtime=true`);
