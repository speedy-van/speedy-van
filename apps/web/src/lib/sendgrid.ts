...
export async function sendReceipt(to: string, booking: { reference: string; totalGBP: number; currency: string; paidAt?: Date | null }) {
  try {
    if (!process.env.SENDGRID_API_KEY || !process.env.SENDGRID_API_KEY.startsWith('SG.')) {
    ...
    const pdf = await buildReceiptPDF({
      company: { name: "Speedy Van", address: "140 Charles Street, Glasgow City, G21 2QB", email: process.env.MAIL_FROM },
