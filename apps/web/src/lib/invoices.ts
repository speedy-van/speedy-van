 ...
 */
export async function createInvoiceForBooking(input: CreateInvoiceInput): Promise<InvoiceData> {
  try {
    ...
    console.log(`✅ Mock invoice created: ${mockInvoice.invoiceNumber} for booking ${booking.reference}`);
 ...
 */
export async function getCustomerInvoices(customerEmail: string, page: number = 1, limit: number = 20) {
  try {
    // TODO: Implement when Invoice model is added to schema
 ...
 */
export async function getAdminInvoices(page: number = 1, limit: number = 20, filters: {
  search?: string;
  status?: string;
  fromDate?: string;
  toDate?: string;
