...
class LogExporter {
  ...
  async exportLogs(options: ExportOptions, actorId: string): Promise<ExportResult> {
  ...
  private async getAuditLogs(options: ExportOptions): Promise<any[]> {
    ...
    if (options.filters?.entity) {
      where.targetType = { contains: options.filters.entity, mode: "insensitive" };
    }

    if (options.filters?.actorRole) {
      where.actorRole = { contains: options.filters.actorRole, mode: "insensitive" };
    }
  ...
  private async getSystemLogs(options: ExportOptions): Promise<any[]> {
    ...
    if (options.filters?.level) {
      filteredLogs = filteredLogs.filter(log => log.level === options.filters?.level);
    }

    if (options.filters?.service) {
      filteredLogs = filteredLogs.filter(log => log.service === options.filters?.service);
    }

    if (options.dateFrom) {
      filteredLogs = filteredLogs.filter(log => new Date(log.timestamp) >= options.dateFrom!);
    }

    if (options.dateTo) {
      filteredLogs = filteredLogs.filter(log => new Date(log.timestamp) <= options.dateTo!);
    }
  ...
  private async generateExportData(logs: any[], options: ExportOptions): Promise<string | Buffer> {
