...
export class UKAddressService {
  ...
  constructor() {
    if (!MAPBOX_TOKEN) {
      console.warn(
        'Mapbox token not found. Address autocomplete will use fallback service.'
      );
    }
  }
  ...
  async validateAddress(address: Partial<Address>): Promise<{
    ...
    try {
      ...
      const closeMatch = suggestions.find(suggestion => {
        ...
        return (
          normalizePostcode(suggestionPostcode) ===
      ...
      return {
        ...
        suggestions,
        errors: [
          'Address may not be accurate. Please select from suggestions.',
        ],
      };
    } catch (error) {
      return {
        isValid: false,
        errors: [
          'Unable to validate address. Please check your internet connection.',
        ],
      };
    }
  }
  ...
  private async fallbackAddressSearch(
    query: string,
    limit: number
  ): Promise<AddressSuggestion[]> {
  ...
  private async fallbackPostcodeSearch(
    postcode: string,
    limit: number
  ): Promise<AddressSuggestion[]> {
  ...
  private fallbackDistanceCalculation(
    origin: Coordinates,
    destination: Coordinates
  ): DistanceResult {
...
export const useDebouncedAddressSearch = (delay: number = 300) => {
  ...
  const debouncedSearch = debounce(
    async (
      ...
      try {
        const suggestions = await addressService.searchAddresses(
          query,
          options
        );
