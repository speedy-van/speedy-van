...
import { FiUpload, FiDownload, FiEye, FiTrash2, FiFileText, FiShield, FiTruck, FiCalendar } from "react-icons/fi";
...
export default function DocumentsPage() {
  ...
  const getDocumentIcon = (category: string) => {
    const config = DOCUMENT_CATEGORIES[category as keyof typeof DOCUMENT_CATEGORIES];
  ...
  const missingRequired = uploadedDocs.filter(doc => doc.required && !doc.latest);
  ...
  return (
    <Box>
          ...
          <Card key={doc.category}>
            <CardBody>
              <Flex align="center" mb={3}>
                ...
                {doc.required && (
                  <Badge colorScheme="red" ml={2} size="sm">Required</Badge>
                )}
              </Flex>
                ...
                <VStack align="stretch" spacing={2}>
                  <HStack justify="space-between">
                    <Text fontSize="sm" color="gray.600">
                      Uploaded: {new Date(doc.latest.uploadedAt).toLocaleDateString()}
                    </Text>
                  ...
                  {doc.latest.expiresAt && (
                    <Text fontSize="sm" color="gray.600">
                      Expires: {new Date(doc.latest.expiresAt).toLocaleDateString()}
                    </Text>
                  )}
                  ...
                  {doc.latest.verifiedAt && (
                    <Text fontSize="sm" color="green.600">
                      Verified: {new Date(doc.latest.verifiedAt).toLocaleDateString()}
                    </Text>
                  )}
                </VStack>
              ) : (
      ...
      <Card>
        <CardBody>
          <Heading size="md" mb={4}>Document History</Heading>
                  ...
                  <Tr key={doc.id}>
                    <Td>
                      <HStack>
                        ...
                        <Text>{DOCUMENT_CATEGORIES[doc.category as keyof typeof DOCUMENT_CATEGORIES]?.label || doc.category}</Text>
                      </HStack>
                    </Td>
                    ...
                    <Td>
                      {doc.verifiedAt ? (
                        ...
                        <Text fontSize="sm" color="gray.500">Not verified</Text>
                      )}
                    </Td>
                    <Td>
                      <HStack spacing={2}>
          ...
          <ModalBody pb={6}>
            <VStack spacing={4} align="stretch">
              ...
              <HStack spacing={4} justify="flex-end">
                <Button variant="outline" onClick={onClose} isDisabled={uploading}>
                  Cancel
                </Button>
                <Button
                  colorScheme="blue"
