...
export default function ActiveJobPage() {
  ...
  const handleCompleteStep = async () => {
    ...
    if (config.requiresMedia && mediaUrls.length === 0) {
      toast({
        ...
        description: config.mediaDescription || "Please add photos for this step",
        status: "warning",
        duration: 3000,
        isClosable: true,
      });
    ...
    try {
      const response = await fetch(`/api/driver/jobs/${activeJob.id}/progress`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  if (loading) {
    return (
      <Box minH={{ base: '100dvh', md: '100vh' }} bg={bgColor} display="flex" alignItems="center" justifyContent="center">
        <VStack spacing={4}>
              ...
              <VStack align="stretch" spacing={4}>
                ...
                <Box>
                  <HStack justify="space-between" mb={2}>
                    <Text fontSize="md" fontWeight="medium">Progress</Text>
                    <Text fontSize="md" fontWeight="medium">{Math.round(activeJob.progressPercentage)}%</Text>
                  </HStack>
                  <Progress 
            ...
            <CardBody>
              <VStack align="stretch" spacing={4}>
                ...
                <LiveMap
                  ...
                  pickupLocation={activeJob.job.pickupLat && activeJob.job.pickupLng ? {
                    lat: activeJob.job.pickupLat,
                    lng: activeJob.job.pickupLng,
                    label: 'Pickup'
                  } : undefined}
                  dropoffLocation={activeJob.job.dropoffLat && activeJob.job.dropoffLng ? {
                  ...
                  route={routeData?.route ? {
                    coordinates: [
                      [driverLocation?.lng || 0, driverLocation?.lat || 0],
                      [activeJob.job.pickupLng || 0, activeJob.job.pickupLat || 0],
                      [activeJob.job.dropoffLng || 0, activeJob.job.dropoffLat || 0]
                    ],
                  ...
                  onNavigateToPickup={() => {
                    if (routeData?.navigation?.pickupUrl?.universal) {
                      window.open(routeData.navigation.pickupUrl.universal, '_blank');
                    }
                  }}
                  onNavigateToDropoff={() => {
                    if (routeData?.navigation?.dropoffUrl?.universal) {
                      window.open(routeData.navigation.dropoffUrl.universal, '_blank');
                    }
                  }}
                  height={400}
                />
            ...
            <CardBody>
              <VStack align="stretch" spacing={4}>
                ...
                <Text fontSize="sm" color="gray.600">
                  Share this tracking link with your customer so they can track the delivery in real-time.
                </Text>
            ...
            <CardBody>
              <VStack align="start" spacing={4}>
                ...
                <VStack align="start" spacing={3} w="100%">
                  <HStack>
                    <FiMapPin color="gray" />
                    <Text fontSize="md" fontWeight="medium">Pickup</Text>
                  </HStack>
                  ...
                  <HStack>
                    <FiMapPin color="gray" />
                    <Text fontSize="md" fontWeight="medium">Dropoff</Text>
                  </HStack>
                  <Text fontSize="md" color="gray.700" pl={6}>
                  ...
                  {activeJob.job.contactName && (
                    <>
                      <HStack>
                        <FiUser color="gray" />
                        <Text fontSize="md" fontWeight="medium">Contact</Text>
                      </HStack>
                      <VStack align="start" pl={6} spacing={2}>
                        <Text fontSize="md" color="gray.700">{activeJob.job.contactName}</Text>
                        {activeJob.job.contactPhone && (
                          <HStack>
                            <FiPhone size={16} />
                            <Text fontSize="md" color="blue.600">{activeJob.job.contactPhone}</Text>
                          </HStack>
                        )}
                        {activeJob.job.contactEmail && (
                          <HStack>
                            <FiMail size={16} />
                            <Text fontSize="md" color="blue.600">{activeJob.job.contactEmail}</Text>
                          </HStack>
                        )}
                      </VStack>
                    </>
                  ...
                  {activeJob.job.specialInstructions && (
                    <>
                      <HStack>
                        ...
                        <Text fontSize="md" fontWeight="medium">Special Instructions</Text>
                      </HStack>
                      <Text fontSize="md" color="gray.700" pl={6}>
                        {activeJob.job.specialInstructions}
                      </Text>
                      ...
                      >
                        <VStack align="start" spacing={2} flex={1}>
                          <HStack justify="space-between" w="100%">
                            <Text fontWeight="medium" fontSize="md">{config.title}</Text>
                            {isCompleted && <FiCheck />}
                          </HStack>
              ...
              {selectedStep && stepConfig[selectedStep as keyof typeof stepConfig]?.requiresMedia && (
                <Box>
                  <Text fontWeight="medium" mb={3}>
                    {stepConfig[selectedStep as keyof typeof stepConfig]?.mediaDescription || "Add Photos"}
                  </Text>
