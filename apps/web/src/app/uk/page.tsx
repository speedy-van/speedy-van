...
export async function generateMetadata(): Promise<Metadata> {
  return {
    ...
    description: "Professional man & van and full removals across the UK. Fixed quotes, same-day availability, insured movers.",
  ...
  return (
    <main className="uk-index-container">
      <section className="uk-index-hero">
        <h1>UK Coverage</h1>
        <p>Choose your area to get an instant removal quote. We cover every corner of the United Kingdom.</p>
      </section>
      ...
      <section className="uk-index-popular-areas">
        ...
        <div className="uk-index-areas-grid">
          {places.places.slice(0, 12).map(place => (
            <Link key={place.slug} href={`/uk/${place.slug}`} className="uk-index-area-card">
              <h3>{place.name}</h3>
              <p className="uk-index-place-type">{place.type}</p>
              {place.population && (
                <p className="uk-index-population">{place.population.toLocaleString()} people</p>
              )}
            </Link>
          ))}
        </div>
