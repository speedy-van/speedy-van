...
export async function generateMetadata({ params }: { params: { slug: string[] } }) {
  ...
  const place = places.places.find(p => p.slug === slug);
...
export default async function CatchAllUkPage({ params }: { params: { slug: string[] } }) {
  const slug = params.slug[params.slug.length - 1]; // Get the last slug part
  const place = places.places.find(p => p.slug === slug);
  ...
  return (
    <main className="uk-place-container">
      ...
      <section className="uk-place-content">
        ...
        <p>Professional moving services in {place.name}. We provide reliable, affordable removals with transparent pricing and full insurance coverage.</p>
      ...
      <section className="uk-place-navigation">
        ...
        <div className="area-links">
          {places.places
            ...
            .slice(0, 8)
            .map(nearby => (
              <Link key={nearby.slug} href={`/uk/${nearby.slug}`} className="area-link">
                {nearby.name}
              </Link>
            ))}
        </div>
