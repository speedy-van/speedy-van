...
import { getPlaceBySlug, getNearbyPlaces, canonicalFor, getAllPlaces, routeSlug } from "@/lib/places";
...
export async function generateStaticParams() {
  ...
  return places.places.map((p) => ({ place: p.slug }));
}

export async function generateMetadata({ params }: { params: { place: string } }): Promise<Metadata> {
...
function JsonLd({ place, nearby }: { place: any; nearby: any[] }) {
  const json = {
    ...
    breadcrumb: {
      "@type": "BreadcrumbList",
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Home", item: "https://speedy-van.co.uk" },
        { "@type": "ListItem", position: 2, name: "UK", item: "https://speedy-van.co.uk/uk" },
        { "@type": "ListItem", position: 3, name: place.name, item: `https://speedy-van.co.uk/uk/${place.slug}` }
      ]
    }
  };
...
function CityContent({ place }: { place: any }) {
  return (
    <div className="city-content">
      ...
      <p>As one of the UK's largest cities, {place.name} offers extensive removal services with our network of professional movers covering all postcodes.</p>
      <ul>
...
function TownContent({ place }: { place: any }) {
  return (
    <div className="town-content">
      ...
      <p>Trusted local removal services in {place.name} and surrounding villages. We know the area and provide reliable, affordable moving solutions.</p>
      <ul>
...
function VillageContent({ place }: { place: any }) {
  return (
    <div className="village-content">
      ...
      <p>Personalized removal services for {place.name} and nearby areas. We understand rural moving challenges and provide tailored solutions.</p>
      <ul>
  ...
  if (!place) {
    return (
      <div className="error-container">
        ...
        <Link href="/uk" className="btn btn-primary">Back to UK areas</Link>
      </div>
    );
  }
  ...
  return (
    <main className="uk-place-container">
      ...
      <section className="uk-place-hero">
        ...
        <p>Fast, insured removals in {place.name} and surrounding areas. Transparent pricing and real-time tracking.</p>
        <div className="uk-place-cta">
          <Link className="btn btn-primary" href="/booking">Get a quote</Link>
