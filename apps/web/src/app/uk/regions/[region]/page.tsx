...
export async function generateStaticParams() {
  ...
  const regions = [...new Set(places.places.map(p => p.region).filter(Boolean))];
...
export async function generateMetadata({ params }: { params: { region: string } }) {
  const regionName = params.region.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
...
export default async function RegionPage({ params }: { params: { region: string } }) {
  const regionName = params.region.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  const regionPlaces = places.places.filter(p => 
  ...
  return (
    <main className="uk-place-container">
      <section className="uk-place-hero">
        ...
        <p>Professional removal services across {regionName}. Fast, reliable, and fully insured moving solutions.</p>
      ...
      <section className="uk-place-content">
        ...
        <p>We provide comprehensive removal services across {regionName}, from major cities to smaller towns and villages. Our network of professional movers ensures reliable service wherever you are.</p>
      ...
      <section className="uk-place-areas">
        ...
        <div className="areas-grid">
          {regionPlaces.map(place => (
            <Link key={place.slug} href={`/uk/${place.slug}`} className="area-card">
              <h4>{place.name}</h4>
              <p className="place-type">{place.type}</p>
              {place.population && (
                <p className="population">{place.population.toLocaleString()} people</p>
              )}
            </Link>
          ))}
        </div>
      ...
      <section className="uk-place-navigation">
        ...
        <div className="region-links">
          {[...new Set(places.places.map(p => p.region).filter(Boolean))]
            .filter(r => r !== regionName)
            .slice(0, 6)
            .map(region => (
              <Link 
                key={region} 
                href={`/uk/regions/${region.toLowerCase().replace(/\s+/g, '-')}`} 
