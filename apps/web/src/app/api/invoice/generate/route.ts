...
export async function POST(request: NextRequest) {
  try {
    const { bookingId, amount, customerData, bookingDetails } = await request.json();
...
function generateProfessionalInvoice(data: {
  ...
  const invoice = {
    ...
    invoice: {
      ...
      dueDate: formatDate(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()), // 30 days from now
    ...
    services: [
      {
        ...
        total: data.amount,
        details: data.bookingDetails.description || 'Professional moving service'
      }
    ],
    totals: {
      subtotal: data.amount,
      vat: data.amount * 0.20, // 20% VAT
