...
function mapOpenMeteoCodeToTheme(code: number): { theme: Theme; condition: string } {
  // https://open-meteo.com/en/docs
  if (code === 0) return { theme: 'sunny', condition: 'Clear sky' };
  if ([1, 2, 3].includes(code)) return { theme: 'clouds', condition: 'Partly cloudy' };
  if ([45, 48].includes(code)) return { theme: 'fog', condition: 'Fog' };
  if ([51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82].includes(code)) return { theme: 'rain', condition: 'Rain' };
  if ([71, 73, 75, 77, 85, 86].includes(code)) return { theme: 'snow', condition: 'Snow' };
  if ([95, 96, 99].includes(code)) return { theme: 'rain', condition: 'Thunderstorm' };
