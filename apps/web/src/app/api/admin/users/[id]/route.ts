...
export const PUT = withApiHandler(async (
  ...
  const paramValidation = parseParams(params, idParam);
  ...
  if (existingUser.adminRole === 'superadmin' && isActive === false) {
    ...
    if (currentUser?.adminRole !== 'superadmin') {
      return httpJson(403, { error: 'Only superadmin can deactivate superadmin accounts' });
    }
  }
  ...
  return httpJson(200, updatedUser);
});
...
export const DELETE = withApiHandler(async (
  ...
  if (existingUser.adminRole === 'superadmin') {
    ...
    if (superadminCount <= 1) {
      return httpJson(400, { error: 'Cannot delete the last superadmin account' });
    }
  }
  ...
  return httpJson(200, { message: 'Admin user deleted successfully' });
});
