...
export async function GET(
  ...
  try {
    ...
    await logAudit({
      ...
      before: null,
      after: { reportId: params.id, format, filename }
    });
...
function generateMockExportData() {
  const headers = ['Date', 'Revenue', 'Orders', 'AOV', 'Completion Rate', 'Driver'];
  const data = [];
  ...
  for (let i = 30; i >= 0; i--) {
    ...
    data.push([
      ...
      ['John Smith', 'Sarah Johnson', 'Mike Wilson'][Math.floor(Math.random() * 3)]
    ]);
  }
