...
export async function PATCH(
  ...
  try {
    ...
    const { name, description, metrics, dimensions, filters, schedule, recipients, status } = body;
    ...
    const updatedReport = {
      ...
      lastRun: new Date().toISOString(),
      nextRun: schedule !== "none" ? new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() : undefined,
      createdAt: new Date().toISOString(),
      createdBy: session.user.id
    };
    ...
    await logAudit({
      ...
      targetType: "analytics_report",
      before: { reportId: params.id },
      after: { reportId: params.id, changes: body }
    });
