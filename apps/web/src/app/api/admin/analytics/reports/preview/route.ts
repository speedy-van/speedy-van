...
export async function POST(request: Request) {
  ...
  try {
    ...
    const days = dateRange === '7d' ? 7 : dateRange === '90d' ? 90 : dateRange === '6m' ? 180 : 30;
    ...
    await logAudit({
      ...
      before: null,
      after: { metrics, dimensions, filters }
    });
...
function generateMockData(metrics: string[], dimensions: string[], days: number) {
  const data = [];
  const now = new Date();
  ...
  if (dimensions.includes('date')) {
    ...
    // Driver-based data
    const drivers = ['John Smith', 'Sarah Johnson', 'Mike Wilson', 'Emma Davis', 'David Brown'];
