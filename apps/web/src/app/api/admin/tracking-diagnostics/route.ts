...
async function checkActiveDrivers() {
  try {
    ...
    const driverIds = [...new Set([...activeDrivers.map(d => d.driverId), ...driversWithJobs.map(a => a.driverId)])];
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        activeDriversList: activeDrivers.map(d => {
        ...
        driversWithJobsList: driversWithJobs.map(a => {
