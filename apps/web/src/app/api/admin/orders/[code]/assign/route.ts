...
export async function POST(
  ...
  try {
    ...
    if (booking.status !== "CONFIRMED") {
      return new Response("Booking is not in confirmed status", { status: 400 });
    }
...
export async function GET(req: Request, { params }: { params: { code: string } }) {
  const s = await getServerSession(authOptions);
  if (!s?.user || (s.user as any).role !== "admin") return new Response("Unauthorized", { status: 401 });
  ...
  const scoredDrivers = availableDrivers.map(driver => {
    ...
    const experienceScore = Math.min(10, Math.floor((Date.now() - driver.createdAt.getTime()) / (1000 * 60 * 60 * 24 * 30))); // Months of experience
