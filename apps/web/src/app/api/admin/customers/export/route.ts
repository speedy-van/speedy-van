...
export async function GET(request: Request) {
  ...
  try {
    ...
    const exportData = customers.map(customer => {
      ...
      const totalSpent = customer.bookings.reduce((sum, booking) => sum + (booking.totalGBP || 0), 0);
      const activeTickets = customer.supportTickets.filter(ticket => ticket.status === 'open').length;
      const completedBookings = customer.bookings.filter(booking => booking.status === 'COMPLETED').length;
    ...
    const headers = [
      ...
      'Is Active', 'Email Verified', 'Total Bookings', 'Completed Bookings',
      'Total Spent (£)', 'Active Tickets', 'Address Count', 'Contact Count',
