...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!paymentId || !amount || !reason) {
      return NextResponse.json({ error: 'Payment ID, amount, and reason are required' }, { status: 400 });
    }
    ...
    const totalRefunded = payment.Refund.reduce((sum, refund) => sum + refund.amount, 0);
    const remainingAmount = payment.amount - totalRefunded;

    if (amount > remainingAmount) {
      return NextResponse.json({ error: 'Refund amount exceeds remaining payment amount' }, { status: 400 });
    }
