...
async function calculateDriverScore(
  ...
  if (driver.bookings.length >= rules.maxJobs) {
    reasons.push(`Driver has ${driver.bookings.length} active jobs (max: ${rules.maxJobs})`);
    return { driverId: driver.id, driver, score: 0, factors, reasons };
  }

  if (driver.rating && driver.rating < rules.rating) {
    reasons.push(`Driver rating ${driver.rating} below minimum ${rules.rating}`);
    return { driverId: driver.id, driver, score: 0, factors, reasons };
  }
  ...
  if (distance > rules.radius) {
    reasons.push(`Driver too far: ${Math.round(distance / 1000)}km (max: ${rules.radius / 1000}km)`);
    return { driverId: driver.id, driver, score: 0, factors, reasons };
  }
  ...
  const completedJobs = driver.bookings?.filter((b: any) => b.status === 'completed').length || 0;
  ...
  ) / (rules.distanceWeight + rules.ratingWeight + rules.experienceWeight + rules.loadWeight + 10);
