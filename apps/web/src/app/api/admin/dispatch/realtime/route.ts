...
export async function GET(request: NextRequest) {
  try {
    ...
    const processedLocations = driverLocations.reduce((acc, ping) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'create_incident':
        const { category, description, severity, driverId: incidentDriverId, jobId: incidentJobId } = data;
        
        const incident = await prisma.driverIncident.create({
          data: {
            driverId: incidentDriverId || null,
            assignmentId: incidentJobId ? await getAssignmentId(incidentJobId) : null,
