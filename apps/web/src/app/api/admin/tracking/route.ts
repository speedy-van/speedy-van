...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      confirmedBookings: trackingBookings.filter(b => b.status === 'CONFIRMED').length,
      inProgressBookings: trackingBookings.filter(b => b.status === 'CONFIRMED').length,
      withLiveTracking: trackingBookings.filter(b => b.currentLocation).length,
      averageProgress: trackingBookings.length > 0 
        ? Math.round(trackingBookings.reduce((sum, b) => sum + b.routeProgress, 0) / trackingBookings.length)
        : 0
    };
