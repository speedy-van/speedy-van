  ...
  try {
    ...
    await logAudit({
      ...
      before: {
        ...
        documents: driver.documents.map(d => ({ id: d.id, status: d.status }))
      },
      after: {
        onboardingStatus: updateData.onboardingStatus || driver.onboardingStatus,
        reviewNotes,
        documentId,
...
export async function GET(
  ...
  try {
    ...
    const complianceStatus = {
      ...
      documents: {
        rtw: driver.documents.some(d => d.category === 'rtw' && d.status === 'verified'),
        licence: driver.documents.some(d => d.category === 'licence' && d.status === 'verified'),
        insurance: driver.documents.some(d => d.category === 'insurance' && d.status === 'verified'),
        mot: driver.documents.some(d => d.category === 'mot' && d.status === 'verified'),
        v5c: driver.documents.some(d => d.category === 'v5c' && d.status === 'verified'),
        dbs: driver.documents.some(d => d.category === 'dbs' && d.status === 'verified'),
      }
    };

    const allRequiredDocumentsVerified = Object.values(complianceStatus.documents).every(Boolean);
    ...
    return NextResponse.json({
      ...
      compliance: {
        ...
        allInfoComplete,
        canApprove: allInfoComplete && allRequiredDocumentsVerified && driver.onboardingStatus === 'in_review',
      },
