...
export async function GET(_: Request, { params }: { params: { code: string } }) {
  ...
  return Response.json({ 
    booking,
    timeline,
    canReschedule: ['open', 'pending_dispatch', 'confirmed'].includes(booking.status),
    canCancel: ['open', 'pending_dispatch', 'confirmed'].includes(booking.status),
    canTrack: ['assigned', 'en_route_pickup', 'arrived', 'loaded', 'en_route_dropoff'].includes(booking.status)
  });
}
...
function generateTimeline(booking: any) {
  ...
  if (['confirmed', 'assigned', 'en_route_pickup', 'arrived', 'loaded', 'en_route_dropoff', 'completed'].includes(booking.status)) {
    timeline.push({
      id: 'confirmed',
      status: 'completed',
  ...
  if (['assigned', 'en_route_pickup', 'arrived', 'loaded', 'en_route_dropoff', 'completed'].includes(booking.status)) {
    timeline.push({
      id: 'assigned',
      status: 'completed',
      title: 'Crew Assigned',
      description: booking.driver ? `Your crew has been assigned: ${booking.driver.user.name}` : 'Your crew has been assigned',
      timestamp: booking.createdAt, // Would be actual timestamp
      icon: '👥'
    });
  }

  if (['en_route_pickup', 'arrived', 'loaded', 'en_route_dropoff', 'completed'].includes(booking.status)) {
    timeline.push({
      id: 'en_route_pickup',
      status: 'completed',
  ...
  if (['arrived', 'loaded', 'en_route_dropoff', 'completed'].includes(booking.status)) {
    timeline.push({
      id: 'arrived',
      status: 'completed',
