...
export async function POST(request: NextRequest) {
  try {
    ...
    const channelPattern = /^(private|presence)-(tracking|admin-tracking)(-\w+)?$/;
    ...
    if (channel_name.startsWith('private-admin-tracking') && userRole !== 'admin') {
      return NextResponse.json({ error: 'Admin access required' }, { status: 403 });
    }

    if (channel_name.startsWith('private-tracking-') && userRole !== 'driver' && userRole !== 'customer') {
      return NextResponse.json({ error: 'Driver or customer access required' }, { status: 403 });
    }
