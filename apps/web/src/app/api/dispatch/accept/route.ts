...
export async function POST(req: Request) {
  ...
  if (!session?.user || (session.user as any).role !== 'driver') return new Response('Unauthorized', { status: 401 });
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!offer || offer.driverId !== driver.id || offer.status !== 'invited' || (offer.expiresAt && offer.expiresAt < new Date())) {
      ...
      await tx.booking.update({ where: { id: bookingId }, data: { driverId: driver.id, status: 'CONFIRMED' } });
      await tx.assignment.update({ where: { bookingId }, data: { status: 'accepted' } });
