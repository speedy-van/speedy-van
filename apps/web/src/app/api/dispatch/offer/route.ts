...
export async function POST(req: Request) {
  const { bookingId, driverId, ttlSeconds = 90, round = 1, score = 0 } = await req.json();
  if (!bookingId || !driverId) return NextResponse.json({ error: 'missing parameters' }, { status: 400 });
  ...
  try {
    ...
    const booking = await prisma.booking.findUnique({
      where: { id: bookingId },
      select: { id: true, totalGBP: true, pickupAddress: true, dropoffAddress: true },
    });
    ...
    await getPusherServer().trigger(`driver-${driverId}`, 'job-offer', { bookingId, expiresAt });
