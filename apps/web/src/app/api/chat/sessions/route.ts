...
export async function POST(req: Request) {
  ...
  const userId = (session.user as any).id;
  const userRole = (session.user as any).role;
  const { type, bookingId, participantIds, guestInfo, title } = await req.json();
  ...
  try {
    ...
    if (type === ChatSessionType.customer_admin && userRole !== 'customer' && userRole !== 'admin') {
      return new NextResponse("Forbidden", { status: 403 });
    }

    if (type === ChatSessionType.driver_admin && userRole !== 'driver' && userRole !== 'admin') {
      return new NextResponse("Forbidden", { status: 403 });
    }
      ...
      data: {
        ...
        participants: {
          create: [
            ...
            {
              userId: userId,
              role: userRole === 'admin' ? ChatParticipantRole.admin : 
                    userRole === 'driver' ? ChatParticipantRole.driver : 
