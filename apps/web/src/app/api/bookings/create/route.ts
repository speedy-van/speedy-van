...
export async function POST(request: NextRequest) {
  try {
    ...
    const pickupPropertyType = mapPropertyTypeToPrisma(
      bookingData.pickupProperty?.propertyType || 'DETACHED'
    );
    ...
    const pickupProperty = await prisma.propertyDetails.create({
      data: {
        ...
        accessType: bookingData.pickupProperty?.hasLift
          ? 'WITH_LIFT'
          : 'WITHOUT_LIFT',
      },
    });
    ...
    const dropoffPropertyType = mapPropertyTypeToPrisma(
      bookingData.dropoffProperty?.propertyType || 'DETACHED'
    );
    ...
    const dropoffProperty = await prisma.propertyDetails.create({
      data: {
        ...
        accessType: bookingData.dropoffProperty?.hasLift
          ? 'WITH_LIFT'
          : 'WITHOUT_LIFT',
      },
    });
