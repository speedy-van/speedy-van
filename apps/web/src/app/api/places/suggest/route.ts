...
export async function GET(req: Request) {
  ...
  try {
    ...
    console.log('[PLACES] Postcode detection:', { q, isPostcodeQuery, isPartialPostcode, isPostcodeLike });
    ...
    console.log('[PLACES] Is postcode search:', isPostcodeQuery || isPartialPostcode || isPostcodeLike);
    ...
    function mapboxFeatureToAddress(f: any) {
      ...
      if (isPostcodeQuery || isPartialPostcode || isPostcodeLike) {
        ...
        const buildingName = f?.properties?.name || f?.properties?.building || f?.properties?.amenity;
      ...
      let isPostcodeMatch = false;
         ...
         isPostcodeMatch = postcodeLower.includes(queryLower) || queryLower.includes(postcodeLower);
         
         if (isPostcodeMatch) {
           // For exact postcode matches, give higher priority to real addresses
           if (postcodeLower === queryLower && line1Lower.length > 3 && !line1Lower.match(/^[a-z]{1,2}[0-9]/i)) {
         ...
         } else if (cityLower.includes(queryLower) || queryLower.includes(cityLower)) {
    ...
    let suggestions = ((data?.features || []) as any[]).slice(0, limit).map(mapboxFeatureToAddress);
    ...
    suggestions.sort((a, b) => {
     ...
     if (isPostcodeQuery || isPartialPostcode || isPostcodeLike) {
       suggestions = suggestions.filter(s => {
         ...
         if (s.address?.line1 === q.toUpperCase() || 
         ...
         if (isPostcodeQuery && q.length >= 7) {
           ...
           const isFromSameArea = s.address?.city?.toLowerCase().includes('glasgow') && q.toUpperCase().startsWith('G21');
    ...
    if ((isPostcodeQuery || isPartialPostcode || isPostcodeLike) && suggestions.length < 3) {
      ...
      try {
        ...
        const postcodeResponse = await fetch(postcodeUrl.toString(), { cache: "no-store" });
        if (postcodeResponse.ok) {
          const postcodeData = await postcodeResponse.json();
          const postcodeSuggestions = ((postcodeData?.features || []) as any[]).map(mapboxFeatureToAddress);
        ...
        if (suggestions.length < 5) {
          ...
          const buildingResponse = await fetch(buildingUrl.toString(), { cache: "no-store" });
          if (buildingResponse.ok) {
            const buildingData = await buildingResponse.json();
            const buildingSuggestions = ((buildingData?.features || []) as any[]).map(mapboxFeatureToAddress);
            
            // Filter to only include actual building addresses (not just postcode areas)
            const actualBuildings = buildingSuggestions.filter(s => 
              s.address?.line1 && 
            ...
            suggestions.sort((a, b) => {
              ...
              const aIsBuilding = a.address?.line1 && a.address.line1.length > 3 && !a.address.line1.match(/^[A-Z]{1,2}[0-9]/i);
              const bIsBuilding = b.address?.line1 && b.address.line1.length > 3 && !b.address.line1.match(/^[A-Z]{1,2}[0-9]/i);
        ...
        if (isPostcodeQuery && q.length >= 7) {
          console.log('[PLACES] Getting specific building addresses for full postcode:', q);
          ...
          for (const searchQuery of searchQueries) {
            try {
              ...
              const specificBuildingResponse = await fetch(specificBuildingUrl.toString(), { cache: "no-store" });
              if (specificBuildingResponse.ok) {
                const specificBuildingData = await specificBuildingResponse.json();
                const specificBuildingSuggestions = ((specificBuildingData?.features || []) as any[]).map(mapboxFeatureToAddress);
                
                // Filter for actual buildings (not just postcode areas)
                const actualBuildings = specificBuildingSuggestions.filter(s => 
                  s.address?.line1 && 
                  s.address?.line1.length > 3 && 
                  !s.address.line1.match(/^[A-Z]{1,2}[0-9]/i) && // Not just a postcode
                  s.address?.line1 !== q.toUpperCase() && // Not the postcode itself
                  s.address?.line1 !== `${q.toUpperCase()}, Glasgow, Glasgow City, Scotland, United Kingdom` // Not the full area description
                );
                
                allBuildingSuggestions.push(...actualBuildings);
              }
            } catch (error) {
              console.log('[PLACES] Building search failed for:', searchQuery, error);
            }
          }
          
          // Remove duplicates and add to suggestions
          const uniqueBuildings = allBuildingSuggestions.filter((s, index, self) => 
            index === self.findIndex(t => t.id === s.id)
          );
          
          if (uniqueBuildings.length > 0) {
            // Add buildings to the beginning and remove postcode area results
            suggestions = [...uniqueBuildings, ...suggestions.filter(s => {
              // Remove postcode area results when we have actual buildings
              const isPostcodeArea = s.address?.line1 === q.toUpperCase() || 
                                   s.address?.line1?.includes(`${q.toUpperCase()}, Glasgow, Glasgow City, Scotland, United Kingdom`) ||
                                   s.address?.line1?.includes(`${q.toUpperCase()}, London, Greater London, England, United Kingdom`) ||
                                   s.address?.line1?.includes(`${q.toUpperCase()}, Manchester, Greater Manchester, England, United Kingdom`) ||
                                   s.address?.line1?.includes(`${q.toUpperCase()}, Birmingham, West Midlands, England, United Kingdom`);
    ...
    // Debug logging
    console.log('[PLACES] Final suggestions:', JSON.stringify(suggestions, null, 2));
     ...
     if ((isPostcodeQuery || isPartialPostcode || isPostcodeLike) && suggestions.length < 3) {
       ...
       const hasGoodApiResults = suggestions.some(s => 
      ...
      if (isPostcodeQuery && q.length >= 7) {
        ...
        const exactApiMatches = suggestions.filter(s => 
          s.address?.postcode === q.toUpperCase() && 
          s.address?.line1 && 
          s.address.line1.length > 3 &&
          !s.address.line1.match(/^[A-Z]{1,2}[0-9]/i) // Not just a postcode
        ...
        if (exactApiMatches.length > 0) {
          console.log('[PLACES] Found exact API matches:', exactApiMatches.length);
          ...
          const exactMockMatches = generateMockPostcodeSuggestions(q).filter(item => 
            item.address.postcode === q.toUpperCase()
          );
          
          if (exactMockMatches.length > 0) {
            console.log('[PLACES] No API matches, using mock data:', exactMockMatches.length);
...
function generateMockPostcodeSuggestions(query: string): any[] {
  ...
  const mockData = [
    ...
    },
    {
      ...
      address: { line1: 'Buckingham Palace', city: 'London', postcode: 'SW1A 1AA' },
    ...
    // M1 area (Manchester) - Actual buildings
    {
      ...
      address: { line1: 'Manchester Town Hall', city: 'Manchester', postcode: 'M1 1AA' },
    ...
    },
    {
      ...
      address: { line1: 'Old Trafford', city: 'Manchester', postcode: 'M1 2BB' },
    ...
    },
    {
      ...
      address: { line1: 'Manchester Central Library', city: 'Manchester', postcode: 'M1 1AB' },
      coords: { lat: 53.4780, lng: -2.2440 },
      priority: 3,
      isPostcodeMatch: true
    },
    {
      ...
      address: { line1: 'Manchester Art Gallery', city: 'Manchester', postcode: 'M1 1AC' },
      coords: { lat: 53.4790, lng: -2.2430 },
      priority: 3,
      isPostcodeMatch: true
    },
    // B1 area (Birmingham) - Actual buildings
    {
      ...
      address: { line1: 'Birmingham Town Hall', city: 'Birmingham', postcode: 'B1 1AA' },
      coords: { lat: 52.4800, lng: -1.9036 },
      priority: 3,
      isPostcodeMatch: true
    },
    {
    ...
    },
    {
      ...
      address: { line1: 'Birmingham Museum & Art Gallery', city: 'Birmingham', postcode: 'B1 1AD' },
      coords: { lat: 52.4805, lng: -1.9040 },
      priority: 3,
      isPostcodeMatch: true
    },
         // G21 area (Glasgow) - Actual buildings
     {
       ...
       address: { line1: 'Glasgow Royal Infirmary', city: 'Glasgow', postcode: 'G21 2QB' },
     ...
     },
     {
       ...
       address: { line1: 'Glasgow Cathedral', city: 'Glasgow', postcode: 'G21 2QB' },
       coords: { lat: 55.8710, lng: -4.2280 },
       priority: 3,
       isPostcodeMatch: true
     },
     {
       ...
       address: { line1: 'Glasgow Necropolis', city: 'Glasgow', postcode: 'G21 2QB' },
       coords: { lat: 55.8720, lng: -4.2290 },
       priority: 3,
       isPostcodeMatch: true
     },
     {
     ...
     },
     {
       ...
       address: { line1: '84 Castle Street', city: 'Glasgow', postcode: 'G21 2QB' },
     ...
     },
     {
       ...
       address: { line1: '86 Castle Street', city: 'Glasgow', postcode: 'G21 2QB' },
     ...
     },
     {
       ...
       address: { line1: '88 Castle Street', city: 'Glasgow', postcode: 'G21 2QB' },
     ...
     },
     {
       ...
       address: { line1: '90 Castle Street', city: 'Glasgow', postcode: 'G21 2QB' },
   ...
   const exactMatches = mockData.filter(item => 
     item.address.postcode === queryUpper // Exact postcode match
   );
   
   const startsWithMatches = mockData.filter(item => 
     item.address.postcode.startsWith(queryUpper) && 
     item.address.postcode !== queryUpper // Partial postcode match, but not exact
   );
   
   const cityMatches = mockData.filter(item => 
