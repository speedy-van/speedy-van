...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!upstream.ok) {
      console.warn("[PLACES] Reverse geocoding upstream error", upstream.status);
      return NextResponse.json({
        label: `${latNum.toFixed(6)}, ${lngNum.toFixed(6)}`,
        address: {
          line1: `${latNum.toFixed(6)}, ${lngNum.toFixed(6)}`,
    ...
    function mapboxFeatureToAddress(f: any) {
      ...
      return {
        ...
        address: {
          line1: [number, street].filter(Boolean).join(' ') || f?.place_name?.split(',')[0] || `${latNum.toFixed(6)}, ${lngNum.toFixed(6)}`,
          city,
          postcode,
        },
        coords: { lat: latNum, lng: lngNum },
    ...
    const result = feature ? mapboxFeatureToAddress(feature) : {
      label: `${latNum.toFixed(6)}, ${lngNum.toFixed(6)}`,
      address: {
        line1: `${latNum.toFixed(6)}, ${lngNum.toFixed(6)}`,
        city: "",
