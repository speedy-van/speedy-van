...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cancelledAssignments = allAssignments.filter(
      a => a.status === 'cancelled'
    );
    const offeredAssignments = allAssignments.filter(
      a => a.status === 'invited'
    );
    const claimedAssignments = allAssignments.filter(
      a => a.status === 'claimed'
    );
    ...
    const monthlyAssignments = allAssignments.filter(
      a => a.createdAt >= thirtyDaysAgo
    );
    const monthlyCompleted = monthlyAssignments.filter(
      a => a.status === 'completed'
    );
