...
export async function GET(request: Request) {
  ...
  const endDate = searchParams.get('end') || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(); // 30 days from now
  ...
  try {
    ...
    driver.Assignment.forEach((assignment, index) => {
      ...
      const start = format(new Date(assignment.Booking.scheduledAt), "yyyyMMdd'T'HHmmss");
      const end = format(new Date(assignment.Booking.scheduledAt.getTime() + 2 * 60 * 60 * 1000), "yyyyMMdd'T'HHmmss"); // 2 hours duration
      const created = format(new Date(assignment.createdAt), "yyyyMMdd'T'HHmmss");
