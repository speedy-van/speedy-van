...
export async function POST(request: NextRequest) {
  try {
    ...
    const isValidPassword = await bcrypt.compare(password, driver.user.password);
    ...
    if (activeAssignments.length > 0) {
      return NextResponse.json({ 
        error: "Cannot delete account with active jobs. Please complete or cancel all active jobs first." 
      }, { status: 400 });
    }
    ...
    if (pendingPayouts.length > 0) {
      return NextResponse.json({ 
        error: "Cannot delete account with pending payouts. Please wait for payouts to be processed." 
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Account deletion request submitted successfully. Your account will be permanently deleted in 30 days unless you cancel the request.",
      scheduledFor: deletionRequest.scheduledFor,
      deletionRequestId: deletionRequest.id
    });
  } catch (error) {
