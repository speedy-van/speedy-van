...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      linkingResult = await linkExistingBookingsToCustomer(
        user.id,
        email.toLowerCase(),
        '' // No phone number available from this context
      );
    } catch (linkingError) {
      console.warn('Failed to link existing bookings during registration:', linkingError);
    ...
    await prisma.auditLog.create({
      data: {
        ...
        details: {
          ...
          linkedBookings: linkingResult?.linkedBookings || 0,
    ...
    return NextResponse.json({
      ...
      linkingResult: linkingResult ? {
        linkedBookings: linkingResult.linkedBookings,
