...
import { FiPlus, FiEdit, FiEye, FiTrash2, FiSave, FiX, FiMapPin } from 'react-icons/fi';
...
export default function ServiceAreasPage() {
  ...
  const addPostcode = () => {
    if (newPostcode && !formData.postcodes.includes(newPostcode.toUpperCase())) {
      setFormData({
        ...formData,
    ...
    <Box>
      <HStack justify="space-between" mb={6}>
        <VStack align="start" spacing={1}>
          ...
          <Text color="gray.600">Manage geographic service boundaries and capacity</Text>
        </VStack>
      ...
      <Alert status="info" mb={6}>
        <AlertIcon />
        Service areas define where you operate and can include capacity limits and blackout dates.
      </Alert>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack spacing={4}>
                ...
                <GridItem>
                  <FormControl>
                    ...
                    <NumberInput
                      ...
                      onChange={(value) => setFormData({ ...formData, surgeMultiplier: parseFloat(value) / 100 })}
                      min={100}
                      max={300}
                      precision={0}
                    >
              ...
              <FormControl>
                ...
                <HStack spacing={2} mt={2} flexWrap="wrap">
                  {formData.postcodes.map((postcode) => (
                    <Tag key={postcode} size="md" colorScheme="blue" borderRadius="full">
                      <TagLabel>{postcode}</TagLabel>
                      <TagCloseButton onClick={() => removePostcode(postcode)} />
                    </Tag>
                  ))}
                </HStack>
              </FormControl>
              ...
              <FormControl>
                ...
                <HStack spacing={2} mt={2} flexWrap="wrap">
                  {formData.blackoutDates.map((date) => (
                    <Tag key={date} size="md" colorScheme="red" borderRadius="full">
                      <TagLabel>{date}</TagLabel>
                      <TagCloseButton onClick={() => removeBlackoutDate(date)} />
                    </Tag>
                  ))}
                </HStack>
              </FormControl>
              ...
              <Alert status="info">
                <AlertIcon />
                Geographic boundaries can be set using the map interface in the full editor.
              </Alert>
            </VStack>
          </ModalBody>
          <ModalFooter>
