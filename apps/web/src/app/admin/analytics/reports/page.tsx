...
const AVAILABLE_METRICS = [
  ...
  { id: "aov", name: "Average Order Value", category: "financial", icon: FiTrendingUp },
  { id: "completion_rate", name: "Completion Rate", category: "operational", icon: FiBarChart2 },
  { id: "avg_rating", name: "Average Rating", category: "quality", icon: FiTrendingUp },
  { id: "driver_earnings", name: "Driver Earnings", category: "financial", icon: FiDollarSign },
  { id: "retention_rate", name: "Retention Rate", category: "customer", icon: FiUsers },
  { id: "ltv", name: "Lifetime Value", category: "customer", icon: FiDollarSign },
  { id: "repeat_orders", name: "Repeat Orders", category: "customer", icon: FiPackage },
  { id: "response_time", name: "Response Time", category: "operational", icon: FiClock },
  { id: "on_time_rate", name: "On-Time Rate", category: "operational", icon: FiClock },
  { id: "cancellation_rate", name: "Cancellation Rate", category: "operational", icon: FiBarChart2 }
];

const AVAILABLE_DIMENSIONS = [
  { id: "date", name: "Date", category: "time", icon: FiCalendar },
  { id: "service_type", name: "Service Type", category: "operational", icon: FiPackage },
  ...
  { id: "customer_segment", name: "Customer Segment", category: "customer", icon: FiUsers },
  { id: "area", name: "Service Area", category: "operational", icon: FiBarChart2 },
  { id: "vehicle_type", name: "Vehicle Type", category: "operational", icon: FiPackage },
  { id: "payment_method", name: "Payment Method", category: "financial", icon: FiDollarSign },
  { id: "booking_source", name: "Booking Source", category: "operational", icon: FiBarChart2 }
];
...
const REPORT_TEMPLATES: ReportTemplate[] = [
  ...
  },
  {
    ...
    metrics: ["completion_rate", "avg_rating", "driver_earnings", "on_time_rate"],
  ...
  },
  {
    ...
    category: "operational",
    metrics: ["completion_rate", "response_time", "on_time_rate", "cancellation_rate"],
...
export default function AnalyticsReportsPage() {
  ...
  const exportReport = async (reportId: string, format: 'csv' | 'xlsx') => {
    try {
      const response = await fetch(`/api/admin/analytics/reports/${reportId}/export?format=${format}`);
      if (response.ok) {
    ...
    <Box p={6} display="flex" flexDirection="column" gap={6}>
      ...
      <HStack justify="space-between">
        <VStack align="start" spacing={1}>
          ...
          <Text color="gray.600">Build, schedule, and manage custom reports</Text>
        </VStack>
        <HStack spacing={3}>
        ...
        <TabPanels>
          ...
          <TabPanel>
            <Grid templateColumns={{ base: "1fr", lg: "2fr 1fr" }} gap={6}>
              ...
              <Card bg={bgColor} borderColor={borderColor}>
                <CardBody>
                  <Heading size="md" mb={4}>Saved Reports</Heading>
                  <VStack spacing={4} align="stretch">
                    {reports.map((report) => (
                      <Card key={report.id} variant="outline" bg={bgColor} borderColor={borderColor}>
                        <CardBody>
                          <HStack justify="space-between" mb={3}>
                            ...
                            <Badge colorScheme={getScheduleColor(report.schedule)}>
                              {report.schedule}
                            </Badge>
                          </HStack>
                          ...
                          <HStack spacing={4} mb={3}>
                            <VStack align="start" spacing={1}>
                              <Text fontSize="sm" fontWeight="bold">Metrics</Text>
                              <HStack spacing={1}>
                                {report.metrics.map((metric) => (
                                  <Badge key={metric} size="sm" variant="outline">
                                    {metric}
                                  </Badge>
                                ))}
                              </HStack>
                            ...
                            <VStack align="start" spacing={1}>
                              <Text fontSize="sm" fontWeight="bold">Dimensions</Text>
                              <HStack spacing={1}>
                                {report.dimensions.map((dimension) => (
                                  <Badge key={dimension} size="sm" variant="outline">
                                    {dimension}
                                  </Badge>
                                ))}
                              </HStack>
                          ...
                          <HStack justify="space-between">
                            ...
                            <HStack spacing={2}>
                              ...
                              <Button size="xs" variant="outline" onClick={() => exportReport(report.id, 'csv')}>
                                <FiDownload className="mr-1" />
                                CSV
                              </Button>
                              <Button size="xs" variant="outline" onClick={() => exportReport(report.id, 'xlsx')}>
              ...
              <VStack spacing={6} align="stretch">
                <Card bg={bgColor} borderColor={borderColor}>
                  <CardBody>
                    <Heading size="md" mb={4}>Report Statistics</Heading>
                    <VStack spacing={4} align="stretch">
                      <HStack justify="space-between">
                      ...
                      <HStack justify="space-between">
                        ...
                        <Text fontWeight="bold">{reports.filter(r => r.status === "active").length}</Text>
                      </HStack>
                      <HStack justify="space-between">
                ...
                <Card bg={bgColor} borderColor={borderColor}>
                  <CardBody>
                    <Heading size="md" mb={4}>Popular Reports</Heading>
                    <VStack spacing={3} align="stretch">
                      <HStack justify="space-between">
                        <Text fontSize="sm">Daily Revenue</Text>
                        <Text fontSize="sm" color="gray.600">24 runs</Text>
                      </HStack>
                      <HStack justify="space-between">
                        <Text fontSize="sm">Driver Performance</Text>
                        <Text fontSize="sm" color="gray.600">12 runs</Text>
                      </HStack>
                      <HStack justify="space-between">
                        <Text fontSize="sm">Customer Retention</Text>
                        <Text fontSize="sm" color="gray.600">8 runs</Text>
                      </HStack>
                    </VStack>
                  </CardBody>
                </Card>
          ...
          <TabPanel>
            <Grid templateColumns={{ base: "1fr", lg: "1fr 1fr" }} gap={6}>
              ...
              <Card bg={bgColor} borderColor={borderColor}>
                <CardBody>
                  <Heading size="md" mb={4}>Report Builder</Heading>
                  <VStack spacing={4} align="stretch">
                    <FormControl>
                    ...
                    <FormControl>
                      ...
                      <CheckboxGroup value={selectedMetrics} onChange={(values) => setSelectedMetrics(values as string[])}>
                        <SimpleGrid columns={2} spacing={2}>
                          {AVAILABLE_METRICS.map((metric) => (
                            <Checkbox key={metric.id} value={metric.id}>
                              <HStack>
                                <Text fontSize="sm">{metric.name}</Text>
                                <Badge size="sm" variant="outline">{metric.category}</Badge>
                              </HStack>
                            </Checkbox>
                          ))}
                        </SimpleGrid>
                    ...
                    <FormControl>
                      ...
                      <CheckboxGroup value={selectedDimensions} onChange={(values) => setSelectedDimensions(values as string[])}>
                        <SimpleGrid columns={2} spacing={2}>
                          {AVAILABLE_DIMENSIONS.map((dimension) => (
                            <Checkbox key={dimension.id} value={dimension.id}>
                              <HStack>
                                <Text fontSize="sm">{dimension.name}</Text>
                                <Badge size="sm" variant="outline">{dimension.category}</Badge>
                              </HStack>
                            </Checkbox>
                          ))}
                        </SimpleGrid>
                    ...
                    <HStack spacing={3}>
                      <Button onClick={generatePreview} disabled={loading || selectedMetrics.length === 0}>
                        {loading ? 'Generating...' : 'Preview Report'}
                      </Button>
                      <Button onClick={saveReport} variant="outline" disabled={!reportName || selectedMetrics.length === 0}>
                        Save Report
                      </Button>
                    </HStack>
                  </VStack>
              ...
              <Card bg={bgColor} borderColor={borderColor}>
                <CardBody>
                  <Heading size="md" mb={4}>Report Preview</Heading>
                  {previewData ? (
                    <VStack spacing={4}>
                      <Text fontSize="sm" color="gray.600">
                        Preview data for {selectedMetrics.length} metrics × {selectedDimensions.length} dimensions
                      </Text>
                      <Box bg="gray.50" p={4} rounded="lg" maxH="400px" overflow="auto">
                    ...
                    <VStack spacing={4} justify="center" h="400px">
                      ...
                      <Text color="gray.500">Report preview will appear here</Text>
            ...
            <Card bg={bgColor} borderColor={borderColor}>
              <CardBody>
                ...
                <Table variant="simple">
                  ...
                  <Tbody>
                    {reports.filter(r => r.schedule !== 'none').map((report) => (
                      <Tr key={report.id}>
                        <Td>
                          <VStack align="start" spacing={1}>
                            <Text fontWeight="bold">{report.name}</Text>
                        ...
                        <Td>
                          <Text fontSize="sm">{new Date(report.lastRun).toLocaleDateString()}</Text>
                        </Td>
                        <Td>
                          <Text fontSize="sm">
                            {report.nextRun ? new Date(report.nextRun).toLocaleDateString() : 'N/A'}
                          </Text>
                        </Td>
                        <Td>
                        ...
                        <Td>
                          <HStack spacing={2}>
                            ...
                            >
                              {report.status === 'active' ? <FiPause /> : <FiPlay />}
                            </Button>
                            <Button size="xs" variant="outline">
                              <FiEdit />
                            </Button>
            ...
            <Card bg={bgColor} borderColor={borderColor}>
              <CardBody>
                ...
                <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                  {REPORT_TEMPLATES.map((template) => (
                    <Card key={template.id} variant="outline" bg={bgColor} borderColor={borderColor}>
                      <CardBody>
                        <HStack justify="space-between" mb={3}>
                          <VStack align="start" spacing={1}>
                          ...
                          <VStack align="start" spacing={1}>
                            ...
                            <HStack spacing={1} flexWrap="wrap">
                              {template.metrics.map((metric) => (
                                <Badge key={metric} size="sm" colorScheme="blue">
                                  {metric}
                                </Badge>
                              ))}
                            </HStack>
