...
export default function ErrorLogsPage() {
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case 'up': return <FiTrendingUp color="red" />;
      case 'down': return <FiTrendingDown color="green" />;
      default: return <FiTrendingUp color="gray" />;
    }
  };
      ...
      <Box>
        <HStack justify="space-between" mb={6}>
          <VStack align="start" spacing={1}>
            ...
            <Text color="gray.600">Critical system errors and exceptions requiring immediate attention</Text>
          </VStack>
        </HStack>
  ...
  return (
    <Box>
      <HStack justify="space-between" mb={6}>
        ...
        <HStack spacing={3}>
          <Button leftIcon={<FiRefreshCw />} variant="outline" size="sm" onClick={loadErrorLogs} isLoading={loading}>
            Refresh
          </Button>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber suppressHydrationWarning>{(stats.totalErrors || 0).toLocaleString()}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber color="orange.500" suppressHydrationWarning>{stats.errorsThisWeek}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber color="red.600" suppressHydrationWarning>{stats.criticalErrors}</StatNumber>
              <StatHelpText>
                <FiAlertTriangle style={{ display: 'inline', marginRight: '4px' }} />
                High priority
              </StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber suppressHydrationWarning>{stats.avgResolutionTime}</StatNumber>
              <StatHelpText>
                <FiClock style={{ display: 'inline', marginRight: '4px' }} />
                Time to fix
              </StatHelpText>
            ...
            <Stat>
              ...
              <StatNumber>
                <HStack spacing={2}>
                  ...
                  <Text fontSize="sm" color={stats.errorTrend === 'up' ? 'red.500' : stats.errorTrend === 'down' ? 'green.500' : 'gray.500'}>
                    {stats.errorTrend === 'up' ? 'Increasing' : stats.errorTrend === 'down' ? 'Decreasing' : 'Stable'}
                  </Text>
                </HStack>
              </StatNumber>
              <StatHelpText>
              ...
              <Tbody>
                {logs.map((log) => (
                  <Tr key={log.id}>
                    ...
                    <Td>
                      <Badge colorScheme={getServiceColor(log.service || '')} size="sm">
                        {log.service}
                      </Badge>
                    </Td>
                    <Td>
                      <VStack align="start" spacing={1}>
                        <Text fontSize="sm" color="red.500" fontWeight="medium">{log.error}</Text>
                    ...
                    <Td>
                      <VStack align="start" spacing={1}>
                        ...
                        <Text fontSize="xs" color="gray.600">{log.ip || 'N/A'}</Text>
                      </VStack>
                    </Td>
                    <Td>
                      <HStack spacing={2}>
          ...
          <ModalBody pb={6}>
            {selectedLog && (
              <VStack spacing={4} align="stretch">
                <SimpleGrid columns={2} spacing={4}>
                  ...
                  {selectedLog.userAgent && (
                    <Box>
                      ...
                      <Text fontSize="sm" color="gray.600">{selectedLog.userAgent}</Text>
                    </Box>
                  )}
                </SimpleGrid>
                ...
                {selectedLog.before && (
                  <Box>
                    <Text fontWeight="bold" mb={2}>Before State</Text>
                    <Card variant="outline">
                      <CardBody>
                        <pre style={{ fontSize: '12px', overflow: 'auto' }}>
                ...
                {selectedLog.after && (
                  <Box>
                    <Text fontWeight="bold" mb={2}>After State</Text>
                    <Card variant="outline">
                      <CardBody>
                        <pre style={{ fontSize: '12px', overflow: 'auto' }}>
          ...
          <HStack justify="space-between">
            <VStack align="start" spacing={1}>
              ...
              <Text fontSize="sm" color="gray.600">
                Error logs are retained for 30 days. Critical errors trigger immediate alerts.
              </Text>
            </VStack>
            <HStack spacing={3}>
