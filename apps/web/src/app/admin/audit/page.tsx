...
export default function AuditPage() {
  ...
  if (!mounted || (loading && logs.length === 0)) {
    return (
      <Box textAlign="center" py={20}>
        <Spinner size="xl" />
        <Text mt={4}>{!mounted ? 'Initializing...' : 'Loading audit logs...'}</Text>
      </Box>
    );
  }
  ...
  const safePagination = pagination || { page: 1, limit: 50, total: 0, pages: 0 };
  
  // Ensure stats is properly initialized
  const safeStats = stats || { totalLogs: 0, securityEvents: 0, financialActions: 0, userActions: 0 };
  
  // Ensure filters is properly initialized
  const safeFilters = filters || { search: '', action: '', entity: '', actorRole: '', from: '', to: '' };
    ...
    <Box>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{(safePagination.total || safeLogs.length || 0).toLocaleString()}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber color="purple.500">{safeStats.securityEvents}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber color="green.500">{safeStats.financialActions}</StatNumber>
              <StatHelpText>
                <FiActivity style={{ display: 'inline', marginRight: '4px' }} />
                Last 24 hours
              </StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber color="blue.500">{safeStats.userActions}</StatNumber>
              <StatHelpText>
      ...
      {safeLogs.length > 0 ? (
        <Card>
            ...
            <Tbody>
              {loading && safeLogs.length === 0 ? (
                ...
                safeLogs.map((log) => (
                  ...
                  <Td>
                    <VStack align="start" spacing={1}>
                      <Text fontSize="sm">{log.timestamp ? new Date(log.timestamp).toLocaleDateString() : 'N/A'}</Text>
                  ...
                  <Td>
                    <HStack spacing={2}>
                      ...
                      <Text fontSize="sm">
                        {log.entity || 'Unknown'} {log.entityId ? `#${log.entityId}` : ''}
                      </Text>
                    </HStack>
                  </Td>
                  <Td>
          ...
          {/* Load More */}
          {safeLogs.length > 0 && safePagination.page < safePagination.pages && (
        ...
        <Card>
          <CardBody>
            <Text textAlign="center" color="gray.500" py={8}>
              No audit logs available. Try adjusting your filters or refresh the page.
            </Text>
          </CardBody>
        </Card>
      )}
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            {selectedLog ? (
              <VStack spacing={4} align="stretch">
                <SimpleGrid columns={2} spacing={4}>
                  ...
                  <Box>
                    ...
                    <Text>{selectedLog.entity || 'Unknown'} {selectedLog.entityId ? `#${selectedLog.entityId}` : ''}</Text>
                  </Box>
                  <Box>
              ...
              <Text textAlign="center" color="gray.500">No log details available</Text>
            )}
          </ModalBody>
        </ModalContent>
      </Modal>
