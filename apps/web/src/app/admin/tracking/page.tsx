...
export default function TrackingHub() {
  ...
  const loadTrackingData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setDriverLocations(data.data.driverLocations || []);
      }

      // Load active bookings with tracking data
      const bookingsResponse = await fetch('/api/admin/orders?status=active&includeTracking=true');
  ...
  const getStepLabel = (step: string) => {
    switch (step) {
      ...
      case 'en_route_to_dropoff': return 'En Route to Dropoff';
      case 'arrived_at_dropoff': return 'Arrived at Dropoff';
  ...
  const activeDrivers = driverLocations.filter(driver => driver.status === 'online');
      ...
      <VStack spacing={8} align="stretch">
          ...
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatNumber>
                  {totalBookings > 0 ? Math.round((bookings.filter(b => b.currentLocation).length / totalBookings) * 100) : 0}%
                </StatNumber>
              ...
              <CardBody>
                <VStack spacing={4} align="stretch">
                  <Flex justify="space-between" align="center">
                    ...
                    <Badge colorScheme="green">Real-time tracking enabled</Badge>
                  </Flex>
                    ...
                    <VStack spacing={3} align="stretch" maxH="600px" overflowY="auto">
                      {filteredBookings.map((booking) => (
                        <Card
                          ...
                          bg={selectedBooking?.id === booking.id ? 'blue.50' : undefined}
                          _hover={{ bg: 'gray.50' }}
                        >
                          <CardBody p={4}>
                            <VStack spacing={2} align="stretch">
                              ...
                              {booking.routeProgress > 0 && (
                                <Box>
                                  <Text fontSize="xs" mb={1}>Progress</Text>
                                  <Progress value={booking.routeProgress} size="sm" colorScheme="blue" />
                                </Box>
                              )}
        ...
        {selectedBooking && (
          <Card>
            <CardBody>
              <VStack spacing={4} align="stretch">
                <Heading size="md">Booking Details: {selectedBooking.reference}</Heading>
                ...
                <Grid templateColumns="repeat(auto-fit, minmax(250px, 1fr))" gap={6}>
                  ...
                  <VStack align="start" spacing={2}>
                    ...
                    <Progress value={selectedBooking.routeProgress} w="100%" colorScheme="blue" />
                  ...
                  {selectedBooking.eta && (
                    <VStack align="start" spacing={2}>
                      ...
                      <Text fontSize="sm" color="gray.600">
                        {new Date(selectedBooking.eta.estimatedArrival).toLocaleString()}
                      </Text>
                    </VStack>
                  )}
                </Grid>
                ...
                {selectedBooking.lastEvent && (
                  <Box>
                    <Text fontWeight="bold" mb={2}>Last Event</Text>
                    <Card size="sm">
                      <CardBody>
                        <VStack align="start" spacing={1}>
                          <Badge colorScheme="blue">{getStepLabel(selectedBooking.lastEvent.step)}</Badge>
                          <Text fontSize="sm">
                            {new Date(selectedBooking.lastEvent.createdAt).toLocaleString()}
                          </Text>
                ...
                {selectedBooking.trackingPings.length > 0 && (
                  <Box>
                    <Text fontWeight="bold" mb={2}>Recent Location Updates</Text>
