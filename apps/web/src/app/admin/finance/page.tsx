...
import { Box, Heading, HStack, VStack, Text, Badge, Card, CardBody, Button, SimpleGrid, Stat, StatLabel, StatNumber, StatHelpText, Grid, GridItem, Tabs, TabList, TabPanels, Tab, TabPanel, Table, Thead, Tbody, Tr, Th, Td, Divider, Select, Input, HStack as ChakraHStack, useToast, Alert, AlertIcon, AlertTitle, AlertDescription } from "@chakra-ui/react";
...
export default function FinancePage() {
    ...
    return (
      <Box p={6}>
        <Alert status="warning" mb={4}>
          ...
          <AlertDescription>No finance data is currently available.</AlertDescription>
        </Alert>
        <Button onClick={fetchFinanceData} colorScheme="blue">
          Refresh Data
        </Button>
  ...
  if (!data.recentActivity || !Array.isArray(data.recentActivity.invoices) || !Array.isArray(data.recentActivity.refunds)) {
    return (
      <Box p={6}>
        <Alert status="warning" mb={4}>
          <AlertIcon />
          <AlertTitle>Data Structure Error</AlertTitle>
          <AlertDescription>The finance data structure is incomplete. Please refresh the page.</AlertDescription>
        </Alert>
        <Button onClick={fetchFinanceData} colorScheme="blue">
          Refresh Data
        </Button>
    ...
    return (
      <Box p={6}>
        <Alert status="warning" mb={4}>
          <AlertIcon />
          <AlertTitle>Data Structure Error</AlertTitle>
          <AlertDescription>Driver data is missing. Please refresh the page.</AlertDescription>
        </Alert>
        <Button onClick={fetchFinanceData} colorScheme="blue">
          Refresh Data
        </Button>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{formatCurrency(data.summary.totalRevenue)}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{formatCurrency(data.summary.pendingRevenue)}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{formatCurrency(data.summary.totalRefunds)}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{formatCurrency(data.summary.totalPayouts)}</StatNumber>
              <StatHelpText>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{formatCurrency(data.summary.totalRevenue - data.earnings.netAmount)}</StatNumber>
              <StatHelpText>
                <FiDollarSign style={{ display: 'inline', marginRight: '4px' }} />
                Revenue - Payouts
              </StatHelpText>
              ...
              <Card>
                <CardBody>
                  ...
                  <Table variant="simple" size="sm">
                    ...
                    <Tbody>
                      {data.recentActivity.invoices.filter(invoice => invoice).map((invoice) => (
                        <Tr key={invoice.id || Math.random()}>
                          <Td>
                            <Text fontWeight="bold" fontSize="sm">{invoice.bookingRef || 'N/A'}</Text>
                          </Td>
                          <Td>
                            <Text fontSize="sm">{invoice.customer && invoice.customer.name ? invoice.customer.name : 'Unknown'}</Text>
                          </Td>
                          <Td>
                            <Text fontWeight="bold" fontSize="sm">{formatCurrency(invoice.amount || 0)}</Text>
                          </Td>
                          <Td>
                            <Badge colorScheme={getStatusColor(invoice.status || 'unknown')} size="sm">
              ...
              <Card>
                <CardBody>
                  ...
                  <Table variant="simple" size="sm">
                    ...
                    <Tbody>
                      {data.recentActivity.refunds.filter(refund => refund).map((refund) => (
                        <Tr key={refund.id || Math.random()}>
                          <Td>
                            <Text fontWeight="bold" fontSize="sm">{refund.bookingRef || 'N/A'}</Text>
                          </Td>
                          <Td>
                            <Text fontSize="sm">{refund.customer && refund.customer.name ? refund.customer.name : 'Unknown'}</Text>
                          </Td>
                          <Td>
                            <Text fontWeight="bold" fontSize="sm">{formatCurrency(refund.amount || 0)}</Text>
                          </Td>
                          <Td>
                            <Badge colorScheme={getStatusColor(refund.status || 'unknown')} size="sm">
              ...
              <Card>
                <CardBody>
                  <HStack justify="space-between" mb={4}>
                    ...
                    <Button size="sm" variant="outline" onClick={() => window.location.href = '/admin/drivers'}>
                      View All
                    </Button>
                  </HStack>
                  <Table variant="simple" size="sm">
                    ...
                    <Tbody>
                      {data.topDrivers.filter(driver => driver).map((driver) => (
                        <Tr key={driver.driverId || Math.random()}>
                          <Td>
                            <Text fontSize="sm">{driver.driverName || 'Unknown Driver'}</Text>
                          </Td>
                          <Td>
                            <Text fontWeight="bold" fontSize="sm">{formatCurrency(driver.totalEarnings || 0)}</Text>
                          </Td>
                          <Td>
                ...
                <CardBody>
                  ...
                  <VStack spacing={4} align="stretch">
                    <HStack justify="space-between">
                      ...
                      <Text fontWeight="bold">{formatCurrency(data.summary.totalRevenue)}</Text>
                    </HStack>
                    <HStack justify="space-between">
                      <Text>Refunds</Text>
                      <Text color="red.500">-{formatCurrency(data.summary.totalRefunds)}</Text>
                    </HStack>
                    <HStack justify="space-between">
                      <Text>Driver Payouts</Text>
                      <Text color="blue.500">-{formatCurrency(data.earnings.netAmount)}</Text>
                    </HStack>
                    <HStack justify="space-between">
                      <Text>Platform Fees</Text>
                      <Text color="gray.500">-{formatCurrency(data.summary.totalRevenue - data.earnings.netAmount)}</Text>
                    </HStack>
                    <Divider />
                    <HStack justify="space-between">
                      <Text fontWeight="bold">Net Profit</Text>
                      <Text fontWeight="bold" color="green.500">
                        {formatCurrency(data.summary.totalRevenue - data.summary.totalRefunds - data.earnings.netAmount)}
                      </Text>
                    </HStack>
                  </VStack>
                </CardBody>
