  ...
  return (
    <Box>
      <HStack justify="space-between" mb={6}>
        ...
        <Button leftIcon={<FiPlus />} colorScheme="blue" size="sm" onClick={handleNewRefund}>
          Issue Refund
        </Button>
      </HStack>
        ...
        <CardBody>
          <HStack spacing={4} wrap="wrap">
            ...
            <Button
              leftIcon={<FiFilter />}
              variant="outline"
              onClick={() => setFilters({ search: '', reason: '', fromDate: '', toDate: '' })}
            >
              Clear Filters
            </Button>
          </HStack>
            ...
            <CardBody>
              <HStack>
                ...
                <VStack align="start" spacing={0}>
                  <Text fontSize="sm" color="gray.600">Total Refunds</Text>
                  <Text fontSize="xl" fontWeight="bold">{formatCurrency(data.summary.totalRefunds)}</Text>
                </VStack>
              </HStack>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <HStack>
                <FiRefreshCw size={24} />
                <VStack align="start" spacing={0}>
                  <Text fontSize="sm" color="gray.600">Refund Count</Text>
                  <Text fontSize="xl" fontWeight="bold">{data.summary.refundCount}</Text>
                </VStack>
              </HStack>
            </CardBody>
          </Card>
            ...
            <CardBody>
              <HStack>
                ...
                <VStack align="start" spacing={0}>
                  ...
                  <Text fontSize="xl" fontWeight="bold">{data.refunds.length} of {data.pagination.total}</Text>
                </VStack>
              </HStack>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <HStack>
                <FiUser size={24} />
                <VStack align="start" spacing={0}>
                  <Text fontSize="sm" color="gray.600">Top Reason</Text>
            ...
            <Tbody>
              {data?.refunds.map((refund) => (
                <Tr key={refund.id}>
                  ...
                  <Td>
                    <VStack align="start" spacing={0}>
                      <Text fontSize="sm" fontWeight="medium">{refund.customer.name}</Text>
                      <Text fontSize="xs" color="gray.500">{refund.customer.email}</Text>
                    </VStack>
                  </Td>
                  <Td>
                  ...
                  <Td>
                    <Menu>
                      ...
                      />
                      <MenuList>
                        <MenuItem icon={<FiEye />} onClick={() => handleViewRefund(refund)}>
                          View Details
                        </MenuItem>
              ...
              <VStack spacing={4} align="stretch">
                ...
                <HStack justify="space-between">
                  ...
                  <VStack align="end" spacing={0}>
                    ...
                    <Text fontSize="sm" color="gray.500">{selectedRefund.customer.email}</Text>
                  </VStack>
                </HStack>
                <HStack justify="space-between">
                ...
                <HStack justify="space-between">
                  ...
                  <Text color="red.500">-{formatCurrency(selectedRefund.refundAmountPence)}</Text>
                </HStack>
                <HStack justify="space-between">
                ...
                <HStack justify="space-between">
                  ...
                  <Text>{selectedRefund.refundedAt ? new Date(selectedRefund.refundedAt).toLocaleDateString() : 'N/A'}</Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontWeight="bold">Stripe Refund ID:</Text>
                  <Text fontSize="sm">{selectedRefund.stripeRefundId || 'N/A'}</Text>
                </HStack>
