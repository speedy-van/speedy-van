...
export default function TeamSettings() {
  ...
  const handleSaveUser = async (userData: Partial<AdminUser>) => {
    try {
      ...
      toast({
        title: isEditing ? 'User updated' : 'User invited',
        ...
        <Card bg={bgColor} border="1px solid" borderColor={borderColor}>
          <CardBody>
            ...
            <Grid templateColumns="repeat(auto-fit, minmax(300px, 1fr))" gap={4}>
              {roles.map((role) => (
                <Box key={role.name} p={4} border="1px solid" borderColor={borderColor} borderRadius="md">
                  <HStack mb={2}>
            ...
            <Table variant="simple">
              ...
              <Tbody>
                {loading ? (
                  ...
                  <Tr>
                    <Td colSpan={7} textAlign="center">No admin users found.</Td>
                  </Tr>
                ) : (
                  users.map((user) => (
                    <Tr key={user.id}>
...
function UserModal({ isOpen, onClose, user, isEditing, roles, onSave }: UserModalProps) {
