    ...
    <Box p={6}>
      <VStack align="start" spacing={6} w="full">
        <Box>
          ...
          <Text color="gray.600">
            Configure third-party services and API connections for your platform.
          </Text>
        </Box>

        <Grid templateColumns="repeat(auto-fit, minmax(400px, 1fr))" gap={6} w="full">
          {integrations.map((integration) => (
            <Card key={integration.id} bg={bgColor} border="1px solid" borderColor={borderColor}>
              <CardBody>
                <VStack align="start" spacing={4}>
                  {/* Header */}
                  <HStack justify="space-between" w="full">
                    <HStack>
                      <Icon as={integration.icon} boxSize={5} color="brand.500" />
                      <Box>
                    ...
                    <VStack align="end" spacing={1}>
                      ...
                      <Switch
                        ...
                        onChange={(e) => updateIntegration(integration.id, { isEnabled: e.target.checked })}
                        size="sm"
                      />
                    </VStack>
                  </HStack>
                  ...
                  {/* Status */}
                  <HStack>
                    <Icon as={getStatusIcon(integration.status)} color={`${getStatusColor(integration.status)}.500`} />
                    <Text fontSize="sm">
                      {integration.status === 'connected' && 'Connected and working'}
                  ...
                  {editingIntegration === integration.id ? (
                    <IntegrationConfigForm
                      ...
                      onSave={(config) => updateIntegration(integration.id, { config })}
                    ...
                    <IntegrationConfigDisplay
                      ...
                      onToggleSecrets={() => toggleSecretVisibility(integration.id)}
        ...
        <Alert status="info" borderRadius="md">
          ...
          <Box>
            ...
            <Text fontSize="sm">
              API keys and secrets are encrypted at rest and only displayed when needed. 
...
function IntegrationConfigForm({ integration, onSave, onCancel }: IntegrationConfigFormProps) {
...
function IntegrationConfigDisplay({ integration, showSecrets, onToggleSecrets, onCopy }: IntegrationConfigDisplayProps) {
      ...
      {Object.entries(integration.config).map(([key, value]) => (
        <Box key={key} w="full">
          ...
          <HStack>
            <Input
              size="sm"
              value={isSecretKey(key) && !showSecrets ? '••••••••' : String(value)}
              isReadOnly
              fontFamily="mono"
            />
            {isSecretKey(key) && (
