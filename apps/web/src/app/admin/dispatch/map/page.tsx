...
import { Box, Heading, HStack, VStack, Text, Badge, Card, CardBody, Button, Select, Input, Grid, GridItem, Tabs, TabList, TabPanels, Tab, TabPanel } from "@chakra-ui/react";
import { FiMap, FiTruck, FiUsers, FiSettings, FiFilter } from "react-icons/fi";

export default function DispatchMapPage() {
  // Mock data - in real implementation, this would come from API
  const drivers = [
    { id: 1, name: "Sarah Johnson", status: "online", location: "London Bridge", rating: 4.8, currentJob: "ORD-001" },
    { id: 2, name: "Mike Wilson", status: "online", location: "Canary Wharf", rating: 4.6, currentJob: null },
    { id: 3, name: "Emma Davis", status: "break", location: "Camden", rating: 4.9, currentJob: null },
    { id: 4, name: "Tom Brown", status: "offline", location: "Shoreditch", rating: 4.7, currentJob: null }
  ];

  const activeJobs = [
    { id: 1, ref: "ORD-001", status: "en-route", pickup: "High Street", dropoff: "Oxford Street", driver: "Sarah J." },
    { id: 2, ref: "ORD-002", status: "at-pickup", pickup: "Regent Street", dropoff: "Bond Street", driver: "Mike W." },
    { id: 3, ref: "ORD-003", status: "unassigned", pickup: "Carnaby Street", dropoff: "Soho", driver: null }
  ];
                ...
                {drivers.filter(d => d.status !== "offline").map((driver) => (
                  <HStack key={driver.id} justify="space-between" p={2} bg="gray.50" borderRadius="md">
                    <VStack align="start" spacing={1}>
                      ...
                      <Text fontSize="xs" color="gray.600">{driver.location}</Text>
                      {driver.currentJob && (
                        <Text fontSize="xs" color="blue.600">Job: {driver.currentJob}</Text>
                      )}
                    </VStack>
                    <VStack align="end" spacing={1}>
                      <Badge colorScheme={getStatusColor(driver.status)} size="sm">
                        {driver.status}
                      </Badge>
              ...
              <VStack spacing={3} align="stretch">
                {activeJobs.map((job) => (
                  <HStack key={job.id} justify="space-between" p={2} bg="gray.50" borderRadius="md">
                    <VStack align="start" spacing={1}>
                      <Text fontWeight="bold" fontSize="sm">{job.ref}</Text>
                      <Text fontSize="xs" color="gray.600">
                        {job.pickup} → {job.dropoff}
                      </Text>
                      {job.driver && (
                        <Text fontSize="xs" color="blue.600">Driver: {job.driver}</Text>
                      )}
                    </VStack>
            ...
            <CardBody>
              ...
              <VStack spacing={3} align="stretch">
                <HStack justify="space-between" p={2} bg="red.50" borderRadius="md">
                  <VStack align="start" spacing={1}>
                    <Text fontWeight="bold" fontSize="sm">Late Pickup</Text>
                    <Text fontSize="xs" color="gray.600">ORD-003 • 15 min overdue</Text>
                  </VStack>
                ...
                <HStack justify="space-between" p={2} bg="yellow.50" borderRadius="md">
                  <VStack align="start" spacing={1}>
                    <Text fontWeight="bold" fontSize="sm">Vehicle Issue</Text>
                    <Text fontSize="xs" color="gray.600">Sarah J. • Flat tire</Text>
                  </VStack>
