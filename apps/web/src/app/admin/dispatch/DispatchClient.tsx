...
export default function DispatchClient({ data }: DispatchClientProps) {
    ...
    return (
      <Box p={6}>
        <Alert status="error" mb={4}>
          ...
          <AlertDescription>Invalid dispatch data received. Please refresh the page.</AlertDescription>
        </Alert>
  ...
  if (!data.jobsByStatus || !data.activeJobs || !data.availableDrivers || !data.openIncidents) {
    return (
      <Box p={6}>
        <Alert status="warning" mb={4}>
          <AlertIcon />
          <AlertTitle>Incomplete Data!</AlertTitle>
          <AlertDescription>Some dispatch data is missing. Please refresh the page.</AlertDescription>
        </Alert>
        <Button onClick={() => window.location.reload()} colorScheme="blue">
          Refresh Page
        </Button>
  ...
  return (
    <Box>
      ...
      <VStack align="start" spacing={4} mb={6}>
        ...
        <Text color="gray.600">Manage job assignments and monitor operations</Text>
      </VStack>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{Object.values(data.jobsByStatus).reduce((a, b) => a + b, 0)}</StatNumber>
              <StatHelpText>
        ...
        <CardBody>
          <SimpleGrid columns={{ base: 2, md: 4 }} spacing={4}>
            {Object.entries(data.jobsByStatus).map(([status, count]) => (
              <Box key={status} textAlign="center" p={4} borderWidth={1} borderRadius="md">
                <Badge colorScheme={getStatusColor(status)} mb={2}>
                  {status}
                </Badge>
                <Text fontSize="2xl" fontWeight="bold">{count}</Text>
                <Text fontSize="sm" color="gray.600">jobs</Text>
              </Box>
            ))}
          </SimpleGrid>
        </CardBody>
              ...
              <Tbody>
                {data.activeJobs.slice(0, 10).map((job) => (
                  <Tr key={job.id}>
                    ...
                    <Td>
                      <Text fontSize="sm">{job.customer && job.customer.name ? job.customer.name : 'Unknown'}</Text>
                    </Td>
                    <Td>
                      <Text fontSize="sm">{job.driver && job.driver.user && job.driver.user.name ? job.driver.user.name : 'Unassigned'}</Text>
                    </Td>
                    <Td>
                    ...
                    <Td>
                      {!job.driver && data.availableDrivers.length > 0 && (
                        <Button
                          ...
                          colorScheme="blue"
                          isLoading={loading}
                          onClick={() => handleAssignJob(job.id, data.availableDrivers[0].id)}
                        >
                          Assign
                        </Button>
                      )}
              ...
              {data.availableDrivers.map((driver) => (
                <Box key={driver.id} p={4} borderWidth={1} borderRadius="md">
                  <HStack justify="space-between" mb={2}>
                    <Text fontWeight="bold">{driver.user && driver.user.name ? driver.user.name : 'Unknown'}</Text>
                    <Badge colorScheme="green" size="sm">Available</Badge>
                  </HStack>
                  <Text fontSize="sm" color="gray.600">{driver.user && driver.user.email ? driver.user.email : 'No email'}</Text>
            ...
            <Table variant="simple">
              ...
              <Tbody>
                {data.openIncidents.map((incident) => (
                  <Tr key={incident.id}>
                    <Td>
                      <Text fontSize="sm">{incident.driver && incident.driver.user && incident.driver.user.name ? incident.driver.user.name : 'Unknown'}</Text>
                    </Td>
                    <Td>
                      <Badge colorScheme="red" size="sm">{incident.type}</Badge>
                    </Td>
                    <Td>
