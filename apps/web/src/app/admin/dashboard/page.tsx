...
export default function AdminDashboard() {
  ...
  const handleCardClick = (type: string, value?: string) => {
    switch (type) {
      case 'revenue':
        ...
        break;
      case 'activeJobs':
        router.push('/admin/orders?status=assigned,en-route,at-pickup,in-transit,at-drop');
        break;
      case 'newOrders':
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'at-pickup': return 'yellow';
      case 'in-transit': return 'purple';
      case 'at-drop': return 'orange';
      case 'late-pickup': return 'red';
      default: return 'gray';
    }
  };
      ...
      <Box>
        ...
        <Grid templateColumns={{ base: "1fr", lg: "2fr 1fr" }} gap={6}>
                      ...
                      <CardBody p={3}>
                        <HStack justify="space-between">
                          <VStack align="start" spacing={1} flex={1}>
                            ...
                            <Text fontSize="sm" color="gray.600">{job.driver}</Text>
                          ...
                          <VStack align="end" spacing={1}>
                            ...
                            <Text fontSize="sm" color="gray.600">{job.eta}</Text>
                        ...
                        {job.timeSinceClaimed > 0 && (
                          <Progress 
                            value={Math.min((job.timeSinceClaimed / 60) * 100, 100)} 
                            size="xs" 
                            mt={2}
                            colorScheme={job.timeSinceClaimed > 45 ? 'red' : 'blue'}
                          />
                        )}
                      </CardBody>
                    </Card>
          ...
          <VStack spacing={6}>
            ...
            <Card>
              <CardBody>
                <Heading size="md" mb={4}>Queue</Heading>
                <VStack spacing={3} align="stretch">
                  <Card 
                    size="sm" 
                    cursor="pointer"
                  ...
                  >
                    <CardBody p={3}>
                      <HStack justify="space-between">
                        ...
                        <Badge colorScheme="blue">{data.queue.driverApplications}</Badge>
                      </HStack>
                    </CardBody>
                  </Card>
                  ...
                  >
                    <CardBody p={3}>
                      <HStack justify="space-between">
                        ...
                        <Badge colorScheme="yellow">{data.queue.pendingRefunds}</Badge>
                      </HStack>
                    </CardBody>
                  </Card>
                  ...
                  >
                    <CardBody p={3}>
                      <HStack justify="space-between">
                        ...
                        <Badge colorScheme="red">{data.queue.disputedPayouts}</Badge>
                      </HStack>
                    </CardBody>
                  </Card>
                </VStack>
                ...
                <VStack spacing={3} align="stretch">
                  {Object.entries(data.systemHealth).map(([service, status]) => (
                    <HStack key={service} justify="space-between">
                      <Text textTransform="capitalize">{service.replace(/([A-Z])/g, ' $1')}</Text>
