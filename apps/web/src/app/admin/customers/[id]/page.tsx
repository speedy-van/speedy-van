...
import { Box, Heading, HStack, VStack, Text, Badge, Card, CardBody, Button, Tabs, TabList, TabPanels, Tab, TabPanel, Grid, GridItem, Avatar, Stat, StatLabel, StatNumber, StatHelpText, SimpleGrid, Table, Thead, Tbody, Tr, Th, Td, Divider, useToast, Spinner, Alert, AlertIcon, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalFooter, ModalCloseButton, useDisclosure, Input, Textarea, Select } from "@chakra-ui/react";
import { FiUser, FiMail, FiPhone, FiMapPin, FiPackage, FiMessageSquare, FiFileText, FiDollarSign, FiFlag, FiDownload, FiRefreshCw, FiEdit } from "react-icons/fi";
...
export default function CustomerDetailPage({ params }: CustomerDetailPageProps) {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/customers/${customer.id}/actions`, {
        method: 'POST',
        headers: {
      ...
      onClose();
      setActionData({ action: '', amount: '', reason: '', bookingId: '', notes: '' });
      ...
      toast({
        title: 'Error',
        description: err instanceof Error ? err.message : 'Failed to process action',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
      ...
      <HStack justify="space-between" mb={6}>
        <HStack spacing={4}>
          ...
          <VStack align="start" spacing={1}>
            ...
            <Text color="gray.600">Joined {new Date(customer.createdAt).toLocaleDateString()}</Text>
          </VStack>
        </HStack>
        <HStack spacing={3}>
          <Button 
          ...
          <CardBody>
            <Stat>
              ...
              <StatHelpText>
                ...
                Rate: {(((customer.cancellations || 0) / (customer.totalOrders || 1)) * 100).toFixed(1)}%
              </StatHelpText>
            </Stat>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody>
            <Stat>
              ...
              <StatNumber>{new Date(customer.joinedAt || customer.createdAt).getFullYear()}</StatNumber>
              <StatHelpText>
                <FiUser style={{ display: 'inline', marginRight: '4px' }} />
                {new Date(customer.joinedAt || customer.createdAt).toLocaleDateString()}
              </StatHelpText>
            </Stat>
          </CardBody>
        </Card>
      ...
      <Tabs>
        ...
        <TabPanels>
              ...
              <VStack spacing={6} align="stretch">
                ...
                <Card>
                  <CardBody>
                    ...
                    <VStack spacing={3} align="stretch">
                      <Button leftIcon={<FiPackage />} colorScheme="blue" size="sm">
                        Create New Order
                      </Button>
          ...
          <TabPanel>
            <Card>
              <CardBody>
                <Heading size="md" mb={4}>Support Tickets</Heading>
                <Table variant="simple">
                  <Thead>
                    <Tr>
                      <Th>Subject</Th>
                  ...
                  <Tbody>
                    {customer.supportTickets.map((ticket) => (
                      <Tr key={ticket.id}>
                        ...
                        <Td>
                          <Text fontSize="sm">{new Date(ticket.createdAt).toLocaleDateString()}</Text>
                        </Td>
                        <Td>
                          <Text fontSize="sm">
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            {(actionData.action === 'add_flag' || actionData.action === 'remove_flag') && (
              <VStack spacing={4}>
                <Select
                  placeholder="Reason"
                  value={actionData.reason}
