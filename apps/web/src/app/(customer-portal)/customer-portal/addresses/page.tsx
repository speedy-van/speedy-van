...
export default function AddressesPage() {
  ...
  const { isOpen: isAddressFormOpen, onOpen: onAddressFormOpen, onClose: onAddressFormClose } = useDisclosure();
  const { isOpen: isContactFormOpen, onOpen: onContactFormOpen, onClose: onContactFormClose } = useDisclosure();
  const { isOpen: isDeleteAddressOpen, onOpen: onDeleteAddressOpen, onClose: onDeleteAddressClose } = useDisclosure();
  ...
  const handleSaveAddress = async (addressData: any) => {
    try {
      ...
      const body = selectedAddress ? { ...addressData, id: selectedAddress.id } : addressData;
  ...
  const confirmDeleteAddress = async () => {
    ...
    try {
      const response = await fetch(`/api/portal/addresses?id=${deleteAddress.id}`, {
        method: 'DELETE',
      });
  ...
  const handleSaveContact = async (contactData: any) => {
    try {
      ...
      const body = selectedContact ? { ...contactData, id: selectedContact.id } : contactData;
  ...
  const confirmDeleteContact = async () => {
    ...
    try {
      const response = await fetch(`/api/portal/contacts?id=${deleteContact.id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <VStack align="stretch" spacing={8}>
      <Box>
        ...
        <Text color="gray.600">Manage your saved addresses and contacts for quick booking</Text>
      </Box>
      ...
      <Box>
        ...
        {addresses.length === 0 ? (
          <Box textAlign="center" py={8} borderWidth="1px" borderRadius="md" borderStyle="dashed">
            <Text fontSize="lg" mb={2}>No saved addresses</Text>
              ...
              <GridItem key={address.id}>
                ...
                >
                  ...
                  <VStack align="start" spacing={1}>
                    ...
                    {address.line2 && <Text fontSize="sm">{address.line2}</Text>}
                    <Text fontSize="sm">{address.city}, {address.postcode}</Text>
                    {(address.floor || address.flat) && (
                      <Text fontSize="sm" color="gray.600">
                        Floor: {address.floor || 'N/A'} | Flat: {address.flat || 'N/A'}
                      </Text>
                    )}
      ...
      <Box>
        ...
        {contacts.length === 0 ? (
          <Box textAlign="center" py={8} borderWidth="1px" borderRadius="md" borderStyle="dashed">
            <Text fontSize="lg" mb={2}>No saved contacts</Text>
            <Text color="gray.600" mb={4}>
              ...
              <GridItem key={contact.id}>
                ...
                >
                  ...
                  <VStack align="start" spacing={1}>
                    ...
                    {contact.email && (
                      <Text fontSize="sm" color="blue.600">{contact.email}</Text>
                    )}
                    {contact.notes && (
                      <Text fontSize="sm" color="gray.600" fontStyle="italic">
                        "{contact.notes}"
      ...
      <Box p={4} bg="blue.50" borderRadius="md">
        <Text fontSize="sm" color="blue.800">
          💡 <strong>Tip:</strong> Saved addresses and contacts will automatically prefill when you make new bookings, saving you time!
        </Text>
      </Box>
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              Are you sure you want to delete "{deleteAddress?.label}"? This action cannot be undone.
            </AlertDialogBody>
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              Are you sure you want to delete "{deleteContact?.label}"? This action cannot be undone.
            </AlertDialogBody>
