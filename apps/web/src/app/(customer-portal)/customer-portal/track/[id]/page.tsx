...
const STATUS_CONFIG = {
  ...
  en_route_dropoff: { label: "En Route to Destination", color: "green", progress: 90 },
...
export default function TrackByIdPage({ params }: { params: { id: string } }) {
  ...
  async function loadTracking() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          toast({
            ...
            description: "This booking doesn't exist or you don't have permission to view it",
            status: "error",
            duration: 5000,
            isClosable: true,
          });
      ...
      if (data.last && data.booking.pickupLat && data.booking.pickupLng) {
        loadETA(data.last.lat, data.last.lng, data.booking.pickupLat, data.booking.pickupLng);
      }
  ...
  async function loadETA(fromLat: number, fromLng: number, toLat: number, toLng: number) {
    try {
      const response = await fetch(`/api/track/eta?from=${fromLat},${fromLng}&to=${toLat},${toLng}`);
      if (response.ok) {
        const data = await response.json();
        setEtaInfo(data);
      }
  ...
  function setupRealtimeUpdates(channel: string) {
    ...
    channelInstance.bind('location-update', (data: any) => {
      ...
      if (trackingInfo?.booking.pickupLat && trackingInfo?.booking.pickupLng) {
        loadETA(data.lat, data.lng, trackingInfo.booking.pickupLat, trackingInfo.booking.pickupLng);
      }
    });
  ...
  function handleContactDriver() {
    if (trackingInfo?.driver?.phone) {
      ...
      const maskedPhone = trackingInfo.driver.phone.replace(/(\d{4})(\d{3})(\d{3})/, '$1 *** $3');
      toast({
  ...
  const statusConfig = STATUS_CONFIG[trackingInfo.booking.status as keyof typeof STATUS_CONFIG] || 
      ...
      <VStack spacing={6} align="stretch">
        ...
        <Card>
          <CardBody>
            <VStack spacing={4} align="stretch">
              ...
              <Progress value={statusConfig.progress} colorScheme={statusConfig.color} size="lg" />
              {lastUpdate && (
            ...
            <VStack spacing={4} align="stretch">
              ...
              {trackingInfo.booking.preferredDate && (
                <HStack>
                  <FiClock />
                  <Text fontSize="sm">
                    Scheduled: {new Date(trackingInfo.booking.preferredDate).toLocaleDateString()}
                    {trackingInfo.booking.timeSlot && ` (${trackingInfo.booking.timeSlot})`}
                  </Text>
                </HStack>
              )}
              ...
              <HStack>
                <FiTruck />
                <Text fontSize="sm">
                  {trackingInfo.booking.vanSize} van • {trackingInfo.booking.crewSize} crew member{trackingInfo.booking.crewSize !== 1 ? 's' : ''}
                </Text>
              </HStack>
            </VStack>
          </CardBody>
                ...
                {etaInfo ? (
                  <VStack spacing={2} align="stretch">
                    <Stat>
                      ...
                      <StatNumber>{Math.round(etaInfo.duration / 60)} min</StatNumber>
                      <StatHelpText>{Math.round(etaInfo.distance / 1000)} km away</StatHelpText>
                    </Stat>
                  </VStack>
                ) : driverLocation ? (
                  <VStack spacing={2} align="center">
                    <Spinner size="sm" />
                    <Text fontSize="sm" color="gray.500">Calculating ETA...</Text>
                  </VStack>
                ) : (
        ...
        <Alert status="info">
          ...
          <VStack align="start" spacing={1}>
            ...
            <Text fontSize="sm">
              Driver location updates every 15-30 seconds. Contact information is masked for security.
            </Text>
          </VStack>
        </Alert>
      </VStack>
