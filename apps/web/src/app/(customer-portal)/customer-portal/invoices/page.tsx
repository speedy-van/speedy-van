...
import { FiDownload, FiFileText, FiFilter, FiSearch, FiCalendar, FiExternalLink } from "react-icons/fi";
...
export default function InvoicesPage() {
  ...
  const getPaymentStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'requires_action': return 'yellow';
      default: return 'gray';
    }
  };
  ...
  const exportCSV = async () => {
    try {
      ...
      toast({
        title: 'Export failed',
        description: err instanceof Error ? err.message : 'Failed to export invoices',
        status: 'error',
        duration: 5000,
      });
    } finally {
  ...
  if (loading && invoices.length === 0) {
    return (
      <VStack align="stretch" spacing={6}>
        <Box>
          <Heading size="lg" mb={2}>Invoices & Payments</Heading>
          <Text color="gray.600">View and download your invoices and payment history</Text>
        </Box>
    ...
    <VStack align="stretch" spacing={6}>
      ...
      {invoices.length === 0 && !loading ? (
        <Box textAlign="center" py={12}>
          <Text fontSize="lg" mb={4}>No invoices yet</Text>
        ...
        <Box>
                            ...
                            {invoice.stripePaymentIntentId && (
                              <Tooltip label="View Stripe Receipt">
                                <IconButton
                                  ...
                                  variant="ghost"
                                  onClick={async () => {
                  const url = await getStripeReceiptUrl(invoice.stripePaymentIntentId);
                  if (url) {
          ...
          <Box mt={6} p={4} bg="gray.50" borderRadius="md">
            <Text fontSize="sm" color="gray.600">
              Need a copy of your invoice? Contact our support team and we'll be happy to help.
            </Text>
          </Box>
        </Box>
      )}
