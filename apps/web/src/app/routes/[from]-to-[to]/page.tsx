  ...
  return (
    <main className="route-container">
      ...
      <section className="route-hero">
        <h1>Man and Van {from.name} to {to.name}</h1>
        <p>Trusted removals between {from.name} and {to.name}. Fixed quotes and real-time tracking.</p>
        ...
        <div className="route-details">
          <div className="from-to">
            <div className="location from">
              ...
              <p>{from.type} • {from.region}</p>
              {from.population && <p>Population: {from.population.toLocaleString()}</p>}
            </div>
            <div className="arrow">→</div>
            <div className="location to">
              <h3>To: {to.name}</h3>
              <p>{to.type} • {to.region}</p>
              {to.population && <p>Population: {to.population.toLocaleString()}</p>}
            </div>
          </div>
        </div>
      </section>
