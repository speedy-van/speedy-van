  ...
  return (
    <VStack align="stretch" spacing={6}>
      ...
      {invoices.length === 0 ? (
        <Center py={12}>
          <VStack spacing={4}>
            <Text fontSize="lg" color="gray.600">No invoices yet</Text>
            ...
            <Tbody>
              {invoices.map((invoice) => (
                <Tr key={invoice.id}>
                  ...
                  <Td>
                    <Badge
                      colorScheme={getPaymentStatusColor(invoice.status === 'PENDING_PAYMENT' ? 'pending' : 'paid')}
                      variant="subtle"
                    >
                      {(invoice.status === 'PENDING_PAYMENT' ? 'pending' : 'paid').toUpperCase()}
                    </Badge>
                  </Td>
                  <Td>
                    <Text fontSize="sm">
