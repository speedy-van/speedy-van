...
import { trackingService, TrackingData, TrackingUpdate, RealTimeTrackingOptions } from '@/lib/tracking-service';
...
export function useRealTimeTracking(options: UseRealTimeTrackingOptions = {}): UseRealTimeTrackingReturn {
  ...
  const lookupBooking = useCallback(async (bookingCode: string): Promise<TrackingData | null> => {
    if (!bookingCode.trim()) {
...
export function useAdminRealTimeTracking(options: RealTimeTrackingOptions = {}) {
