...
export function useTrackingUpdates({ 
  ...
  const sendLocationUpdate = async (lat: number, lng: number, bookingId: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to send location update');
      throw err;
    }
  };
...
export function useAdminTrackingUpdates() {
  const [allUpdates, setAllUpdates] = useState<Record<string, TrackingUpdate[]>>({});
  const [isConnected, setIsConnected] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const pusherRef = useRef<Pusher | null>(null);
  const channelRef = useRef<Channel | null>(null);
  ...
  useEffect(() => {
    ...
    if (!channelRef.current) {
      channelRef.current = pusherRef.current.subscribe('private-admin-tracking');
