...
export function useChat(options: UseChatOptions = {}) {
  ...
  // Send a message
  const sendMessage = useCallback(async (sessionId: string, content: string, type: string = 'text') => {
  ...
  const updateTypingStatus = useCallback(async (sessionId: string, isTyping: boolean) => {
    if (!sessionId) return;
  ...
  const handleTyping = useCallback((sessionId: string, isTyping: boolean) => {
  ...
  const initializePusher = useCallback((sessionId: string) => {
    ...
    try {
      ...
      channel.bind('typing:update', (data: { userId: string; isTyping: boolean; userName: string }) => {
