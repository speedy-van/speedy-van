...
type ErrorType = 'validation' | 'authentication' | 'authorization' | 'network' | 'server' | 'unknown';
...
export interface RealtimeDataConfig<T = any> {
  ...
  cacheConfig?: typeof cacheConfigs.fast | typeof cacheConfigs.standard | typeof cacheConfigs.slow | typeof cacheConfigs.realtime;
...
export function useRealtimeData<T = any>(
  ...
  // Generate cache key
  const cacheKey = channelId ? `${namespace}-${channelId}` : namespace;
  const fullCacheKey = `${cacheKey}:${event}`;

  // SWR for caching
  const { data: cachedData, error: cacheError, mutate } = useCachedData(
    fullCacheKey,
    async () => {
  ...
  const initializeManager = useCallback(async () => {
    ...
    try {
      ...
      const unsubscribeConnection = manager.onConnectionStateChange((connectionState) => {
      ...
      const unsubscribe = manager.subscribe(
        ...
        (data: any) => {
          try {
            ...
            if (data.type?.includes('COMPLETED') || data.type?.includes('CONFIRMED')) {
              // addToast({
              //   type: 'success',
  ...
  }, [namespace, channelId, event, enablePolling, pollingInterval, errorConfig.maxRetries, transform, onDataUpdate, onConnectionChange, onError, addToast, mutate]);
  ...
  const handleOptimisticUpdate = useCallback(async (optimisticData: T) => {
