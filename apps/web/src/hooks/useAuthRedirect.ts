...
export function useAuthRedirect() {
    ...
    if (returnTo) {
      ...
      try {
        ...
        if (returnToUrl.origin === window.location.origin) {
          redirectUrl = returnToUrl.pathname + returnToUrl.search + returnToUrl.hash;
        }
      } catch (error) {
  ...
  const getValidReturnTo = useCallback(() => {
    ...
    try {
      const returnToUrl = new URL(returnTo, typeof window !== 'undefined' ? window.location.origin : 'http://localhost');
      // Only allow same-origin redirects
      if (returnToUrl.origin === (typeof window !== 'undefined' ? window.location.origin : 'http://localhost')) {
