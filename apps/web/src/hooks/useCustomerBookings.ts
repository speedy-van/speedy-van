...
interface UseCustomerBookingsReturn {
  ...
  linkExistingBookings: (email: string, phone: string) => Promise<{
    success: boolean;
    message: string;
    linkedCount: number;
  }>;
...
export function useCustomerBookings(): UseCustomerBookingsReturn {
  ...
  const linkExistingBookings = useCallback(async (email: string, phone: string) => {
    if (!session?.user?.id) {
      return {
        success: false,
  ...
  const linkSpecificBooking = useCallback(async (bookingId: string) => {
    if (!session?.user?.id) {
      return {
        success: false,
        message: 'User not authenticated'
    ...
    try {
      const response = await fetch(`/api/customer/bookings/${bookingId}/link`, {
        method: 'POST'
      });
