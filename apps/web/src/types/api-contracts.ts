...
export type RefundReason = 'customer_request' | 'service_issue' | 'duplicate_charge' | 'fraud' | 'other';
...
export type WebhookEvent = OrderStatusWebhook | DriverAssignmentWebhook | PaymentWebhook;
...
export function isRefundReason(reason: string): reason is RefundReason {
  return ['customer_request', 'service_issue', 'duplicate_charge', 'fraud', 'other'].includes(reason);
}
