...
export function OrderDetailDrawer({ isOpen, onClose, orderCode }: OrderDetailDrawerProps) {
  ...
  const loadOrder = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEditData({
          status: data.status,
          scheduledAt: data.scheduledAt ? format(new Date(data.scheduledAt), 'yyyy-MM-dd') : '',
          notes: ''
        });
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: autoAssign ? 'Driver auto-assigned successfully' : 'Driver assigned successfully',
          status: 'success',
          duration: 3000,
          isClosable: true,
        });
                      ...
                      <Card>
                        <CardBody>
                          <HStack mb={4}>
                            ...
                            <Text fontWeight="bold" fontSize="lg">Route</Text>
                          </HStack>
                        ...
                        <CardBody>
                          ...
                          {order.items && order.items.length > 0 && (
                            <Box mt={4} p={3} bg="blue.50" borderRadius="md">
                              <Text fontSize="sm" fontWeight="medium" mb={2}>Items ({order.items.length})</Text>
                              <VStack spacing={1} align="stretch">
                                {order.items.slice(0, 5).map((item, index) => (
                                  <Text key={index} fontSize="sm">
                                    {item.quantity}x {item.name} ({item.volumeM3}m³)
                                  </Text>
                                ))}
                    ...
                    <VStack spacing={6} align="stretch">
                      ...
                      <Card>
                        <CardBody>
                          <HStack mb={4}>
                            <Icon as={FiUser} />
                            <Text fontWeight="bold" fontSize="lg">Customer</Text>
                          </HStack>
                          <VStack spacing={3} align="stretch">
                            ...
                            <HStack spacing={3}>
                              <Button size="sm" leftIcon={<Icon as={FiMail} />} variant="outline">
                                Email
                              </Button>
                              <Button size="sm" leftIcon={<Icon as={FiPhone} />} variant="outline">
                                Call
                              </Button>
                            </HStack>
                          </VStack>
                      ...
                      <Card>
                        <CardBody>
                          <HStack mb={4}>
                            <Icon as={FiTruck} />
                            <Text fontWeight="bold" fontSize="lg">Driver</Text>
                          </HStack>
                          {order.driver ? (
                            <VStack spacing={3} align="stretch">
                              <HStack>
                                <Avatar size="sm" name={order.driver.user.name} />
                                <VStack align="start" spacing={1}>
                                  <Text fontWeight="bold">{order.driver.user.name}</Text>
                                  <Text fontSize="sm">{order.driver.user.email}</Text>
                                  {order.driver.rating && (
                                    <HStack>
                                      <Icon as={FiStar} color="yellow.400" />
                                      <Text fontSize="sm">{order.driver.rating.toFixed(1)}</Text>
                                    </HStack>
                                  )}
                                </VStack>
                              </HStack>
                      ...
                      <Card>
                        <CardBody>
                          <HStack mb={4}>
                            ...
                            <Text fontWeight="bold" fontSize="lg">Payment</Text>
                          </HStack>
                          <VStack spacing={3} align="stretch">
                            <HStack justify="space-between">
                              <Text>Status</Text>
                            ...
                            {order.paidAt && (
                              <HStack justify="space-between">
                                <Text>Paid At</Text>
                                <Text fontSize="sm">{format(new Date(order.paidAt), 'MMM dd, yyyy HH:mm')}</Text>
                              </HStack>
                            )}
                ...
                <TabPanel>
                  <Card>
                    <CardBody>
                      <HStack mb={4}>
                        <Icon as={FiClock} />
                        <Text fontWeight="bold" fontSize="lg">Order Timeline</Text>
                      </HStack>
                      <VStack spacing={4} align="stretch">
                ...
                <TabPanel>
                  <Card>
                    <CardBody>
                      <HStack mb={4}>
                        <Icon as={FiDollarSign} />
                        <Text fontWeight="bold" fontSize="lg">Payment Information</Text>
                      </HStack>
                      <VStack spacing={4} align="stretch">
                        <HStack justify="space-between">
                          <Text>Status</Text>
                        ...
                        <VStack spacing={2} align="stretch">
                          <Text fontWeight="bold" mb={2}>Pricing Breakdown</Text>
                          <HStack justify="space-between">
                            <Text>Distance Cost ({order.baseDistanceMiles.toFixed(1)} miles)</Text>
                          ...
                          <HStack justify="space-between">
                            <Text>Crew Multiplier ({order.crewMultiplierPercent > 0 ? '+' : ''}{order.crewMultiplierPercent}%)</Text>
                            <Text>{order.crewMultiplierPercent > 0 ? '+' : ''}£{Math.round((order.totalGBP * order.crewMultiplierPercent) / 100)}</Text>
                          </HStack>
                          <HStack justify="space-between">
                            <Text>Availability Multiplier ({order.availabilityMultiplierPercent > 0 ? '+' : ''}{order.availabilityMultiplierPercent}%)</Text>
                            <Text>{order.availabilityMultiplierPercent > 0 ? '+' : ''}£{Math.round((order.totalGBP * order.availabilityMultiplierPercent) / 100)}</Text>
                          </HStack>
                        </VStack>
                        ...
                        <HStack justify="space-between">
                          ...
                          <Text fontWeight="bold" fontSize="lg">£{order.totalGBP}</Text>
                        </HStack>
                        
                        {order.paidAt && (
                          <HStack justify="space-between">
                            <Text>Paid At</Text>
                            <Text>{format(new Date(order.paidAt), 'MMM dd, yyyy HH:mm')}</Text>
                          </HStack>
                        )}
                  ...
                  <Card>
                    <CardBody>
                      <HStack mb={4}>
                        ...
                        <Text fontWeight="bold" fontSize="lg">Communications</Text>
                      </HStack>
                      <VStack spacing={4} align="stretch">
                        ...
                        <HStack justify="space-between">
                          <HStack>
                            ...
                            <VStack align="start" spacing={1}>
                              ...
                              <HStack spacing={2}>
                                <Badge colorScheme={driver.suitability === 'available' ? 'green' : 'red'}>
