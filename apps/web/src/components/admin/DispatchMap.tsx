...
export default function DispatchMap({
  ...
  const [mapView, setMapView] = useState<'drivers' | 'jobs' | 'heatmap'>('drivers');
  ...
  const filteredDrivers = drivers.filter(driver => {
    if (filters.driverStatus !== 'all' && driver.availability.status !== filters.driverStatus) {
      return false;
    }
    return true;
  });
  ...
  return (
    <Box>
      ...
      <Card>
        <CardBody p={0}>
          ...
          >
            ...
            {/* Mock Driver Markers */}
            {mapView === 'drivers' && filteredDrivers.map((driver, index) => (
              <Box
                key={driver.id}
                position="absolute"
                left={`${20 + (index % 5) * 15}%`}
              ...
              >
                <Tooltip label={`${driver.user.name} - ${driver.availability.status}`}>
                  <Box
                    w={8}
                    h={8}
            ...
            {/* Mock Job Markers */}
            {mapView === 'jobs' && filteredJobs.map((job, index) => (
              <Box
                key={job.id}
                position="absolute"
                left={`${25 + (index % 4) * 18}%`}
      ...
      <Drawer isOpen={driverDetailsOpen} onClose={() => setDriverDetailsOpen(false)} size="md">
        <DrawerOverlay />
        <DrawerContent>
          <DrawerCloseButton />
      ...
      {/* Job Details Drawer */}
      <Drawer isOpen={jobDetailsOpen} onClose={() => setJobDetailsOpen(false)} size="md">
        <DrawerOverlay />
        <DrawerContent>
          <DrawerCloseButton />
