...
export default function OnboardingWizard({
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      ...
      case 2: // Documents
        if (!formData.documents.drivingLicense) newErrors.documents_drivingLicense = "Driving license is required";
        if (!formData.documents.insurance) newErrors.documents_insurance = "Insurance document is required";
        if (!formData.documents.mot) newErrors.documents_mot = "MOT certificate is required";
        break;
          ...
          <VStack spacing={4} align="stretch" w="full">
            ...
            <FormControl>
              ...
              <Input
                ...
                onChange={(e) => updateFormData("nationalInsurance", e.target.value.toUpperCase())}
      ...
      case 2:
        return (
          <VStack spacing={4} align="stretch" w="full">
            ...
            <Alert status="info">
              <AlertIcon />
              Please upload clear, readable copies of your documents. All documents must be valid and not expired.
            </Alert>
            ...
            <FormControl isRequired>
              ...
              <Input
                ...
                onChange={(e) => updateDocument("drivingLicense", e.target.files?.[0] || null)}
              />
            ...
            <FormControl isRequired>
              ...
              <Input
                type="file"
                accept=".pdf,.jpg,.jpeg,.png"
                onChange={(e) => updateDocument("insurance", e.target.files?.[0] || null)}
              />
              <Text fontSize="sm" color="gray.600">
            ...
            <FormControl isRequired>
              ...
              <Input
                type="file"
                accept=".pdf,.jpg,.jpeg,.png"
                onChange={(e) => updateDocument("mot", e.target.files?.[0] || null)}
              />
              <Text fontSize="sm" color="gray.600">
            ...
            <FormControl>
              ...
              <Input
                type="file"
                accept=".pdf,.jpg,.jpeg,.png"
                onChange={(e) => updateDocument("rightToWork", e.target.files?.[0] || null)}
              />
              <Text fontSize="sm" color="gray.600">
        ...
        return (
          <VStack spacing={4} align="stretch" w="full">
            ...
            <Box p={4} border="1px" borderColor="gray.200" borderRadius="md">
              ...
              {formData.nationalInsurance && <Text>NI Number: {formData.nationalInsurance}</Text>}
            </Box>

            <Box p={4} border="1px" borderColor="gray.200" borderRadius="md">
              <Text fontWeight="bold" mb={2}>Vehicle Information</Text>
              <Text>Vehicle: {formData.vehicleMake} {formData.vehicleModel}</Text>
              <Text>Registration: {formData.vehicleReg}</Text>
              <Text>Type: {formData.vehicleType}</Text>
              {formData.vehicleCapacity && <Text>Capacity: {formData.vehicleCapacity}</Text>}
              <Text>MOT Expiry: {formData.motExpiry}</Text>
            </Box>

            <Box p={4} border="1px" borderColor="gray.200" borderRadius="md">
              <Text fontWeight="bold" mb={2}>Documents</Text>
              <Text>Driving License: {formData.documents.drivingLicense ? "✓ Uploaded" : "✗ Missing"}</Text>
              <Text>Insurance: {formData.documents.insurance ? "✓ Uploaded" : "✗ Missing"}</Text>
              <Text>MOT Certificate: {formData.documents.mot ? "✓ Uploaded" : "✗ Missing"}</Text>
              <Text>Right to Work: {formData.documents.rightToWork ? "✓ Uploaded" : "Optional"}</Text>
            </Box>
