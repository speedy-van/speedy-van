...
import { FiClock, FiCheck, FiX, FiMapPin, FiTruck, FiDollarSign } from "react-icons/fi";
...
export default function ClaimedJobHandler({ onJobAccepted, onJobDeclined }: ClaimedJobHandlerProps) {
  ...
  const acceptJob = async () => {
    ...
    try {
      const response = await fetch(`/api/driver/jobs/${claimedJob.jobId}/accept`, {
        method: 'POST'
      });
  ...
  const declineJob = async () => {
    ...
    try {
      const response = await fetch(`/api/driver/jobs/${claimedJob.jobId}/decline`, {
        method: 'POST'
      });
  ...
  const progressPercentage = claimedJob.expiresAt ? 
    Math.max(0, Math.min(100, ((300 - timeLeft) / 300) * 100)) : 0;
      ...
      <CardBody>
        <VStack align="stretch" spacing={4}>
          ...
          <HStack justify="space-between">
            <HStack>
              <Text fontWeight="bold" fontSize="lg">Job #{claimedJob.job.reference}</Text>
          ...
          <Box>
            <HStack justify="space-between" mb={2}>
              ...
              <Text fontSize="lg" fontWeight="bold" color={timeLeft < 60 ? "red.500" : "orange.500"}>
                {formatTime(timeLeft)}
              </Text>
            </HStack>
            <Progress 
