      ...
      <VStack spacing={6} align="stretch">
                  ...
                  <VStack align="start" spacing={1}>
                    <HStack>
                      ...
                      <Badge colorScheme="blue" size="sm">
                        {bookingData.pickupProperty.propertyType || 'Not specified'}
                      </Badge>
                    </HStack>
                    <HStack>
                    ...
                    <HStack>
                      <Text fontSize="sm" color="gray.600">Lift:</Text>
                      <Badge colorScheme={bookingData.pickupProperty.hasLift ? 'green' : 'red'} size="sm">
                ...
                <Box p={3} bg="gray.50" borderRadius="md">
                  ...
                  <VStack align="start" spacing={1}>
                    <HStack>
                      <Text fontSize="sm" color="gray.600">Type:</Text>
                      <Badge colorScheme="blue" size="sm">
                        {bookingData.dropoffProperty.propertyType || 'Not specified'}
                      </Badge>
                    </HStack>
                    <HStack>
                      <Text fontSize="sm" color="gray.600">Floor:</Text>
                    ...
                    <HStack>
                      <Text fontSize="sm" color="gray.600">Lift:</Text>
                      <Badge colorScheme={bookingData.dropoffProperty.hasLift ? 'green' : 'red'} size="sm">
        ...
        <Box>
          ...
          <HStack justify="space-between" mb={3}>
            <Text fontSize="sm" color="gray.600">
              {bookingData.items?.length || 0} item{(bookingData.items?.length || 0) !== 1 ? 's' : ''}
            </Text>
            ...
            <VStack spacing={2} align="stretch">
              {bookingData.items.map((item: any, index: number) => (
                <HStack key={index} justify="space-between" p={2} bg="gray.50" borderRadius="md">
                  ...
                  <HStack spacing={2}>
                    <Badge colorScheme="green" size="sm">Qty: {item.quantity}</Badge>
                    <Badge colorScheme="blue" size="sm">{item.volume} m³</Badge>
                  </HStack>
                </HStack>
              ))}
            </VStack>
          ...
          <Box>
            ...
            <VStack align="start" spacing={2}>
              <HStack>
                ...
                <Text fontSize="sm" color="gray.700">{formatDate(bookingData.date)}</Text>
              </HStack>
              <HStack>
                <Text fontSize="sm" fontWeight="medium" color="gray.600">Time:</Text>
                <Text fontSize="sm" color="gray.700">{formatTime(bookingData.timeSlot)}</Text>
              </HStack>
            </VStack>
          </Box>
