...
import { useScheduleService, convertToEnhancedTimeSlot } from '@/lib/booking/schedule-service';
                ...
                <VStack spacing={2} align="stretch">
                  <HStack>
                    ...
                    <Text fontSize="xs" color="gray.600" alignSelf="end" pb={2}>
                      × {item.volume}m³ ={' '}
                      {(item.quantity * item.volume).toFixed(1)}m³
                    </Text>
                  </HStack>
...
const EditableCustomer: React.FC<EditableCustomerProps> = ({
  customer,
  onUpdate,
}) => {
  const [isEditing, setIsEditing] = useState(false);
...
export const LuxuryBookingSummary: React.FC<LuxuryBookingSummaryProps> = ({
  ...
  const pricing = useMemo(() => {
    if (
      !bookingData.step1?.items?.length ||
      !bookingData.step1?.distance ||
      !bookingData.step2?.schedule?.timeSlot
    ) {
      return null;
    }
    ...
    try {
      return pricingEngine.calculatePricing({
        ...
        serviceType: bookingData.step2.serviceType || 'man-and-van',
        distance: bookingData.step1.distance || 0,
        estimatedDuration: 2,
        timeSlot: convertToEnhancedTimeSlot({
          id: '1',
          startTime: '09:00',
          endTime: '17:00',
          available: true,
        ...
        date: new Date(bookingData.step2.schedule?.date || Date.now()),
        pickupProperty: {
          type: 'house',
          floor: 0,
          hasLift: false,
  ...
  const handleItemUpdate = useCallback(
    (itemId: string, updates: Partial<BookingItem>) => {
  ...
  const handleAddressUpdate = useCallback(
    (type: 'pickup' | 'dropoff', updates: Partial<Address>) => {
      ...
      updateFormState({
        data: {
          ...
          step1: {
            ...formState.data.step1,
            [addressKey]: {
              ...formState.data.step1?.[addressKey],
              ...updates,
            },
          },
        },
  ...
  const handleCustomerUpdate = useCallback(
    (updates: Partial<CustomerDetails>) => {
      updateFormState({
        data: {
          ...formState.data,
          step3: {
  ...
  const handleApplyPromoCode = useCallback(async () => {
    ...
    try {
      const validation = pricingEngine.validatePromoCode(
        ...
        pricing.subtotal,
        {
                  serviceType: bookingData.step2?.serviceType,
        distance: bookingData.step1?.distance,
  ...
  return (
    <VStack spacing={6} align="stretch" maxW="4xl" mx="auto">
          ...
          <VStack spacing={3} align="stretch">
            ...
            <HStack justify="space-between">
              ...
              <Text fontWeight="bold" color="blue.600">
                {(bookingData.step1.items?.reduce((sum, item) => sum + (item.volume * item.quantity), 0) || 0).toFixed(1)}m³
              </Text>
            </HStack>
          </VStack>
        </CardBody>
      ...
      <Card>
        ...
        <CardBody>
          <VStack spacing={4} align="stretch">
            {bookingData.step1?.pickupAddress && (
              <EditableAddress
                address={{ ...bookingData.step1.pickupAddress, verified: true }}
                label="Pickup Address"
                onUpdate={updates => handleAddressUpdate('pickup', updates)}
              />
            )}

            {bookingData.step1?.dropoffAddress && (
              <EditableAddress
                address={{ ...bookingData.step1.dropoffAddress, verified: true }}
            ...
            {bookingData.step1?.distance && (
              <HStack justify="space-between" pt={2}>
                ...
                <Text fontWeight="bold" color="blue.600">
                  {bookingData.step1.distance.toFixed(1)} km
                </Text>
              </HStack>
            )}
          </VStack>
      ...
      <Card>
        ...
        <CardBody>
          <VStack spacing={4} align="stretch">
            {bookingData.step2?.schedule?.date && (
              <HStack justify="space-between">
                <Text fontWeight="medium">Date:</Text>
                <Text fontWeight="bold">
                  {format(
                    new Date(bookingData.step2.schedule.date),
                    'EEEE, MMMM d, yyyy'
                  )}
                </Text>
              </HStack>
      ...
      <Card>
        ...
        <CardBody>
          {bookingData.step3?.customer ? (
            <EditableCustomer
              customer={{
                firstName: bookingData.step3.customer.name.split(' ')[0] || '',
                lastName: bookingData.step3.customer.name.split(' ').slice(1).join(' ') || '',
                email: bookingData.step3.customer.email,
                phone: bookingData.step3.customer.phone,
                marketingConsent: false,
                alternativePhone: bookingData.step3.customer.alternativePhone,
                specialRequirements: bookingData.step3.customer.specialInstructions,
              }}
        ...
        <CardBody>
          <VStack spacing={4}>
            ...
            <Text fontSize="sm" color="gray.600" textAlign="center">
              By confirming, you agree to our terms and conditions and privacy
              policy.
            </Text>
          </VStack>
        </CardBody>
