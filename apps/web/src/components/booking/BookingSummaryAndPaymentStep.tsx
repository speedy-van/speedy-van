...
export default function BookingSummaryAndPaymentStep({
  ...
  useEffect(() => {
    console.log(
      'BookingSummaryAndPaymentStep - currentPrice changed to:',
      currentPrice
    );
  }, [currentPrice]);
  ...
  const handleContinue = () => {
    ...
    console.log(
      'BookingSummaryAndPaymentStep - Storing calculatedTotal:',
      currentPrice
    );
    updateBookingData({ calculatedTotal: currentPrice });
    onNext?.();
  ...
  const handlePaymentSubmit = async () => {
    ...
    if (!totalAmount || totalAmount <= 0) {
      toast({
        ...
        description:
          'Please ensure you have a valid total amount before proceeding.',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
              ...
              <VStack align="start" spacing={4}>
                <Box>
                  ...
                  <Text fontSize="sm" color="text.secondary">
                    ...
                    {bookingData.pickupAddress?.postcode || ''}
                  </Text>
                  {bookingData.pickupProperty?.propertyType && (
                    <Text fontSize="sm" color="text.tertiary">
                      Property: {bookingData.pickupProperty.propertyType}{' '}
                      (Floor: {bookingData.pickupProperty.floor || 0})
                    </Text>
                  )}
                </Box>
                <Box>
                  <Text fontWeight="medium" color="neon.500">
                    Dropoff Address:
                  </Text>
                  <Text fontSize="sm" color="text.secondary">
                    {bookingData.dropoffAddress?.line1 || 'Not set'},{' '}
                    {bookingData.dropoffAddress?.city || ''}{' '}
                    {bookingData.dropoffAddress?.postcode || ''}
                  </Text>
                  {bookingData.dropoffProperty?.propertyType && (
                    <Text fontSize="sm" color="text.tertiary">
                      Property: {bookingData.dropoffProperty.propertyType}{' '}
                ...
                <VStack align="start" spacing={3}>
                  {bookingData.items.map((item: any, index: number) => (
                    <HStack key={index} justify="space-between" w="full">
                      ...
                      <HStack spacing={3}>
                        <Badge colorScheme="neon" variant="outline">
                          Qty: {item.quantity}
                        </Badge>
                ...
                <HStack spacing={4} align="start">
                  ...
                  <VStack align="start" spacing={2}>
                    ...
                    <Text fontSize="md" color="text.secondary">
                      ...
                      booking details are correct. I understand that payment
              ...
              >
                ...
                <Box>
                  ...
                  <AlertDescription fontSize="md">
                    The total amount is {formatCurrency(totalAmount)}. Please
                    ensure you have completed the pricing step before proceeding
                    to payment.
                  </AlertDescription>
                </Box>
              </Alert>
