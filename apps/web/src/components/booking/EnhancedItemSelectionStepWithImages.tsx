...
const EnhancedItemSelectionStepWithImages: React.FC<EnhancedItemSelectionStepWithImagesProps> = ({
  bookingData,
  onUpdate,
  onNext,
  onBack,
  ...
  const [selectedImages, setSelectedImages] = useState<Map<string, ImageSelection>>(new Map());
  ...
  const stableOnUpdate = useCallback((updates: any) => {
    onUpdate(updates);
  }, [onUpdate]);
  ...
  const handleSmartNoteChange = useCallback(async (value: string) => {
    ...
    if (value.length < 1) {
      ...
      const popularWords: AutocompleteSuggestion[] = commonMovingWords.slice(0, 6).map(word => ({
        id: `word-${word}`,
        canonicalName: word,
    ...
    try {
      ...
      console.log('📦 Setting catalog suggestions:', uniqueSuggestions.slice(0, 6)); // Debug log
  ...
  const handleSmartNoteKeyDown = useCallback((e: React.KeyboardEvent) => {
  ...
  const handleSmartNoteSelect = useCallback((suggestion: AutocompleteSuggestion) => {
  ...
  const handleImageSelectionChange = useCallback((selections: Map<string, ImageSelection>) => {
  ...
  const handleNext = useCallback(() => {
    ...
    // Validate selections
    const totalItems = Array.from(selectedImages.values()).reduce((sum, selection) => sum + selection.quantity, 0);
    if (totalItems === 0) {
      toast({
  ...
  return (
    <Box p={6} borderWidth="2px" borderRadius="xl" bg="dark.900" borderColor="border.neon" boxShadow="neon.glow" className="booking-step-card" overflow="visible">
      <VStack spacing={6} align="stretch" overflow="visible">
          ...
          <HStack justify="space-between" align="center" mb={4}>
            <VStack align="start" spacing={1}>
              ...
              <Text fontSize="sm" color="text.tertiary" mt={2}>
                💡 Tip: Use the search box below to quickly find items, or browse our visual catalog. 
                Don't worry about being exact - we'll help you find the right items and calculate accurate pricing.
              </Text>
            </VStack>
            
            <HStack spacing={3}>
              <Badge colorScheme="neon" variant="solid" fontSize="md" px={3} py={1}>
        ...
        <Box 
          ...
          _before={{
            ...
            bottom: 0,
            borderRadius: "xl",
            background: "linear-gradient(135deg, rgba(0,194,255,0.05) 0%, rgba(0,209,143,0.05) 100%)",
            pointerEvents: "none"
          }}
        >
          <VStack spacing={5} align="stretch" overflow="visible" position="relative" zIndex={1}>
            <Box textAlign="center">
              ...
              <Text fontSize="md" color="text.secondary" mb={4}>
                Type what you're moving and we'll instantly suggest items from our catalog
              </Text>
            </Box>
            
            <Box position="relative" className="smart-note-container" overflow="visible">
              <InputGroup>
                <Input
                  value={smartNote}
              ...
              <VStack spacing={3} mt={4}>
                <Text fontSize="sm" color="neon.400" fontWeight="medium" textAlign="center">
                ...
                <Text fontSize="xs" color="text.tertiary" textAlign="center">
                  Popular items appear automatically when you focus the search box
                </Text>
              </VStack>
                ...
                >
                  {smartNoteSuggestions.map((suggestion, index) => (
                    <Box
                      ...
                      borderBottom={index < smartNoteSuggestions.length - 1 ? "1px solid" : "none"}
                      borderColor="border.primary"
                      bg={activeSuggestionIndex === index ? "dark.600" : "transparent"}
                      transition="all 0.2s"
                    >
                      <HStack justify="space-between">
                        <VStack align="start" spacing={0}>
                          ...
                          {suggestion.category && suggestion.category !== 'common word' && (
                        ...
                        <HStack spacing={2}>
                          {suggestion.category === 'common word' ? (
                            ...
                            <Badge colorScheme="neon" variant="subtle" fontSize="xs">
                              📦 Item
                            </Badge>
                          )}
            ...
            <VStack align="start" spacing={3}>
              ...
              <HStack spacing={4} wrap="wrap">
                ...
                <Badge colorScheme="brand" variant="subtle">
                  {Array.from(selectedImages.values()).reduce((sum, selection) => sum + selection.quantity, 0)} total items
                </Badge>
                <Badge colorScheme="purple" variant="subtle">
                  {Array.from(selectedImages.values()).filter(selection => selection.customNotes).length} with notes
                </Badge>
              </HStack>
              
              <Text fontSize="sm" color="text.secondary">
                Your selections have been saved and will be used for pricing calculations.
              </Text>
            </VStack>
          </Box>
        )}
        ...
        <ModalContent bg="dark.900" borderColor="border.neon" borderWidth="1px">
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
              <Box>
                <Text fontWeight="semibold" mb={2} color="text.primary">Visual Selection</Text>
                <List spacing={2}>
                  <ListItem color="text.secondary">• Browse items by category using the filter buttons</ListItem>
                  <ListItem color="text.secondary">• Search for specific items using the search bar</ListItem>
                  <ListItem color="text.secondary">• Click on items to view details and add to selection</ListItem>
                  <ListItem color="text.secondary">• Adjust quantities using the number inputs</ListItem>
                  <ListItem color="text.secondary">• Add custom notes for special handling instructions</ListItem>
                </List>
              </Box>
              
              <Box>
                <Text fontWeight="semibold" mb={2} color="text.primary">Quick Search</Text>
                <List spacing={2}>
                  <ListItem color="text.secondary">• Type item names to see autocomplete suggestions</ListItem>
                  <ListItem color="text.secondary">• Select from the dropdown suggestions</ListItem>
                  <ListItem color="text.secondary">• Items are automatically added to your selection</ListItem>
                  <ListItem color="text.secondary">• Use natural language (e.g., "large sofa", "6 boxes")</ListItem>
                </List>
              </Box>
            </VStack>
          </ModalBody>
          <ModalFooter>
            <Button colorScheme="neon" onClick={onHelpClose}>Got it!</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </Box>
