...
export default function WeatherMoodHero() {
  ...
  React.useEffect(() => {
    const fetchByCoords = async (lat?: number, lon?: number) => {
      ...
      const res = await fetch(`/api/weather/current${qs}`, { cache: "no-store" });
      if (!res.ok) {
  ...
  const getTextColor = () => {
    ...
    if (isNight || tone === "Rain" || tone === "Drizzle" || tone === "Thunderstorm") {
      return "white";
    }
  ...
  const secondaryTextColor = textColor === "white" ? "whiteAlpha.850" : "gray.600";

  const line =
    wx ? messages(wx)[msgIndex % messages(wx).length] : "Checking local weather…";
          ...
          <Stack spacing={6} align="flex-start" position="relative" zIndex={1}>
            ...
            >
              <Button
                ...
                _hover={{
                  bg: "#00C5E6",
                  boxShadow: "0 0 20px rgba(0, 224, 255, 0.6), 0 0 40px rgba(0, 224, 255, 0.3)",
