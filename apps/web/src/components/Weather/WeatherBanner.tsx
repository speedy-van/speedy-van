...
export function WeatherBanner({ f }: { f?: Forecast | null }) {
  ...
  return (
    ...
    >
      {f.theme === 'sunny' && (
        <Box position="absolute" right="-10%" top="-20%" w="60%" h="60%" bg="radial-gradient(circle at 30% 30%, rgba(255,255,255,0.7), transparent 60%)" filter="blur(8px)" />
      )}
      {f.theme === 'rain' && (
        <Box as="span" className="rain-overlay" />
      )}

      <Flex align="flex-end" justify="space-between">
        <Box>
          <Text fontSize="lg" fontWeight="semibold">{f.desc}</Text>
          <HStack spacing={4} mt={2} fontSize="sm">
            {f.temp != null && <Text>{Math.round(f.temp)}°C</Text>}
            {f.pop != null && <Text>Rain: {Math.round((f.pop) * 100)}%</Text>}
            {f.windGust != null && <Text>Gusts: {Math.round(f.windGust)} km/h</Text>}
            {f.uvi != null && <Text>UV: {Math.round(f.uvi)}</Text>}
          </HStack>
          <Text fontSize="xs" color="text.muted" mt={1}>Forecast for selected time. May change.</Text>
        </Box>
        {f.temp != null && (
          <Text fontSize="48px" fontWeight="bold">{Math.round(f.temp)}°</Text>
        )}
      </Flex>
    </MotionBox>
  );
