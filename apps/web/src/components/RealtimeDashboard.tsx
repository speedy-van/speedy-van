...
const RealtimeDashboard: React.FC = () => {
  ...
  const [selectedDriverId, setSelectedDriverId] = useState<string>('driver-456');
  ...
  const getConnectionStatus = () => {
    switch (connectionState) {
      ...
      case 'connecting':
        return { icon: FiRefreshCw, color: 'yellow.500', text: 'Connecting...' };
      case 'reconnecting':
        return { icon: FiRefreshCw, color: 'orange.500', text: 'Reconnecting...' };
      case 'disconnected':
  ...
  const handleCacheOperations = () => {
    ...
    cacheUtils.invalidate(`orders-${selectedOrderId}:${CHANNEL_EVENTS.ORDER_UPDATED}`);
      ...
      <Box mb={6}>
        <HStack justify="space-between" align="center">
          <VStack align="start" spacing={1}>
            ...
            <Text color="gray.600">
              Live updates with caching, error handling, and performance monitoring
            </Text>
          </VStack>
              ...
              {orderState.isLoading ? (
                ...
                <VStack align="start" spacing={4}>
                  <HStack justify="space-between" w="full">
                    ...
                    <Badge colorScheme="blue">{orderState.connectionState}</Badge>
                  </HStack>
                  ...
                  {orderState.data && (
                    <VStack align="start" spacing={2} w="full">
                      <HStack justify="space-between" w="full">
                        ...
                        <Badge colorScheme="green">{orderState.data.status}</Badge>
                      </HStack>
                      <HStack justify="space-between" w="full">
                        <Text fontSize="sm">Customer:</Text>
                        <Text fontSize="sm">{orderState.data.customerName}</Text>
                      </HStack>
                      <HStack justify="space-between" w="full">
                      ...
                      {orderState.lastUpdated && (
                        <HStack justify="space-between" w="full">
                          ...
                          <Text fontSize="sm" color="gray.500">
                            {new Date(orderState.lastUpdated).toLocaleTimeString()}
                          </Text>
                        </HStack>
                      )}
                    </VStack>
          ...
          <Card bg={cardBg} border="1px" borderColor={borderColor}>
            <CardHeader>
              <HStack justify="space-between">
                ...
                <Badge
                  colorScheme={driverState.connectionState === 'connected' ? 'green' : 'red'}
                  variant="subtle"
                >
                ...
                <VStack align="start" spacing={4}>
                  <HStack justify="space-between" w="full">
                    ...
                    <Box
                      ...
                      color={driverState.data?.isOnline ? 'green.500' : 'red.500'}
                      boxSize={4}
                    />
                  </HStack>
                  ...
                  {driverState.data && (
                    <VStack align="start" spacing={2} w="full">
                      <HStack justify="space-between" w="full">
                        <Text fontSize="sm">Status:</Text>
                        <Badge colorScheme={driverState.data.isOnline ? 'green' : 'red'}>
                          {driverState.data.status}
                        </Badge>
                      </HStack>
                      <HStack justify="space-between" w="full">
                        <Text fontSize="sm">Rating:</Text>
                        <Text fontSize="sm">⭐ {driverState.data.rating}/5</Text>
                      </HStack>
                      <HStack justify="space-between" w="full">
          ...
          <Card bg={cardBg} border="1px" borderColor={borderColor}>
            ...
            <CardBody>
              <VStack align="start" spacing={4}>
                ...
                <VStack align="start" spacing={2} w="full">
                  <Text fontSize="sm" fontWeight="medium">Recent Job Offers</Text>
          ...
          <Card bg={cardBg} border="1px" borderColor={borderColor}>
            ...
            <CardBody>
              <VStack align="start" spacing={4}>
                ...
                <VStack align="start" spacing={2} w="full">
                  <Text fontSize="sm" fontWeight="medium">Recent Activity</Text>
                  <List spacing={2} w="full">
                    <ListItem>
                      <HStack>
