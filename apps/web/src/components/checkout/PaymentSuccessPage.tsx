...
export default function PaymentSuccessPage({ bookingData }: PaymentSuccessPageProps) {
  const router = useRouter();
  const { data: session, status } = useSession();
  const [showAccountCreation, setShowAccountCreation] = useState(false);
  const [accountCreated, setAccountCreated] = useState(false);
  const [currentStep, setCurrentStep] = useState<'success' | 'account-creation' | 'account-created'>('success');
  ...
  const handleSkipAccount = () => {
    ...
    toast({
      ...
      description: 'You can create an account later to view your booking history.',
      status: 'info',
      duration: 4000,
    });
  };
              ...
              <VStack spacing={4} align="stretch">
                ...
                {bookings && (
                  <Alert status="info" borderRadius="md">
                    <AlertIcon />
                    <Text>
                      We found {bookings.totalCount} booking{bookings.totalCount !== 1 ? 's' : ''} linked to your account!
                    </Text>
                  </Alert>
                )}
    ...
    <Box maxW="4xl" mx="auto" py={8}>
      <VStack spacing={8}>
            ...
            <VStack spacing={6} align="stretch">
              ...
              <Box>
                ...
                <VStack align="start" spacing={2} pl={4}>
                  <HStack>
                    ...
                    <Text><strong>Name:</strong> {bookingData.customer.name}</Text>
                  </HStack>
                  <HStack>
                    <Icon as={FaEnvelope} color="gray.500" boxSize={4} />
                    <Text><strong>Email:</strong> {bookingData.customer.email}</Text>
                  </HStack>
                  <HStack>
                    <Icon as={FaPhone} color="gray.500" boxSize={4} />
                    <Text><strong>Phone:</strong> {bookingData.customer.phone}</Text>
                  </HStack>
                </VStack>
              </Box>
              ...
              <Box>
                ...
                <Text pl={4} color="gray.600">
                  {bookingData.addresses.pickup.city}, {bookingData.addresses.pickup.postcode}
                </Text>
              </Box>
              ...
              <Box>
                ...
                <Text pl={4} color="gray.600">
                  {bookingData.addresses.dropoff.city}, {bookingData.addresses.dropoff.postcode}
                </Text>
              </Box>
              ...
              <Box>
                ...
                <VStack align="start" spacing={2} pl={4}>
                  <Text><strong>Date:</strong> {formatDate(bookingData.schedule.date)}</Text>
                  <Text><strong>Time:</strong> {formatTime(bookingData.schedule.timeSlot)}</Text>
                </VStack>
              </Box>
        ...
        <Card w="full" borderColor="blue.200" borderWidth="2px">
          <CardHeader>
            <Heading size="md" color="blue.600">What Happens Next?</Heading>
          </CardHeader>
          <CardBody>
            <VStack spacing={4} align="stretch">
              <Text>
                Our team will review your booking and confirm the details. You'll receive:
              </Text>
              <VStack align="start" spacing={2} pl={4}>
        ...
        <Card w="full" borderColor="orange.200" borderWidth="2px">
          ...
          <CardBody>
            <VStack spacing={4} align="stretch">
              <Text>
                Create a free account to track your booking, view your move history, and manage future bookings all in one place.
              </Text>
              ...
              <Box bg="orange.50" p={4} borderRadius="md">
                ...
                <VStack align="start" spacing={1} fontSize="xs" color="orange.600">
