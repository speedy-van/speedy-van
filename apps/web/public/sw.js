...
async function doDriverStatusSync() {
  try {
    ...
    const pendingActions = await getPendingActions();
    const driverActions = pendingActions.filter(action => 
      action.type === 'location_update' || action.type === 'availability_update'
    );
    ...
    for (const action of driverActions) {
      try {
        ...
        console.error('Driver status sync failed for action:', action.id, error);
      }
    }
  } catch (error) {
...
async function doJobStatusSync() {
  try {
    ...
    const pendingActions = await getPendingActions();
    const jobActions = pendingActions.filter(action => 
      action.type === 'job_progress' || action.type === 'job_claim' || action.type === 'job_decline'
    );
