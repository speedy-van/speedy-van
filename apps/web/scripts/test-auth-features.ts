...
async function testAuthFeatures() {
  ...
  try {
    ...
    const oldPasswordValid = await bcrypt.compare(resetTestPassword, updatedResetUser.password);
    const newPasswordValid = await bcrypt.compare(newPassword, updatedResetUser.password);
    ...
    console.log(`✅ Generated verification token: ${verifyToken.substring(0, 16)}...`);
    ...
    if (!updatedVerifyUser) throw new Error('User not found after verification');
    
    if (!updatedVerifyUser.emailVerified) throw new Error('Email not marked as verified');
    if (updatedVerifyUser.emailVerificationToken || updatedVerifyUser.emailVerificationExpiry) {
    ...
    if (expiredResetUser) throw new Error('Expired reset token should not be found');
    ...
    if (expiredVerifyUser) throw new Error('Expired verification token should not be found');
    ...
    if (caseSensitiveUser) throw new Error('Case-sensitive token should not be found');
    ...
    console.log('\n📋 Test Summary:');
    console.log('- ✅ Password reset flow (token generation, password change, token cleanup)');
    console.log('- ✅ Email verification flow (token generation, verification, token cleanup)');
