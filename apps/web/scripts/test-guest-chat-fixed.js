...
async function testGuestChatFixed() {
  try {
    ...
    const guestChatResponse = await fetch('http://localhost:3000/api/chat/guest', {
      method: 'POST',
      headers: {
    ...
    if (!guestChatResponse.ok) {
      ...
      throw new Error(`Guest chat creation failed: ${guestChatResponse.status} - ${errorText}`);
    }
    ...
    const messagesResponse = await fetch(`http://localhost:3000/api/chat/sessions/${guestChatData.sessionId}/messages?guestEmail=${encodeURIComponent('test-fixed@example.com')}`);
    
    if (!messagesResponse.ok) {
      const errorText = await messagesResponse.text();
      throw new Error(`Message fetch failed: ${messagesResponse.status} - ${errorText}`);
    }
    ...
    console.log(`   Message metadata: ${JSON.stringify(messages[0]?.metadata)}\n`);

    // Test 3: Send a message as guest
    console.log('3. Sending message as guest...');
    const sendMessageResponse = await fetch(`http://localhost:3000/api/chat/sessions/${guestChatData.sessionId}/messages`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!sendMessageResponse.ok) {
      ...
      throw new Error(`Send message failed: ${sendMessageResponse.status} - ${errorText}`);
    }
    ...
    console.log(`   Message metadata: ${JSON.stringify(sentMessage.metadata)}\n`);

    // Test 4: Fetch messages again to see the new message
    console.log('4. Fetching messages again to verify new message...');
    const messagesResponse2 = await fetch(`http://localhost:3000/api/chat/sessions/${guestChatData.sessionId}/messages?guestEmail=${encodeURIComponent('test-fixed@example.com')}`);
    
    if (!messagesResponse2.ok) {
      const errorText = await messagesResponse2.text();
      throw new Error(`Message fetch failed: ${messagesResponse2.status} - ${errorText}`);
    }

    const messages2 = await messagesResponse2.json();
    console.log('✅ Messages fetched successfully');
    console.log(`   Updated message count: ${messages2.length}`);
    console.log(`   Latest message: ${messages2[messages2.length - 1]?.content}\n`);

    console.log('🎉 All tests passed! Guest chat functionality is working correctly with fixes.');
