...
async function testSystemMonitoring() {
  ...
  try {
    ...
    console.log(`✅ Health checks completed: ${healthResults.size} services checked`);
    ...
    console.log(`✅ System metrics retrieved: ${metrics.performance.requestsPerSecond} req/s`);
...
async function testLogExport() {
  ...
  try {
    ...
    console.log(`✅ JSON export: ${jsonExport.recordCount} records, ${jsonExport.filename}`);
    ...
    console.log(`✅ CSV export: ${csvExport.recordCount} records, ${csvExport.filename}`);
...
async function testDatabaseQueries() {
  ...
  try {
    ...
    console.log(`✅ Security logs query: ${securityLogs.length} security events found`);
    ...
    console.log(`✅ Recent logs query: ${recentLogs.length} logs in last 24 hours`);
...
async function testAPIEndpoints() {
  ...
  try {
    ...
    // Test logs endpoint
    const logsResponse = await fetch(`${baseUrl}/api/admin/logs?type=audit&limit=5`);
    if (logsResponse.ok) {
    ...
    if (auditResponse.ok) {
      ...
      console.log(`✅ Audit API: ${auditData.items.length} audit entries retrieved`);
    } else {
