...
async function testTelemetryEndpoints(): Promise<TestResult[]> {
  ...
  try {
    const analyticsResponse = await fetch(`${baseUrl}/api/telemetry/analytics`, {
      method: 'POST',
  ...
  try {
    const performanceResponse = await fetch(`${baseUrl}/api/telemetry/performance`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function testDatabaseSchema(): Promise<TestResult[]> {
  ...
  try {
    ...
    const expectedTables = ['telemetry_events', 'performance_metrics', 'business_metrics'];
  ...
  try {
    ...
    const requiredColumns = ['id', 'event', 'properties', 'user_id', 'session_id', 'timestamp'];
...
async function main() {
  try {
    ...
    const allResults = [...endpointResults, ...schemaResults, ...metricsResults];
    ...
    if (successRate >= 80) {
      ...
      console.log('⚠️ Some telemetry tests failed. Please check the implementation.');
      process.exit(1);
    }
