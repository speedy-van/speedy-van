...
async function checkMiddlewareManifest() {
  ...
  try {
    ...
    console.log("⚠️  Could not read middleware-manifest.json:", error.message);
  }
}
...
async function scanCompiledFiles() {
  ...
  try {
    if (!(await fs.access(appDir).then(() => true).catch(() => false))) {
    ...
    console.log("⚠️  Error scanning compiled files:", error.message);
  }
}
