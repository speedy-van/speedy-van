...
class ReleaseDeployer {
      ...
      for (let i = this.currentStep; i >= 0; i--) {
        ...
        if (step.rollbackCommand) {
          ...
          try {
            ...
            console.error(`❌ Rollback step failed: ${step.name}`, rollbackError);
  ...
  private getFeatureFlagCommand(): string {
    ...
    const flagCommands = this.config.featureFlags.map(flag => 
  ...
  private async recordDeploymentStart(): Promise<void> {
    console.log(`🚀 Starting deployment ${this.deploymentId} for ${this.config.environment} v${this.config.version}`);
  ...
  private async recordDeploymentFailure(error: any): Promise<void> {
    console.error(`❌ Deployment ${this.deploymentId} failed:`, error instanceof Error ? error.message : 'Unknown error');
    // Deployment logging not implemented yet
  }

  private async recordStepSuccess(step: DeploymentStep, duration: number): Promise<void> {
    console.log(`✅ Step '${step.name}' completed in ${duration}ms`);
    // Deployment logging not implemented yet
  }

  private async recordStepFailure(step: DeploymentStep, error: any, duration: number): Promise<void> {
    console.error(`❌ Step '${step.name}' failed after ${duration}ms:`, error instanceof Error ? error.message : 'Unknown error');
    // Deployment logging not implemented yet
  }
