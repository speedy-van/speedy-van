...
function detectParentChildRelationships(places: RawPlaceData[]): Map<string, string> {
...
function validateAndTransform(places: any[], parentMap: Map<string, string>): UkPlace[] {
  const transformed: UkPlace[] = [];
  
  for (const place of places) {
    // Validate coordinates
    if (place.lat < -90 || place.lat > 90 || place.lon < -180 || place.lon > 180) {
      console.warn(`Invalid coordinates for ${place.name}: ${place.lat}, ${place.lon}`);
      continue;
    }
    
    // Validate place type
    const validTypes = ["city", "town", "village", "borough", "district", "neighbourhood"];
    ...
    const slug = place.slug || place.name
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, '')
      .replace(/\s+/g, '-')
      .trim();
...
async function main() {
  try {
    ...
    console.log(`🎯 Places generation complete! Generated ${transformedPlaces.length} place records`);
    
    // Summary statistics
    const typeCounts = transformedPlaces.reduce((acc, p) => {
