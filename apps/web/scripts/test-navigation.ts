...
async function testNavigation() {
  ...
  try {
    ...
    if (assignments.length === 0) {
      console.log("❌ No active assignments found. Please create a test assignment first.");
      return;
    }
    ...
    if (assignment.Booking.pickupAddress.lat && assignment.Booking.pickupAddress.lng && 
        assignment.Booking.dropoffAddress.lat && assignment.Booking.dropoffAddress.lng) {
      console.log("\n✅ Coordinates available for navigation:");
      console.log(`   Pickup: ${assignment.Booking.pickupAddress.lat}, ${assignment.Booking.pickupAddress.lng}`);
      console.log(`   Dropoff: ${assignment.Booking.dropoffAddress.lat}, ${assignment.Booking.dropoffAddress.lng}`);
      console.log(`   Scheduled: ${assignment.Booking.scheduledAt}`);
      console.log(`   Duration: ${assignment.Booking.estimatedDurationMinutes} minutes`);
    } else {
    ...
    if (assignment.Driver.availability) {
      ...
      console.log(`   Location Consent: ${assignment.Driver.availability.locationConsent}`);
      console.log(`   Last Seen: ${assignment.Driver.availability.lastSeenAt}`);
      
      if (assignment.Driver.availability.lastLat && assignment.Driver.availability.lastLng) {
        console.log(`   Last Location: ${assignment.Driver.availability.lastLat}, ${assignment.Driver.availability.lastLng}`);
      }
    } else {
    ...
    events.forEach((event, index) => {
      console.log(`   ${index + 1}. ${event.step} - ${event.createdAt.toISOString()}`);
    });
