...
function validateKeyFormat(key: string, expectedPrefix: string, keyName: string): boolean {
  if (!key) {
  ...
  if (!key.startsWith(expectedPrefix)) {
    ...
    console.error(`   Expected: ${expectedPrefix}..., got: ${key.substring(0, 7)}...`);
    return false;
  }
...
async function main() {
  ...
  if (!secretKeyValid || !publishableKeyValid || !webhookSecretValid || !baseUrlValid) {
  ...
  if (!connectivityTest) {
    console.error('❌ Stripe connectivity test failed');
    console.error('   Please check your network connection and API key validity');
    process.exit(1);
  }
