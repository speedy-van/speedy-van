...
import { offlineManager, queueJobProgress, queueLocationUpdate, queueAvailabilityUpdate, queueJobClaim } from '../src/lib/offline';
...
async function testOfflineFunctionality() {
  ...
  try {
    ...
    const jobProgressId = await queueJobProgress('test-job-123', 'arrived_pickup', {
    ...
    const locationActions = offlineManager.getPendingActionsByType('location_update');
    const availabilityActions = offlineManager.getPendingActionsByType('availability_update');
    ...
    console.log(`📊 Remaining pending actions: ${newState.pendingActions.length}`);
    ...
    console.log(`📊 Final pending actions: ${finalState.pendingActions.length}`);
    ...
    console.log(`📊 Actions after clearing: ${clearedState.pendingActions.length}`);
...
async function testConflictResolution() {
  ...
  try {
    ...
    await queueJobProgress(jobId, 'arrived_pickup', { notes: 'Second arrival' });
    ...
    jobActions.forEach((action, index) => {
      ...
      console.log(`  ${index + 1}. ${payload.step}: ${payload.payload?.notes || 'No notes'}`);
    });
