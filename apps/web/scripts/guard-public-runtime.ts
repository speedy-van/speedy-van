...
interface Violation {
  file: string;
  line: number;
  type: "edge-runtime" | "force-dynamic" | "no-store-cache" | "preferred-region" | "next-server-import" | "next-navigation-import";
  content: string;
}
...
async function scanFile(filePath: string): Promise<Violation[]> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes("export const dynamic") && line.includes("force-dynamic")) {
      violations.push({
        file: filePath,
        line: lineNum,
        type: "force-dynamic",
    ...
    if (line.includes("export const preferredRegion") || line.includes("export const regions")) {
      violations.push({
        file: filePath,
        line: lineNum,
...
async function scanDirectory(dir: string, excludePatterns: string[]): Promise<Violation[]> {
  const violations: Violation[] = [];
...
async function main() {
  ...
  console.error("   - Using next: { revalidate: 86400 } instead of cache: 'no-store'");
  console.error("   - Removing preferredRegion/regions exports from pages/layouts");
  console.error("   - Moving next/server and next/navigation imports to client components or API routes");
  console.error("");
