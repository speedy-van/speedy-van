...
test.describe('Admin Dashboard Performance Tests', () => {
  ...
  test('orders list interactions should be under 100ms', async ({ page }) => {
    ...
    console.log(`Filter time: ${filterTime}ms, Sort time: ${sortTime}ms, Search time: ${searchTime}ms`);
  });
  ...
  test('real-time updates should be under 100ms perceived', async ({ page }) => {
    await page.goto('/admin/orders');
    await expect(page.locator('[data-testid="orders-table"]')).toBeVisible();
  ...
  test('dashboard KPIs should be live and accurate', async ({ page }) => {
    ...
    const avgEtaText = await page.locator('[data-testid="avg-eta"]').textContent();
    expect(avgEtaText).toMatch(/\d+\s*min/);
    
    const responseTimeText = await page.locator('[data-testid="first-response-time"]').textContent();
    expect(responseTimeText).toMatch(/\d+\.\d+\s*min/);
    
    const incidentsText = await page.locator('[data-testid="open-incidents"]').textContent();
  ...
  test('admin shell should be responsive and fast', async ({ page }) => {
    ...
    console.log(`Navigation: ${navTime}ms, Search: ${searchTime}ms, Shortcut: ${shortcutTime}ms`);
  });

  test('should handle concurrent operations without conflicts', async ({ page, context }) => {
    ...
    if (page1 && page2) {
      ...
      await page1.locator('[data-testid="email-input"]').fill('admin@speedy-van.co.uk');
      await page1.locator('[data-testid="password-input"]').fill('AdminPassword123!');
      await page1.locator('[data-testid="login-button"]').click();
      
      await page2.goto('/admin/login');
      await page2.locator('[data-testid="email-input"]').fill('admin@speedy-van.co.uk');
      await page2.locator('[data-testid="password-input"]').fill('AdminPassword123!');
      ...
      const successCount = await page1.locator('[data-testid="edit-success"]').count() + 
                          await page2.locator('[data-testid="edit-success"]').count();
      const conflictCount = await page1.locator('[data-testid="edit-conflict"]').count() + 
