...
test.describe('Driver Header Visibility', () => {
  test('should show driver header after successful login', async ({ page }) => {
    ...
    await expect(page.locator('header')).toContainText('test-driver@speedy-van.co.uk');
  ...
  test('should maintain header visibility when navigating between driver pages', async ({ page }) => {
    // Login first
    await page.goto('/driver/login');
    await page.fill('[data-testid="email-input"]', 'test-driver@speedy-van.co.uk');
    await page.fill('[data-testid="password-input"]', 'TestPassword123!');
  ...
  test('should show proper header styling and layout', async ({ page }) => {
    ...
    await expect(header).toHaveCSS('border-bottom', '1px solid rgb(237, 242, 247)'); // gray border
