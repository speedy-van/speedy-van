...
test.describe('Customer Portal Observability & QA', () => {
  ...
  test('Deep-link to track page while signed out → login → lands on track page', async ({ page }) => {
  ...
  test('Unauthorized role (driver/admin) cannot access customer portal', async ({ page }) => {
    // Login as driver
  ...
  test('Skeleton loading states are shown while fetching', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="orders-skeleton"]')).not.toBeVisible();
  });
});
