...
test.describe('Admin Dashboard QA & Acceptance', () => {
  ...
  test('should load dashboard with KPIs under 1 second', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="first-response-time"]')).toBeVisible();
    ...
    const revenueText = await page.locator('[data-testid="today-revenue"]').textContent();
    expect(revenueText).toMatch(/£[\d,]+\.\d{2}/);
    
    const activeJobsText = await page.locator('[data-testid="active-jobs"]').textContent();
    expect(parseInt(activeJobsText || '0')).toBeGreaterThanOrEqual(0);
  });

  test('should filter 10k orders and bulk assign with atomic success', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="orders-table"] tbody tr')).toBeVisible();
    ...
    const assignedOrders = await page.locator('[data-testid="order-status-assigned"]').count();
    expect(assignedOrders).toBeGreaterThan(0);
    
    // Verify audit trail
    await page.click('[data-testid="audit-log-button"]');
    await expect(page.locator('[data-testid="bulk-assignment-audit"]')).toBeVisible();
  });

  test('should approve driver and verify they can claim jobs', async ({ page }) => {
    ...
    const driverEmail = await pendingApp.locator('[data-testid="driver-email"]').textContent();
    ...
    await page.locator('[data-testid="approval-notes"]').fill('All documents verified');
    ...
    if (driverPage && driverEmail) {
      ...
      await driverPage.locator('[data-testid="password-input"]').fill('TestPassword123!');
      ...
      await expect(driverPage.locator('[data-testid="available-jobs-list"]')).toBeVisible();
      
      // Verify driver can see and claim jobs
      const jobCard = driverPage.locator('[data-testid="job-card"]').first();
      await expect(jobCard).toBeVisible();
      await expect(jobCard.locator('[data-testid="claim-job-button"]')).toBeVisible();
      
      await driverContext?.close();
    }
  });

  test('should suspend driver and verify they cannot claim jobs', async ({ page }) => {
    ...
    const driverEmail = await activeDriver.locator('[data-testid="driver-email"]').textContent();
    
    // Suspend the driver
    await activeDriver.locator('[data-testid="driver-actions"]').click();
    await page.locator('[data-testid="suspend-driver"]').click();
    await page.locator('[data-testid="suspension-reason"]').fill('Test suspension');
    await page.locator('[data-testid="confirm-suspension"]').click();
    
    // Verify suspension success
    await expect(page.locator('[data-testid="suspension-success"]')).toBeVisible();
    ...
    if (driverPage && driverEmail) {
      ...
      await expect(driverPage.locator('[data-testid="account-suspended"]')).toBeVisible();
      
      // Try to access jobs (should be blocked)
      await driverPage.goto('/driver/jobs/available');
      await expect(driverPage.locator('[data-testid="access-denied"]')).toBeVisible();
      
      await driverContext?.close();
    }
  });

  test('should issue partial refund and verify Stripe webhook sync', async ({ page }) => {
    ...
    const completedOrder = page.locator('[data-testid="completed-order"]').first();
    const orderRef = await completedOrder.locator('[data-testid="order-reference"]').textContent();
    const originalAmount = await completedOrder.locator('[data-testid="order-amount"]').textContent();
    
    // Issue partial refund
    await completedOrder.locator('[data-testid="issue-refund"]').click();
    await page.locator('[data-testid="refund-amount"]').fill('25.00');
    await page.locator('[data-testid="refund-reason"]').selectOption('customer_request');
    await page.locator('[data-testid="refund-notes"]').fill('Partial refund for damaged item');
    ...
    await expect(page.locator('[data-testid="finance-dashboard"]')).toBeVisible();
    
    // Check refund appears in finance view
    const refundEntry = page.locator(`[data-testid="refund-entry"][data-order="${orderRef}"]`);
    await expect(refundEntry).toBeVisible();
    
    // Verify amount reconciliation
    const refundAmount = await refundEntry.locator('[data-testid="refund-amount"]').textContent();
    expect(refundAmount).toBe('£25.00');
    
    // Verify Stripe integration
    await page.click('[data-testid="stripe-sync-status"]');
    await expect(page.locator('[data-testid="webhook-sync-success"]')).toBeVisible();
  });
  ...
  test('should handle admin role-based access control', async ({ page }) => {
    ...
    await page.goto('/admin/drivers/applications');
    await expect(page.locator('[data-testid="approve-application"]')).toBeVisible();
  ...
  test('should verify real-time updates and performance', async ({ page }) => {
    ...
    const initialStatus = await orderRow.locator('[data-testid="order-status"]').textContent();
  ...
  test('should verify audit trail and compliance', async ({ page }) => {
    ...
    await expect(auditEntry.locator('[data-testid="audit-action"]')).toContainText('order.edit');
    await expect(auditEntry.locator('[data-testid="audit-user"]')).toContainText('admin@speedy-van.co.uk');
