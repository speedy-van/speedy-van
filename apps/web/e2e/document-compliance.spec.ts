...
test.describe('Document Compliance & Blocking', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('[data-testid="email-input"]', 'test-driver@speedy-van.co.uk');
  ...
  test('should block job claiming with expired documents', async ({ page }) => {
    ...
    if (expiredCount > 0) {
      ...
      await expect(page.locator('[data-testid="document-blocking-banner"]')).toBeVisible();
      await expect(page.locator('[data-testid="document-blocking-banner"]')).toContainText('Expired documents');
      
      // Claim button should be disabled
      await expect(page.locator('[data-testid="claim-job-button"]').first()).toBeDisabled();
      ...
      await page.goto('/driver/jobs/available');
      await expect(page.locator('[data-testid="claim-job-button"]').first()).toBeEnabled();
    }
  });
  ...
  test('should show document expiry warnings', async ({ page }) => {
    ...
    const expiringSoonDocs = page.locator('[data-testid="expiring-soon-document"]');
    const expiringCount = await expiringSoonDocs.count();
    
    if (expiringCount > 0) {
      // Should show warning banner
      await expect(page.locator('[data-testid="document-warning-banner"]')).toBeVisible();
      await expect(page.locator('[data-testid="document-warning-banner"]')).toContainText('expiring soon');
    }
  });
  ...
  test('should handle document review status', async ({ page }) => {
    ...
    if (pendingCount > 0) {
      ...
      await expect(page.locator('[data-testid="pending-status-badge"]')).toBeVisible();
      await expect(page.locator('[data-testid="pending-status-badge"]')).toContainText('Pending Review');
    }
  });
  ...
  test('should show document audit trail', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="document-audit-trail"]')).toBeVisible();
  ...
  test('should handle document rejection', async ({ page }) => {
    ...
    if (rejectedCount > 0) {
      ...
      await expect(page.locator('[data-testid="resubmit-document-button"]')).toBeVisible();
  ...
  test('should block access with missing required documents', async ({ page }) => {
    // Navigate to documents page
    await page.goto('/driver/documents');
    ...
    if (missingCount > 0) {
      ...
      await expect(page.locator('[data-testid="missing-documents-block"]')).toBeVisible();
      await expect(page.locator('[data-testid="missing-documents-block"]')).toContainText('Required documents missing');
    }
  });
  ...
  test('should send document expiry notifications', async ({ page }) => {
    ...
    const docNotifications = page.locator('[data-testid="document-expiry-notification"]');
    const notificationCount = await docNotifications.count();
    
    if (notificationCount > 0) {
      // Should show notification about expiring documents
      await expect(page.locator('[data-testid="document-expiry-notification"]').first()).toBeVisible();
      await expect(page.locator('[data-testid="document-expiry-notification"]').first()).toContainText('expiring');
      
      // Click notification should navigate to documents
      await page.locator('[data-testid="document-expiry-notification"]').first().click();
      await expect(page).toHaveURL('/driver/documents');
    }
  });
});
