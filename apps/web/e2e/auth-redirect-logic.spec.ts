...
test.describe('Auth Redirect Logic (Deep-linking)', () => {
  ...
  test('should redirect to protected route after sign in from home', async ({ page }) => {
    // Try to access a protected route (driver dashboard)
    await page.goto('/driver/dashboard');
    
    // Should be redirected to home with auth parameters
    await expect(page).toHaveURL(/\/\?returnTo=%2Fdriver%2Fdashboard&showAuth=true/);
  ...
  test('should redirect to admin route after sign in with admin credentials', async ({ page }) => {
  ...
  test('should redirect to customer portal after sign in with customer credentials', async ({ page }) => {
    // Try to access customer portal
    await page.goto('/customer-portal');
    
    // Should be redirected to home with auth parameters
    await expect(page).toHaveURL(/\/\?returnTo=%2Fcustomer-portal&showAuth=true/);
  ...
  test('should stay on home and scroll to book section when no returnTo', async ({ page }) => {
  ...
  test('should handle sign up flow with redirects', async ({ page }) => {
    ...
    await page.fill('[data-testid="signup-confirm-password"]', 'NewPassword123!');
    await page.click('button[type="submit"]');
  ...
  test('should clean up URL parameters after successful auth', async ({ page }) => {
    // Try to access protected route
    await page.goto('/driver/dashboard');
