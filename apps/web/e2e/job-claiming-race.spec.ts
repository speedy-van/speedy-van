...
test.describe('Job Claiming Race Conditions & Stepper Flow', () => {
  ...
  test('should handle job claiming race condition', async ({ page, context }) => {
    // Navigate to available jobs
    await page.goto('/driver/jobs/available');
    
    // Wait for jobs to load
    await expect(page.locator('[data-testid="available-jobs-list"]')).toBeVisible();
    ...
    if (page2) {
      ...
      await page2.fill('[data-testid="email-input"]', 'test-driver@speedy-van.co.uk');
      ...
      const successCount = await page.locator('[data-testid="claim-success-message"]').count();
      const failureCount = await page.locator('[data-testid="claim-failed-message"]').count();
  ...
  test('should complete job stepper flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="step-navigate-pickup"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="step-arrived-pickup"]')).toHaveClass(/completed/);
    ...
    await expect(page.locator('[data-testid="step-arrived-dropoff"]')).toHaveClass(/completed/);
    
    // Step 6: Unload items
    await expect(page.locator('[data-testid="step-unload-items"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="job-complete-message"]')).toBeVisible();
    await expect(page.locator('[data-testid="job-complete-message"]')).toContainText('Job completed successfully');
  });
  ...
  test('should prevent skipping steps', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="step-error-message"]')).toBeVisible();
    await expect(page.locator('[data-testid="step-error-message"]')).toContainText('Complete previous steps first');
  });
  ...
  test('should require required proofs for completion', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="proof-error-message"]')).toContainText('Required photos missing');
  });
  ...
  test('should handle job cancellation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="job-cancelled-message"]')).toBeVisible();
  });
});
