...
test.describe('Admin Dashboard Security Tests', () => {
  ...
  test('should enforce least-privilege roles', async ({ page }) => {
    ...
    await page.fill('[data-testid="email-input"]', 'limited-admin@speedy-van.co.uk');
    ...
    await expect(page.locator('[data-testid="bulk-assign-button"]')).not.toBeVisible();
    ...
    await expect(page.locator('[data-testid="approve-application"]')).not.toBeVisible();
  });
  ...
  test('should prevent SSRF attacks', async ({ page }) => {
    ...
    await page.locator('[data-testid="pickup-address"]').fill('http://localhost:3000/admin/internal');
  ...
  test('should prevent CSRF attacks', async ({ page }) => {
    ...
    await page.goto('data:text/html,<html><body><form id="csrf-form" method="POST" action="http://localhost:3000/api/admin/orders/123/assign"><input type="hidden" name="driverId" value="malicious-driver"><input type="submit" value="Submit"></form><script>document.getElementById("csrf-form").submit();</script></body></html>');
    
    // Should be blocked by CSRF protection
    await expect(page.locator('text=CSRF token missing or invalid')).toBeVisible();
  });
  ...
  test('should not log secrets in audit trail', async ({ page }) => {
    ...
    const auditDetails = await auditEntry.locator('[data-testid="audit-details"]').textContent();
  ...
  test('should validate IP allowlist for high-risk actions', async ({ page }) => {
    // Login as admin
    await page.goto('/admin/login');
    await page.fill('[data-testid="email-input"]', 'admin@speedy-van.co.uk');
    await page.fill('[data-testid="password-input"]', 'AdminPassword123!');
    ...
    await expect(page.locator('[data-testid="ip-allowlist-check"]')).toBeVisible();
  });
  ...
  test('should sanitize user inputs', async ({ page }) => {
    ...
    const orderRow = page.locator('[data-testid="order-row"]').first();
    await orderRow.locator('[data-testid="edit-order"]').click();
    await page.locator('[data-testid="order-notes"]').fill('<script>alert("xss")</script>');
    await page.locator('[data-testid="save-order"]').click();
    
    // Should sanitize the input
    await expect(page.locator('[data-testid="order-notes"]')).not.toContainText('<script>');
    
    // Try SQL injection in search
    await page.locator('[data-testid="search-orders"]').fill("'; DROP TABLE orders; --");
