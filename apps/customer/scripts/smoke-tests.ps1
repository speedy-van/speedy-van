...
Write-Host "2. Testing /api/bookings endpoint availability" -ForegroundColor White
try {
    $BookingResponse = Invoke-WebRequest -Uri "$BaseUrl/api/bookings" -Method GET -ErrorAction SilentlyContinue
    Write-Host "✅ Booking API endpoint exists" -ForegroundColor Green
    Write-Host "   Status: $($BookingResponse.StatusCode)" -ForegroundColor Gray
} catch {
    Write-Host "⚠️  Booking API endpoint not found (may be handled differently)" -ForegroundColor Yellow
