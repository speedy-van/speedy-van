    ...
    const transformedDrivers = drivers.map(driver => {
      ...
      const avgRating =
        driver.ratings?.length > 0
          ? driver.ratings.reduce((sum: number, r: any) => sum + r.rating, 0) /
      ...
      if (
        driver.checks?.licenceExpiry &&
        new Date(driver.checks.licenceExpiry) < new Date()
      ) {
        complianceIssues.push('License expired');
      }

      if (
        driver.checks?.rtwExpiresAt &&
        new Date(driver.checks.rtwExpiresAt) < new Date()
      ) {
        complianceIssues.push('Right to work expired');
      }

      if (
        driver.checks?.policyEnd &&
        new Date(driver.checks.policyEnd) < new Date()
      ) {
        complianceIssues.push('Insurance expired');
      }

      if (
        driver.vehicles[0]?.motExpiry &&
        new Date(driver.vehicles[0].motExpiry) < new Date()
      ) {
