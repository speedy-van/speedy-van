  ...
  try {
    ...
    if (STRIPE_SECRET_KEY) {
      console.log(
        ...
        STRIPE_SECRET_KEY.substring(0, 7)
      );
    }
    if (STRIPE_PUBLISHABLE_KEY) {
      console.log(
        '🔧 Stripe Publishable Key prefix:',
        STRIPE_PUBLISHABLE_KEY.substring(0, 7)
      );
    }
    ...
    const validSecretPrefixes = isProduction
      ? ['sk_live_']
      : ['sk_test_', 'sk_live_'];
    const validPublishablePrefixes = isProduction
      ? ['pk_live_']
      : ['pk_test_', 'pk_live_'];

    const hasValidSecretKey = validSecretPrefixes.some(prefix =>
      STRIPE_SECRET_KEY.startsWith(prefix)
    );
    const hasValidPublishableKey = validPublishablePrefixes.some(prefix =>
      STRIPE_PUBLISHABLE_KEY.startsWith(prefix)
    );

    if (!hasValidSecretKey) {
      console.error(
        `❌ Invalid Stripe secret key format for ${NODE_ENV} environment. Expected one of: ${validSecretPrefixes.join(', ')}`
      );
      return NextResponse.json(
        {
    ...
    if (!hasValidPublishableKey) {
      console.error(
        `❌ Invalid Stripe publishable key format for ${NODE_ENV} environment. Expected one of: ${validPublishablePrefixes.join(', ')}`
      );
      return NextResponse.json(
        {
          error: 'Payment service configuration error',
