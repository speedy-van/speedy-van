...
export type AnalyticsEvent =
  | {
      ...
      payload: { bookingId: string; timestamp: number };
    }
  | {
      type: 'step_viewed';
      payload: { step: number; bookingId: string; timestamp: number };
    }
  | {
      type: 'step_completed';
      payload: { step: number; bookingId: string; duration: number };
    }
  | {
      type: 'item_added';
      payload: { itemId: string; quantity: number; bookingId: string };
    }
  | { type: 'item_removed'; payload: { itemId: string; bookingId: string } }
  | {
      type: 'address_selected';
      payload: { addressType: 'pickup' | 'dropoff'; bookingId: string };
    }
  | {
      type: 'schedule_selected';
      payload: { date: string; timeSlot: string; bookingId: string };
    }
  | {
      type: 'service_selected';
      payload: { serviceType: string; bookingId: string };
    }
  | {
      type: 'promo_code_applied';
      payload: { promoCode: string; discount: number; bookingId: string };
    }
  | {
      type: 'booking_confirmed';
      payload: { bookingId: string; totalPrice: number; timestamp: number };
    }
  | {
      type: 'booking_failed';
      payload: { bookingId: string; error: string; timestamp: number };
    }
  | {
      type: 'funnel_dropout';
      payload: { step: number; bookingId: string; reason: string };
    };
    ...
    switch (event.type) {
      ...
      case 'step_viewed':
        (window as any).ga(
          ...
          'event',
          'Booking',
          `Step ${event.payload.step} Viewed`,
          event.payload.bookingId
        );
        break;
      case 'step_completed':
        (window as any).ga(
          ...
          'event',
          'Booking',
          `Step ${event.payload.step} Completed`,
          event.payload.bookingId,
          {
      ...
      case 'funnel_dropout':
        (window as any).ga(
          ...
          'event',
          'Booking',
          `Dropout at Step ${event.payload.step}`,
      ...
      (function (e, a) {
        if (!a.__SV) {
          ...
          try {
            ...
            g &&
              ...
              'mpeditor' === i.action &&
                ...
                history.replaceState(i.state, null, j.pathname + j.search)));
          } catch (m) {}
          var k, h;
          ...
          a.init = function (b, c, f) {
            ...
            var e = a;
            'undefined' !== typeof f ? (e = a[f] = []) : (f = 'mixpanel');
            e.people = e.people || [];
            ...
            k =
              'disable track track_pageview track_links track_forms register register_once alias unregister identify name_tag set_config reset people.set people.set_once people.increment people.append people.union people.track_charge people.clear_charges people.delete_user'.split(
                ' '
              );
          ...
          a.__SV = 1.2;
          const scriptElement = e.createElement('script');
...
export class AnalyticsService {
  ...
  constructor() {
    ...
    if (process.env.NEXT_PUBLIC_GA_TRACKING_ID) {
      this.providers.push(
        new GoogleAnalyticsProvider(process.env.NEXT_PUBLIC_GA_TRACKING_ID)
      );
    }
  ...
  public startBooking() {
    ...
    this.track({
      type: 'booking_started',
      payload: { bookingId: this.bookingId, timestamp: Date.now() },
    });
  ...
  public viewStep(step: number) {
    ...
    this.track({
      type: 'step_viewed',
      payload: { step, bookingId: this.bookingId, timestamp: Date.now() },
    });
  }
  ...
  public completeStep(step: number, duration: number) {
    ...
    this.track({
      type: 'step_completed',
      payload: { step, bookingId: this.bookingId, duration },
    });
  }
  ...
  public addItem(itemId: string, quantity: number) {
    ...
    this.track({
      type: 'item_added',
      payload: { itemId, quantity, bookingId: this.bookingId },
    });
  }
  ...
  public removeItem(itemId: string) {
    ...
    this.track({
      type: 'item_removed',
      payload: { itemId, bookingId: this.bookingId },
    });
  }
  ...
  public selectAddress(addressType: 'pickup' | 'dropoff') {
    ...
    this.track({
      type: 'address_selected',
      payload: { addressType, bookingId: this.bookingId },
    });
  }
  ...
  public selectSchedule(date: string, timeSlot: string) {
    ...
    this.track({
      type: 'schedule_selected',
      payload: { date, timeSlot, bookingId: this.bookingId },
    });
  }
  ...
  public selectService(serviceType: string) {
    ...
    this.track({
      type: 'service_selected',
      payload: { serviceType, bookingId: this.bookingId },
    });
  }
  ...
  public applyPromoCode(promoCode: string, discount: number) {
    ...
    this.track({
      type: 'promo_code_applied',
      payload: { promoCode, discount, bookingId: this.bookingId },
    });
  }
  ...
  public confirmBooking(totalPrice: number) {
    ...
    this.track({
      type: 'booking_confirmed',
      payload: { bookingId: this.bookingId, totalPrice, timestamp: Date.now() },
    });
  ...
  public failBooking(error: string) {
    ...
    this.track({
      type: 'booking_failed',
      payload: { bookingId: this.bookingId, error, timestamp: Date.now() },
    });
  }
  ...
  public trackDropout(step: number, reason: string) {
    ...
    this.track({
      type: 'funnel_dropout',
      payload: { step, bookingId: this.bookingId, reason },
    });
  }
