...
type BookingAction =
  ...
  | {
      type: 'SAVE_DRAFT';
      payload: { data: Partial<BookingFormData>; timestamp: Date };
    }
    ...
    case 'TRACK_ABANDONMENT':
      return {
        ...
        analytics: {
          ...state.analytics,
          abandonmentPoints: [
            ...state.analytics.abandonmentPoints,
            action.payload,
          ],
        },
      };
...
export const BookingProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const validateCurrentStep = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      dispatch({
        type: 'SET_ERRORS',
        payload: { general: ['Validation failed'] },
      });
      return false;
    } finally {
  ...
  const saveDraft = useCallback(async () => {
    try {
      ...
      dispatch({
        type: 'SAVE_DRAFT',
        payload: { data: state.data, timestamp },
      });
