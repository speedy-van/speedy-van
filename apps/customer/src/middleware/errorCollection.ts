...
export class ErrorCollectionMiddleware {
  ...
  private errorService: ErrorMonitoringService;

  constructor(config: Partial<ErrorCollectionConfig> = {}) {
    this.config = { ...defaultConfig, ...config };
    this.errorService = ErrorMonitoringService.getInstance();
  }
  ...
  setupGlobalErrorHandlers(): void {
    if (!this.config.enabled || !this.config.captureUnhandledErrors) {
      return;
    }

    // Handle unhandled promise rejections
    process.on('unhandledRejection', async (reason, promise) => {
      await this.logUnhandledPromiseRejection(reason, promise);
    });

    // Handle uncaught exceptions
    process.on('uncaughtException', async (error) => {
      await this.logUncaughtException(error);
  ...
  private async logUnhandledPromiseRejection(reason: any, promise: Promise<any>): Promise<void> {
    try {
      ...
      console.error('Failed to log unhandled promise rejection:', error);
    }
  }
