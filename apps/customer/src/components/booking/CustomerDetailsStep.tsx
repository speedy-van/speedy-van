...
export default function CustomerDetailsStep({
  ...
  const validateForm = () => {
    ...
    if (!bookingData.customer?.phone?.trim()) {
      ...
      if (
        !phoneRegex.test(bookingData.customer.phone.trim().replace(/\s/g, ''))
      ) {
  ...
  const getFieldIconWithAnimation = (field: string) => {
    ...
    return (
      <Icon
        ...
        transform={
          isHovered ? 'scale(1.2) rotate(5deg)' : 'scale(1) rotate(0deg)'
        }
        filter={
          status === 'success' ? 'drop-shadow(0 0 8px currentColor)' : 'none'
        }
        animation={
          status === 'success' ? `${glow} 2s ease-in-out infinite` : 'none'
        }
      />
    );
  };
            ...
            {['name', 'email', 'phone'].map((field, index) => (
              <VStack key={field} spacing={3} position="relative">
                ...
                <Box
                  ...
                  cursor="pointer"
                  onClick={() =>
                    setHoveredField(hoveredField === field ? '' : field)
                  }
                >
          ...
          <VStack spacing={8}>
            ...
            <FormControl isInvalid={!!errors.name}>
              ...
              <InputGroup size="lg">
                ...
                <InputRightElement>
                  <Text fontSize="xs" color="text.tertiary" fontWeight="medium">
                    {getCharacterCount('name').current}/
                    {getCharacterCount('name').max}
                  </Text>
                </InputRightElement>
              </InputGroup>
              <FormErrorMessage
              ...
              <HStack justify="space-between" mt={3}>
                <Text fontSize="xs" color="text.tertiary" fontWeight="medium">
                  {getCharacterCount('name').current < 2
                    ? '⚠️ Name must be at least 2 characters'
                    : getCharacterCount('name').current > 40
                      ? '⚠️ Consider shortening your name'
                ...
                <SimpleGrid columns={{ base: 1, md: 2 }} spacing={3}>
                  ...
                  >
                    <Text
                      ...
                      fontWeight="medium"
                    >
                      • Int'l Landline: +44 20 7946 0958
                    </Text>
                  </Box>
                </SimpleGrid>
                <Text
          ...
          <HStack spacing={4} position="relative" zIndex={1}>
            ...
            <Box flex={1}>
              ...
              <Text fontSize="sm" color="text.secondary" lineHeight="1.6">
                Your contact information is encrypted and will only be used to
                communicate about your move. We never share your data with third
                parties.
              </Text>
            </Box>
          </HStack>
