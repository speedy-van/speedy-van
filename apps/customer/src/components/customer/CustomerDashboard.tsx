...
export default function CustomerDashboard({ userId }: CustomerDashboardProps) {
  ...
  const [linkingBookings, setLinkingBookings] = useState<Set<string>>(
    new Set()
  );
        ...
        <Alert status="info" borderRadius="md" mb={6}>
          ...
          <Box flex="1">
            <Text fontWeight="medium">
              We found {bookings.unlinkedBookings.length} booking
              {bookings.unlinkedBookings.length !== 1 ? 's' : ''} that might be
              yours!
            </Text>
            <Text fontSize="sm" mt={1}>
              These bookings were made before you created an account. Click the
              link button to connect them to your profile.
            </Text>
          </Box>
        </Alert>
      )}
            ...
            {bookings.linkedBookings.length === 0 ? (
              <Card>
                <CardBody textAlign="center" py={8}>
                  ...
                  <Text color="gray.400">
                    Your bookings will appear here once you make them or link
                    existing ones.
                  </Text>
                </CardBody>
              </Card>
      ...
      <Collapse in={isOpen} animateOpacity>
        <CardBody pt={0}>
          ...
          <Box mt={4}>
            ...
            <HStack spacing={4}>
              <Text>
                <strong>Date:</strong> {formatDate(booking.scheduledAt as any)}
              </Text>
              <Text>
                <strong>Time:</strong> {formatTime(booking.scheduledAt as any)}
              </Text>
              <Text>
                <strong>Created:</strong> {formatDate(booking.createdAt as any)}
              </Text>
              {booking.paidAt && (
                <Text>
                  <strong>Paid:</strong> {formatDate(booking.paidAt as any)}
                </Text>
              )}
            </HStack>
          </Box>
      ...
      <Collapse in={isOpen} animateOpacity>
        <CardBody pt={0}>
          ...
          <Box mt={4}>
            ...
            <HStack spacing={4}>
              <Text>
                <strong>Date:</strong> {formatDate(invoice.scheduledAt)}
              </Text>
              <Text>
                <strong>Crew:</strong> {invoice.crewSize}
              </Text>
              <Text>
          ...
          <Box mt={4}>
            ...
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={2}>
              <Text>
                ...
                {(invoice.distanceCostGBP / 100).toFixed(2)}
              </Text>
              <Text>
                <strong>Access:</strong> £
                {(invoice.accessSurchargeGBP / 100).toFixed(2)}
              </Text>
              <Text>
                <strong>Weather:</strong> £
                {(invoice.weatherSurchargeGBP / 100).toFixed(2)}
              </Text>
              <Text>
                <strong>Items:</strong> £
                {(invoice.itemsSurchargeGBP / 100).toFixed(2)}
              </Text>
              <Text>
                <strong>Crew:</strong>{' '}
                {invoice.crewMultiplierPercent > 0 ? '+' : ''}
                {invoice.crewMultiplierPercent}%
              </Text>
              <Text>
                <strong>Availability:</strong> +
                {invoice.availabilityMultiplierPercent}%
              </Text>
            </SimpleGrid>
          </Box>
        </CardBody>
