...
export default function ItemsGrid(props: ItemsGridProps) {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<CategoryKey>('furniture');
  ...
  function inc(key: ItemKey) {
    setQty(key, (value[key] ?? 0) + 1);
  }
  function dec(key: ItemKey) {
    setQty(key, (value[key] ?? 0) - 1);
  }
  ...
  const filteredByCategory: Record<CategoryKey, CatalogItem[]> = useMemo(() => {
    ...
    for (const cat of CATEGORIES) {
      ...
      res[cat] = q
        ? items.filter(
            i => i.label.toLowerCase().includes(q) || i.key.includes(q as any)
          )
        : items;
    }
    return res;
      ...
      >
        ...
        {recent.length > 0 && (
          <HStack spacing={2} mt={2} overflowX="auto">
            ...
            {recent.map(k => (
              <Badge key={k} colorScheme="orange">
                {k.replace(/_/g, ' ')}
              </Badge>
            ))}
          </HStack>
            ...
            {CATEGORIES.map(cat => (
              ...
              >
                <HStack spacing={2}>
                  <span
                    role="img"
                    aria-label={`${CATEGORY_META[cat].label} icon`}
                  >
        ...
        <Box mt={3}>
          <Box position="relative">
            <Box
              ...
              filter="blur(32px)"
              style={{
                background: `radial-gradient(120px 120px at 20% 10%, ${CATEGORY_META[selectedCategory].color}, transparent 70%)`,
              }}
            />
          </Box>
          <MotionBox
          ...
          <ModalFooter>
            <HStack>
              ...
              <Button
                onClick={() => {
                  if (!customName.trim()) {
                    toast({ status: 'warning', title: 'Enter a name' });
                    return;
                  }
                  // Map all custom items under 'custom' key, accumulate count
                  const next = { ...value } as ItemsGridValue;
                  next['custom' as ItemKey] =
                    (next['custom' as ItemKey] ?? 0) + customQty;
                  onChange(next);
